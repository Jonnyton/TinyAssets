<!--
  ...
  Separates the real Destiny (tiny) reference from TinyAssets' current test tiny rail;
...
<svelte:head>
  ...
  <meta name="description" content="License, terms, privacy, and token disclosures for TinyAssets, test tiny, and the Destiny (tiny) reference." />
</svelte:head>
...
<section class="legal">
  <div class="wrap">
    ...
    <p class="lead">
      TinyAssets is open-source software under MIT. Public goal content is CC0-1.0. Current TinyAssets settlement testing uses <code>test tiny</code> on Base Sepolia. The real currency reference is <code>{legal.token.display_name}</code>, and real-token integration is deferred.
    </p>
    ...
    <div class="chains" aria-label="Reference token contracts">
      {#each tokenInfo.deploys as d (d.address_main)}
        <article class="chain" class:chain--primary={d.primary} class:chain--legacy={d.legacy}>
          ...
          <p class="chain__note">{d.legacy ? 'legacy address; no TinyAssets action' : 'no TinyAssets action — test tiny rail only'}</p>
