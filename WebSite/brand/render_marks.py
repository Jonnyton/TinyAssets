...
export function TinyAssetsMark({{
  ...
  return (
    ...
    >
      {{tile && (
        <rect width="{viewbox:g}" height="{viewbox:g}" rx="{tile_radius:g}" fill="var(--bg-0, {ink})" />
      )}}
      <path d="{path}" fill="var(--fg-1, {cream})" />
      <rect x="{rule_x:g}" y="{rule_y:g}" width="{rule_w:g}" height="{rule_h:g}" fill="var(--ember-600, {accent})" />
    </svg>
  );
}}
