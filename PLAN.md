...
**Agentic hybrid search is memory.** Durable memory is a policy over multiple stores (KG traversal, vector similarity, hierarchical summaries, notes, world-state, direct tool calls). No single *index* owns truth — the canonical source is the bundle (Brain Module), and every index over it is derived and rebuildable. Routing across those indexes matters more than any one of them.
...
**Principles:**
- *Canonical source and retrieval routing are different questions.* The bundle owns truth (below); no single *index* owns retrieval. Routing across stores is the policy; routing matters more than any one index.
