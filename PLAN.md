...
**Authority derives from an unforgeable fact, re-verified at the decision point.** Mutable or insertable state — a `status` column, an unsigned row field, an append-only-but-INSERT-permitted event — may *narrow*, *reject*, or *audit* an authority decision. It may never *grant* one. Anything an attacker can reach with a single DML write is not a trust root, and a decision that reads one is forgeable no matter how the surrounding checks are ordered. The re-verification happens where the decision is made, not where the record was written; a cached projection of a verified fact is a cache, never authority. Whether a given read is an authority read is decided by what it *permits*, not by which table it touches — a display or reject-only read of the same mutable column is fine. See the Design Decisions entry below for the three verification mechanisms this principle resolves to.
...
- *Brain conditions every authority decision.* Permissive by default — Brain logs and hints. Per-Goal opt-in to strict mode where Brain may refuse to authorize a contradicting write. Conditioning is a *narrowing* path only: Brain memory is mutable state, so it may reject or constrain a decision but never supply the fact that grants it (see Cross-Cutting Principles, and the unified authority-derivation entry in Design Decisions).
...
- **Universe intelligence owns the user-facing control flow; the chatbot is a thin relay.** Host reshape 2026-07-02. The relay/`converse` M1 is built and deployed, and is turn-scoped — the proactive 24/7 persistent loop remains planned, not shipped. Each universe has one first-party personified intelligence on an LLM assigned at create; it is the sole writer of its own brain and speaks first-person. "Sole action-taker" scopes to that control flow, not to every write: the data-plane primitives (`write_graph`/`write_page`) stay daemon-free and directly callable, so founders remain a second authorized write principal (this resolution touches the load-bearing zero-daemons-for-authoring principle and still needs host/navigator confirmation — design note §6.2). The connecting chatbot MCP client onboards and relays — it never embodies the universe and never composes its voice. This REVERSES the earlier "chatbot embodies, never relays" model, which was live-falsified: a behavioral contract delivered in a tool result is indistinguishable from prompt injection, so careful hosts correctly refuse it. Embodiment is therefore only expressible through first-party channels (server `instructions`, `control_station`, user-invoked `meet_universe`). Requirements: `openspec/specs/universe-personification-and-relay/spec.md`; rationale: `docs/design-notes/2026-07-02-universe-intelligence-relay-architecture.md`.
...
- **Unified authority derivation — one principle, three verification mechanisms.** Host decision 2026-07-20, taken after the same defect class ("trust mutable/insertable DB state as authority") surfaced independently in lease completion, daemon identity/tokens, GitHub review-and-merge, paid-market claim ownership and settlement, and blob refs. Asked whether to keep closing it surface-by-surface or rebuild the shape once, the host chose one unified derivation model. The unifying layer is the Cross-Cutting Principle above plus a single `Verified[T]` return type, so every authority-consuming call site looks the same — **not** a single constructor. Underneath it there are three distinct mechanisms, and collapsing them was explicitly rejected as a false unification:
  - **M1 — platform signature.** The platform signed a canonical payload with a custody-held key (domain-separated ed25519 over canonical JSON). Covers lease grant, completion attestation, enrollment approval, claim ownership. *This is the only mechanism that unifies*: one `RecordVerifier`, whose verify step is parse → verify signature → assert every mutable row column equals the signed binding.
  - **M2 — content addressing.** The identifier *is* the content hash, so changing the bytes changes the id. Covers blob refs, git `head_sha`, capsule digests, receipt ids. Verified by re-hashing or a CAS-existence check at the decision point. Signing these would add a platform-asserted fact that proves strictly less than the hash already does.
  - **M3 — external re-confirmation.** An external trust root owns the fact and signs it its own way. Covers WorkOS human identity, GitHub approval and merge state, ledger-backed settlement. Verified against that source at the decision point. Forcing these to be re-signed by the platform would manufacture a second, weaker source of truth beside the real one.

  **Enforcement is structural, not conventional.** The guarantee comes from verify-key custody — no consumer module holds a raw verify key, so a consumer that wants a `Verified[T]` must route through the verifier — backed by mutation-probe tests that forge the mutable row via raw DML and assert the decision still fails closed. A semantic lint over column names was considered and rejected: it cannot distinguish an authority read from a display or reject-only read, so it false-positives every projection and false-negatives every new column.
