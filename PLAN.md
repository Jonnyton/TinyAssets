...
**In scope:** Daemon identity (souls, fingerprints, forks), credential-bound runtime execution, soul eligibility per node/gate, soul-guided dispatch, the live file-locked claim bridge, and its fail-closed migration to server-authoritative transactional activation and claiming.
...
- *Always ready for the next user and daemon.* Multi-tenant from the first build. Storage, authorization, queues, budgets, audits, daemon bindings, and runtime activations carry tenant/owner boundaries without assuming pre-provisioned workers.
...
- *Credential cardinality is user-chosen, not platform-shaped.* A universe vault may contain zero, one, or many credentials. Runtime capacity follows the credentials and endpoints the user has explicitly bound; the platform has no fixed worker count or provider composition.
...
- *One credential authority per execution.* Daemon and opt-in requester-host runtimes may execute the same immutable Branch contract, but every LLM turn resolves the workflow's current serving binding and requester-owned credential. Stop/cutover/rollback advances transactional authority with compare-and-swap; missing, stale, partitioned, or unavailable credentials hold work rather than widening authority.

**Substrate:** `tinyassets/branch_tasks.py` / `tinyassets/singleton_lock.py` retain the queue and live epoch-1 bridge; the daemon consumes queued work and resolves its universe's assigned serving credential before each LLM turn. `tinyassets/branch_tasks_v2.py` and `tinyassets/storage/request_admissions.py` provide transactional successor seams. The target also uses `tinyassets/provider_serving_binding.py`, `tinyassets/credential_vault.py`, `tinyassets/identity.py`, `tinyassets/discovery.py`, `tinyassets/runtime/`, and the canonical transactional control plane. Soul/fork machinery currently lives in the `author_definitions` substrate transitioning to a domain-agnostic daemon registry (content provenance retains `author_id` + `author_kind` discriminator). The approved custom-agent successor uses immutable public definitions, component lineage, and private universe bindings before runtime activation. Soul-guided dispatch read path landed via open-brain v2 slice B 2026-05-19.
...
**Purpose:** Resolve the credential explicitly assigned to a workflow and preserve role separation without hiding failure.

**In scope:** Provider registry, universe credential vaults, per-workflow serving bindings, local/self-hosted endpoint credentials, and provider-specific adapters.
...
- *Execution is credential-driven and fail-closed.* A universe vault may hold any user-chosen mix of subscription, API-key, and self-hosted/local-router credentials. Each workflow resolves only its assigned serving binding at execution time. If the binding is absent or its credential is unavailable, work holds with a typed reason such as `no_requester_owned_executor`; the platform never borrows a host credential or tries another provider.
- *Fallback is workflow design, never provider routing.* Users may explicitly model retries or alternatives as workflow steps with their own serving bindings. The provider layer has no fallback chains, free-provider chains, implicit provider switching, or writer-pin override.
...
**Substrate:** `tinyassets/providers/`, `tinyassets/credential_vault.py`, `tinyassets/provider_serving_binding.py`, and `tinyassets/provider_assignment.py`. Required-file and assigned-credential availability failures are reported through typed status/hold evidence.
