...
- *Universe authority is the capability boundary.* A universe may use only the
  provider capabilities its user has explicitly bound to it. Voice is available
  when that universe has a compatible user-owned subscription, credential, or
  local resource; otherwise Voice is shown as a capability the user can unlock
  by binding one. Platform or maintainer credentials MUST NOT silently supply a
  missing capability or aggregate usage across users.
