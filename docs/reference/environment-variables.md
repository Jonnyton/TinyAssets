...
| `TINYASSETS_MAX_CONCURRENT_PROVIDER_CALLS` | How many provider SUBPROCESSES may run at once. Measured 2026-08-28: each costs ~189 MB RSS / ~77 MB PSS, floor, and the anyio threadpool admits 40 sync handlers — so without this bound, 40 simultaneous turns meant 40 subprocesses and an OOM that kills the host (and the Cloudflare tunnel with it). Raise it with RAM, not cores. | `6` (fits the 2 GB box). |
