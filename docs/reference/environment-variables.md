...
| `TINYASSETS_NON_AMBIENT_WORK` | Arms the non-ambient work gate (design note 2026-07-15 gap G7): when on, the cloud-worker supervisor works a universe ONLY if it has engine/daemon capacity explicitly bound to it (`tinyassets/engine_binding.py`); an unbound universe is honestly idle-until-bound, and a declared-but-broken binding fails loud. Off = byte-for-byte today's behavior (ambient work). Flipping this on is NOT the host-gated production switch-off of the platform-global daemon. | `off`. |
| `TINYASSETS_BYO_VAULT_ENCRYPTED` | Gates the executable BYO-API-key path (deposit + execution + direct BYO routing), INDEPENDENT of the non-ambient flag. Hosted BYO keys are stored plaintext-base64 in the per-universe vault; the 2026-07-02 credential-custody research requires KMS-wrapped / external-secret-manager storage before the platform may hold + execute founder keys. OFF (this deploy) → the executable BYO path is DARK: `set_engine` refuses BYO deposits, `resolve_engine_binding` never reports a BYO key as bound, and the provider router does no direct BYO routing. Flip ON only once envelope encryption is implemented AND verified. | `off`. |
