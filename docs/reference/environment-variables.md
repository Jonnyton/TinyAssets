...
| `TINYASSETS_SLACK_SIGNING_SECRET` | Slack app signing secret. Sole trust anchor for `POST /mcp/app/slack/events` — every admitted event is authenticated by HMAC over the exact request bytes. **No default, by design:** an empty key still produces a verifying HMAC, so a fallback would be a silent auth bypass rather than a degraded mode. Must be at least 16 characters — Slack issues 32 hex chars, and a short key is brute-forceable, so a truncated paste or placeholder is refused rather than run with. A real secret; supply via `/etc/tinyassets/env` or the vault, never a committed file. | Unset; the ingress route refuses every request. |
| `TINYASSETS_SLACK_API_APP_ID` | Slack `api_app_id` the ingress accepts (e.g. `A0BN1Q98MTQ`). Events addressed to any other app are refused with the same fixed response as a bad signature, so the endpoint is not an oracle for which apps are installed. | Unset; the ingress route refuses every request. |
| `TINYASSETS_SLACK_BOT_USER_ID` | The Slack user id this app posts as (from `auth.test`). **Load-bearing:** the reply this app sends re-enters as a `message` event, and if the workspace credential is ever a *user* token that reply carries no bot marker and a fresh `event_id`, so the dedup ledger cannot stop it and the universe answers itself indefinitely on the user's subscription. Recognising our own author id breaks that cycle. Slack reply dispatch refuses to answer at all while this is unset — a missing value is inert, never a loop. | Unset; no Slack event is answered. |
| `TINYASSETS_SLACK_TEAM_IDS` | Comma-separated Slack workspace (team) IDs permitted to deliver events. **Load-bearing:** the signing secret is per-*app*, not per-workspace, so a valid signature proves which app is talking and never which workspace — an attacker who installs the app in a workspace they control otherwise produces perfectly valid events. Empty or unset admits nobody. | Unset; no workspace may deliver. |
