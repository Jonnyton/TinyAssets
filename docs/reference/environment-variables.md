...
| `TINYASSETS_SESSION_SEAL_KEY` | AES-GCM key sealing the onboarding app's server-side AuthKit refresh-token store (`tinyassets/onboarding/session_store.py`, `$TINYASSETS_DATA_DIR/.runtime/app_refresh_sessions/`), and the HMAC key for the record filenames. Exactly 32 bytes as base64url or hex (`python -c "import secrets,base64;print(base64.urlsafe_b64encode(secrets.token_bytes(32)).decode())"`). Read ONCE at first store use and immediately popped from `os.environ`, so no provider subprocess inherits it — never re-export it into a child env. Rotating it invalidates every live session (users re-login once). A set-but-malformed value logs an error and falls back to the ephemeral key rather than failing the surface. Vault-first: production supplies it through `/etc/tinyassets/env`; never a committed plaintext file. | Unset — an ephemeral `secrets.token_bytes(32)` per process, with one logged warning: sessions do not survive a daemon restart. |
