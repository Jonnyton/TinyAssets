...
| `WORKOS_API_KEY` | WorkOS management key (`sk_…`). Used by WorkOS Pipes (`tinyassets/workos_pipes.py`) and by account deletion to delete the user record (`tinyassets/account_deletion.py`); unset → deletion reports `identity: not_configured` and the host finishes it by hand. |
...
| `TINYASSETS_REALTIME_VOICE_ENABLED` | Makes the shared app's foreground Voice adapter available. A universe without a compatible user-bound voice resource sees the capability as locked. This flag supplies no credential or spend authority; `TINYASSETS_ALLOW_REALTIME_VOICE_API` must also be truthy for the initial OpenAI adapter. | `off`. |
| `TINYASSETS_ALLOW_REALTIME_VOICE_API` | Defense-in-depth kill switch for the initial OpenAI Realtime adapter. It is not user spend authority: the adapter still requires the authenticated universe's compatible deposited API credential. It never reads an ambient/platform `OPENAI_API_KEY`, treats Codex subscription auth as Realtime API auth, enables API-key primary writers, or changes `TINYASSETS_ALLOW_API_KEY_PROVIDERS`. | `off`. |
