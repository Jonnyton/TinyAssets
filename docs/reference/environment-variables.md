...
| `TINYASSETS_MAX_CONCURRENT_PROVIDER_CALLS` | How many provider SUBPROCESSES may run at once — the binding capacity constraint on real turns. Sized from measurement on the live 2 GB box with verified overlap: 25 concurrent cost ~786 MB, i.e. ~31 MB marginal each (page sharing improves with concurrency; the ~77 MB average at LOW concurrency should not be extrapolated linearly). Default keeps ~3x headroom because that measurement is a floor — `--version` loads no prompt, history or tools. Watch `refused` / `peak_concurrent` in `get_status.provider_admission` before changing it. | `10`. |
