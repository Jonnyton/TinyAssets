...
Isolated temporary-database probes on 2026-07-25 confirmed the call-site findings against the current branch without changing repository state. With Alice as Goal owner and Bob as private-branch author, `goals action=set_canonical` returned `status="ok"` and persisted Bob's private `branch_version_id`; `goals action=bind` returned `status="bound"` and persisted Alice's Goal ID on Bob's private branch. With Mallory as caller, `gates action=claim` returned `status="claimed"` and persisted `claimed_by="mallory"` against Bob's private branch. With Alice as caller, `extensions action=dry_inspect_node` returned no error and included Bob's `secret_node` plus its private prompt template. These are direct authorization failures, not inferred timing or count side channels.
