...
| [`community_branches_phase5.md`](../specs/community_branches_phase5.md) | CLAIMED | `shared-goals-and-convergence` owns Goal/binding/version/leaderboard/convergence primitives, but authenticated nonauthors can currently list/search/get private Goal records. `harden-canonical-absolute-guarantees` owns reconciliation of that legacy author-and-host-only promise with the exact as-built limitation. |
...
| STATUS `harden-canonical-absolute-guarantees` | Private Goal visibility plus its existing money/attribution/birth/learning/receipt absolutes. | Active paid/universe/relay owners merge first. |
