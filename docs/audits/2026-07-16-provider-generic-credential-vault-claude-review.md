...
1. **Adopt Codex's crypto primitive over my AES-GCM suggestion.** XChaCha20-Poly1305-IETF AEAD with the canonical identity as **AAD on both the DEK-wrap and the payload** is the chosen primitive; the AAD binding is what makes ciphertext/wrapped-DEK swapping fail authentication. Use Codex's primitive.

   > **Correction (2026-07-16, post Codex security re-review, finding 7):** the rationale above misstated *why* XChaCha is preferred. XChaCha is **not** "stronger than AES-256-GCM because it supports AAD" — AES-GCM authenticates AAD too. The real reason is operational: XChaCha's **192-bit nonce** makes independent RANDOM per-record nonces collision-safe with no counter/nonce-management state, whereas AES-GCM's 96-bit nonce would need careful nonce management to reach the same margin. AAD binding of the immutable identity (and now store identity) is done identically regardless of cipher. The code comment in `tinyassets/credentials/crypto.py` carries the corrected rationale.
