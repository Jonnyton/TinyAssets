...
| GitHub PR effect and receipts | `tinyassets/effectors/github_pr.py` enforces soul authority, exact destination secret resolution, consent, atomic reservation, remote reconciliation, and PR materialization. `tinyassets/storage/external_write_receipts.py` persists reserve/finalize/reconcile states. | Repository secret inventory on 2026-07-29 showed `WORKFLOW_GITHUB_PR_CAPABILITIES` exists while `TINYASSETS_GITHUB_PR_CAPABILITIES` does not; deploy checks only the latter. Recovery run 30515117371 skipped the deploy job, so it is recovery evidence for service health, not for this secret mismatch. Consent's optional `granted_by` is also caller-influenced. | Immediate collision-free repair: validate the single legacy source, install only the exact TinyAssets destination map, and preserve absence/invalid-input revocation tests. Then bind consent/authority to the authenticated owner. |
