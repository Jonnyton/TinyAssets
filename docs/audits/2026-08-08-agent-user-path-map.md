...
|---|---|---|---|---|
| A0 | Have the agent REMEMBER the conversation across turns | SDK agents run over an accumulating `messages` array — memory is the substrate | PROVEN 2026-08-08 | Was MISSING (stateless turn). Fixed via SDK load-on-start: daemon `app_ingress` loads the DM timeline and feeds bounded, delimited, untrusted history into the turn (founder-gated; Codex ADAPT applied). Live proof: "what did I tell you my favorite test topic is?" → "Tide pool ecology" (recalled from an earlier message it previously could not see). Follow-ups: durable action-attempt ledger for safe retry; tier-preserving multi-party history — memory `agent-needs-cross-turn-memory` |
