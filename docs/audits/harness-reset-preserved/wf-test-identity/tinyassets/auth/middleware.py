...
def request_identity_snapshot() -> dict[str, bool | str]:
    """Report only this request's bearer presence and resolved subject.

    The bearer value, username, capabilities, and provider metadata are
    intentionally absent. There is no subject argument: callers can observe
    only the identity already resolved for their own request.
    """
    identity = current_identity()
    subject = (getattr(identity, "user_id", "") or "").strip() or "anonymous"
    return {
