...
from tinyassets.api.identity import (
    attach_request_identity as _attach_request_identity,
)
from tinyassets.api.identity import reset_identity as _reset_identity_impl
...
def get_status(universe_id: str = "") -> str:
    ...
    """
    return _attach_request_identity(_get_status_impl(universe_id=universe_id))
...
def reset_identity(confirm: bool = False) -> str:
    """Reset the authenticated caller's own first-contact state.

    This operation never accepts a principal argument: the subject is derived
    from the bearer-authenticated request. It removes only that subject's
    universes, founder-home binding, and ACL grants. Branch commons, run
    history, the wiki, and every other principal remain untouched.

    Args:
        confirm: False returns the scoped plan; True executes it. Repeating a
            confirmed reset is safe and an unknown identity is a no-op.
    """
    return _reset_identity_impl(confirm=confirm)


_mcp_reset_identity = _register_structured_tool(
    reset_identity,
    title="Reset My Identity State",
    tags={"identity", "reset", "testing"},
    anonymous_write_challenge=True,
    annotations=ToolAnnotations(
        title="Reset My Identity State",
        readOnlyHint=False,
