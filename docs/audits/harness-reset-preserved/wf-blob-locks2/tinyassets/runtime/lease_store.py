...
from tinyassets.runtime.blob_refs import BlobError, BlobProofError, BlobRef, BlobStore
        ...
        *,
        ...
        with self._blob_transaction(blob_store) as connection:
            ...
            with blob_store.completion_validation_guard():
                try:
                    references = result_blob_references(candidate)
                    blob_proofs = tuple(
                        blob_store.validate_reference(
                            blob_ref,
                            owner_user_id=grant["owner_user_id"],
                    ...
                    for (blob_ref, sha256, size_bytes), proof in zip(
                        references, blob_proofs, strict=True
                    ):
                        if not isinstance(proof, Verified) or not isinstance(
                            proof.payload, BlobRef
                        ):
                            raise BlobProofError(
                                "completion requires a verified blob proof"
                            )
                        if (
                            proof.payload.ref != blob_ref
                            or proof.payload.sha256 != sha256
                            or proof.payload.size_bytes != size_bytes
                        ):
                            raise BlobProofError(
                                "blob verifier returned a proof for different content"
                            )
                except BlobError as exc:
                    raise CandidateValidationError(str(exc)) from exc
