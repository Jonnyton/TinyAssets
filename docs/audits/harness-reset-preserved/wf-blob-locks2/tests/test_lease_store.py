...
def test_record_candidate_marks_result_blobs_referenced(tmp_path: Path) -> None:
    ...
    class BlobStoreSpy:
        ...
        def mark_referenced(self, blob_proof, **binding):
            self.marked.append((blob_proof, binding))
            return self._inner.mark_referenced(blob_proof, **binding)
    ...
    assert [proof.payload.ref for proof, _ in spy.marked] == [
        ref for ref, _ in spy.validated
    ]
...
def test_completion_rejects_raw_blob_ref_from_validation_adapter(tmp_path: Path) -> None:
    from tinyassets.runtime.blob_refs import BlobStore

    class RawProofBlobStore(BlobStore):
        def __init__(self, inner):
            self._inner = inner

        def serialization_guard(self):
            return self._inner.serialization_guard()

        def completion_validation_guard(self):
            return self._inner.completion_validation_guard()

        def validate_reference(self, blob_ref, **binding):
            return self._inner.validate_reference(blob_ref, **binding).payload

    store, task, _, blobs, key, _, raw, expected, clock = _result_lease(tmp_path)
    _record_candidate(store, task, blobs, key, raw, clock.now)

    with pytest.raises(CandidateValidationError, match="verified blob proof"):
        store.complete_validated_result(
            task.branch_task_id,
            expected=expected,
            blob_store=RawProofBlobStore(blobs),
        )


def test_completion_rejects_verified_proof_for_different_content(tmp_path: Path) -> None:
    from tinyassets.runtime.blob_refs import BlobStore

    store, task, lease, blobs, key, _, raw, expected, clock = _result_lease(tmp_path)
    _record_candidate(store, task, blobs, key, raw, clock.now)
    other_content = b"valid proof for the wrong blob"
    other_sha256 = hashlib.sha256(other_content).hexdigest()
    ...
    blobs.write_upload(upload.upload_id, other_content)
    other = blobs.commit_blob(
        upload.upload_id,
        owner_user_id="user:owner-1",
        daemon_id="daemon:builder-1",
    )

    class WrongProofBlobStore(BlobStore):
        def __init__(self, inner):
            self._inner = inner
        ...
        def validate_reference(self, _blob_ref, **binding):
            return self._inner.validate_reference(
                other.ref,
                **dict(
                    binding,
                    expected_sha256=other.sha256,
                    expected_size_bytes=other.size_bytes,
                ),
            )
    ...
    with pytest.raises(CandidateValidationError, match="different content"):
        store.complete_validated_result(
            task.branch_task_id,
            expected=expected,
            blob_store=WrongProofBlobStore(blobs),
        )
