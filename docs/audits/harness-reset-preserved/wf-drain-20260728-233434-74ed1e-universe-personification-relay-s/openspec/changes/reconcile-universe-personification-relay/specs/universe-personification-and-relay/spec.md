...
### Requirement: The anti-collision boundary uses explicit canonical routing, not prose classification
...
Accordingly, canonical `write_page` SHALL route free-form writes by an explicit external
selector rather than classifying their prose. `scope="commons"` SHALL write only to the shared
public commons and SHALL reject a simultaneous `universe_id`. Omitted scope SHALL preserve the
existing private-canon path: resolve the authenticated founder's universe and return the
`relay_to_universe` envelope naming `converse`, without directly writing that universe's brain.
Unknown selectors SHALL fail closed without mutation. Typed `kind=` filings remain commons
coordination operations.

This external router SHALL NOT restrict the universe's own governed learning path, which
deliberately and correctly persists a description of the founder to `founder.md`
(`universe_intelligence.py` `_GROUNDING_FILES`, `"founder.md": "<markdown: who my founder is>"`).
The selector is the enforceable predicate; an unscoped "reject profile-shaped writes" rule
would reject legitimate public `people` pages and contradict landed founder learning
(provenance: retired task 2.6, Codex review 2026-07-22 finding 2, host decision 2026-07-25).
...
- **WHEN** an authenticated caller invokes canonical `write_page` with `scope="commons"`
- **THEN** the write targets only the shared public commons, regardless of prose shape
- **AND** a simultaneous `universe_id` is rejected without mutation

#### Scenario: omitted scope preserves the private-canon relay
- **WHEN** an authenticated caller invokes canonical `write_page` without scope
- **THEN** the result relays to the resolved universe through `converse`
- **AND** no external page-write path writes that universe's brain directly

#### Scenario: an unknown scope fails closed
- **WHEN** a caller supplies an unsupported scope
- **THEN** no page is mutated and the error names `commons` as the explicit public selector
