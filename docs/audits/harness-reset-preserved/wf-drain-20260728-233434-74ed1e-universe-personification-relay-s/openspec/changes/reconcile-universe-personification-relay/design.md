...
**D5 — Anti-collision is explicit canonical routing, not prose classification.** The host
resolved #1583 on 2026-07-25: canonical `write_page` accepts `scope="commons"` for an
intentional shared-public write, while omitted scope preserves the existing private-canon relay
to the founder's universe through `converse`. The selector is the enforceable predicate; the
platform does not guess whether text looks like a profile or dossier because legitimate public
`people` pages belong in the commons. A commons selector combined with `universe_id` is
contradictory and fails without mutation; unknown selectors fail closed. Typed `kind=` filings
remain commons coordination operations. The universe intelligence's in-process governed
learning path is outside this external router, so founder learning remains legal.

This recovery slice is deliberately canonical-only. The deprecated `wiki` tool and directory
server paths inventoried by draft PR #1583 are retirement/migration surfaces, not authority to
expand this change into a cross-surface rewrite.
