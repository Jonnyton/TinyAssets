...
def _create_universe_as(founder: str, text: str = "A founder seed.") -> dict:
...
class TestFounderWriteBoundary:
    ...
    def test_created_universe_not_writable_by_other_founder(self, universe_base):
        created = _create_universe_as("alice")
...
class TestPrivateCanonRelay:
    ...
    def test_founder_page_write_is_relayed_not_written(self, universe_base):
        ...
        created = _create_universe_as("carol")
        assert created.get("status") == "created", created
    ...
    def test_explicit_commons_scope_writes_public_page_not_founder_brain(
        self, universe_base
    ):
        from tinyassets.universe_server import write_page
        ...
        out = json.loads(write_page(
            scope="commons",
            category="people",
            filename="public-builder",
            content="# Public Builder\n\nA profile intentionally shared in the commons.",
            dry_run=False,
        ))
        ...
        for universe_dir in universe_base.glob("u-*"):
            assert not list(
                (universe_dir / "wiki").rglob("public-builder.md")
            )

    def test_explicit_commons_scope_rejects_universe_target_without_writing(
        self, universe_base
    ):
        from tinyassets.universe_server import write_page

        uid = "u-commons-conflict"
        _authenticate(
            "carol",
            _FOUNDER_SCOPES + ["tinyassets.wiki.write", "tinyassets.wiki.read"],
        )
        ...
        out = json.loads(write_page(
            ...
            universe_id=uid,
            category="people",
            filename="ambiguous-target",
            content="This must not be written anywhere.",
            dry_run=False,
        ))

        assert "error" in out, out
        assert "scope=commons" in out["error"]
        assert not list(universe_base.rglob("ambiguous-target.md"))

    def test_unknown_page_scope_fails_closed_without_writing(self, universe_base):
        from tinyassets.universe_server import write_page
        ...
        out = json.loads(write_page(
            scope="private",
            category="people",
            filename="unknown-scope",
            content="This must not be written anywhere.",
            dry_run=False,
        ))

        assert "error" in out, out
        assert "commons" in out["error"]
        assert not list(universe_base.rglob("unknown-scope.md"))

    def test_unknown_scope_cannot_ride_reserved_canary_authority(
        self, universe_base
    ):
        ...
        token = set_wiki_canary_authority(True)
        try:
            out = json.loads(write_page(
                scope="private",
                category="notes",
            ...
            reset_wiki_canary_authority(token)

        assert "error" in out, out
        assert not list(universe_base.rglob("uptime-probe.md"))

    def test_issue_filing_stays_on_commons_not_founder_home(self, universe_base):
        from tinyassets.universe_server import write_page

        created = _create_universe_as("dave")
        assert created.get("status") == "created", created
        uid = created["universe_id"]
        _authenticate(
            "dave",
    ...
    def test_anonymous_page_write_does_not_resolve_to_a_universe(
        ...
        created = _create_universe_as("erin")
        assert created.get("status") == "created", created
        uid = created["universe_id"]
        _authenticate_anonymous()
        write_page(
        ...
        erin_hits = list(
            (universe_base / uid / "wiki").rglob("stray-note.md")
        )
...
class TestBrainWriteDoorsClosed:
    ...
    def test_brain_write_action_is_relayed_not_dispatched(
        ...
        created = _create_universe_as("gwen")
        assert created.get("status") == "created", created
        uid = created["universe_id"]
    ...
    def test_read_action_still_dispatches(self, universe_base):
        ...
        created = _create_universe_as("hank")
        assert created.get("status") == "created", created
