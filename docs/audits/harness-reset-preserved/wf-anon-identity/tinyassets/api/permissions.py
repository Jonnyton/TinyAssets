...
def is_unresolved_external_request() -> bool:
    """Return whether an OAuth-backed request has no resolved principal.

    Dev/no-auth mode deliberately keeps its local single-tenant behavior. In
    every mode that gates writes on identity, however, an anonymous request is
    an unresolved external caller and must not inherit host-local identity or
    universe defaults.
    """
    ...
    try:
        ...
        return _get_provider().writes_require_identity()
    except Exception:
        # Identity-mode uncertainty is itself unresolved identity.
        return True
