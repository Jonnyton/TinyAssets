...
def _resolve_entry_universe(universe_id: str) -> tuple[str, bool]:
    ...
    - An unresolved OAuth-backed caller gets no universe scope.
    - A dev/no-auth caller uses the legacy default resolution
...
def get_status(universe_id: str = "", *, allow_first_contact_birth: bool = True) -> str:
    ...
    if permissions.is_unresolved_external_request():
        return json.dumps({
            ...
            "about": (
                "TinyAssets connects you to public work without signing in. "
                "Sign in to resolve your account and universe."
            ),
            "next_step_for_user": (
                "Sign in before asking for your universe or daemon status."
            ),
        })
