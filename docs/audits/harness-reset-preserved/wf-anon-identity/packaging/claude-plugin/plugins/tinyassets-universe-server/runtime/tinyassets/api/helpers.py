...
def _request_universe(universe_id: str = "") -> str:
    ...
    cross-founder on `universe action=inspect` and friends); a dev/no-auth
    caller keeps the legacy single-tenant default resolution. An unresolved
    OAuth-backed caller gets no implicit universe scope; an explicit public
    universe id still resolves normally. Unlike get_status's
    ``_resolve_entry_universe`` this NEVER creates a universe.
    """
    ...
    if not permissions.is_authenticated_request():
        if permissions.is_unresolved_external_request():
            return ""
