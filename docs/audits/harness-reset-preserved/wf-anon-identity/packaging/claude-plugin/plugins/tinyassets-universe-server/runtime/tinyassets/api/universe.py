...
def _action_list_universes(**_kwargs: Any) -> str:
    ...
    for child in sorted(all_entries):
        ...
        if permissions.is_unresolved_external_request():
            info = {
                "id": child.name,
                "has_premise": bool(read_legacy_premise(child).strip()),
                "has_soul": (child / SOUL_FILENAME).is_file(),
            }
        else:
...
def _action_inspect_universe(universe_id: str = "", **_kwargs: Any) -> str:
    ...
    # Daemon liveness is host-operational state, not public universe content.
    if not permissions.is_unresolved_external_request():
