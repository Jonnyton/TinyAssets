...
def test_effect_loader_returns_only_platform_signed_capsule_bindings(
    ...
    assert payload["universe_scope"]["universe_id"] == "universe:alpha"
    assert payload["base"]["commit"] == "3" * 40
    assert payload["base"]["tree"] == "4" * 40
...
def test_legacy_active_authenticated_lease_is_reset_without_owner_inference(
    ...
    with sqlite3.connect(store.db_path) as connection:
        ...
        connection.execute("ALTER TABLE lease_tasks DROP COLUMN lease_owner_user_id")
