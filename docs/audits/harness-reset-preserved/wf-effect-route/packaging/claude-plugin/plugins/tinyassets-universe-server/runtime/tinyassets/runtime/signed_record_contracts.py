...
def _validate_lease_grant(
    ...
    if capability_class == "repo":
        ...
        raise ValueError("platform lease grant is missing or malformed")
    if (
        not _SHA256_RE.fullmatch(payload["runner_policy_sha256"])
        or not _OCI_DIGEST_RE.fullmatch(payload["image_digest"])
        or not _GIT_OBJECT_RE.fullmatch(payload["base_commit"])
        or not _GIT_OBJECT_RE.fullmatch(payload["base_tree"])
    ):
        raise ValueError("platform lease grant is missing or malformed")
...
def _validate_completion_attestation(
    ...
    if context.row_status in _TERMINAL_STATUSES:
        if context.row_status != payload["status"]:
            raise ValueError("terminal row status does not match signed attestation")
    ...
    for field, row_value, expected_value in (
        ...
        if row_value is not None and row_value != expected_value:
            raise ValueError(f"terminal row {field} does not match signed attestation")
...
LEASE_GRANT_V2_CONTRACT = SignedRecordContract(
    ...
    fields={
        ...
        "universe_id": _specialized(str),
        "base_commit": _specialized(str),
        "base_tree": _specialized(str),
    },
