...
| ID | Item | Owner | Note |
|---|---|---|---|
| R1 | **Coding-loop eval gate** — **S1 DONE.** `tests/test_acceptance_suite_ci.py` is a rubric'd, *gating* output-eval (min_score 0.9, min-aggregation, named task-success dimensions) run as a normal pytest → CI-gated with no `.yml` change and **zero production-behavior change** (suite-local dispatcher registration). Verified: 3 passed (passes a good call, *blocks* a malformed one), Codex-reviewed SHIP after one fix (`sqlite_only`). **S2** (wire `coding_packet_rubric` into `auto_ship.validate_ship_request`) + **S3** (coding trajectory schema) remain — they change production accept-behavior, so they stay gated for navigator + a watched rollout. | S1: done · S2/S3: navigator + owner | Design note + §Codex review: `docs/design-notes/2026-06-24-coding-loop-eval-gate-wiring.md`. |
...
| R3 | **AGENTS.md dedup — DONE.** Deleted the duplicate "Where new conventions live" block (cross-provider-drift clean). The ~248-ln worktree-manual move into `git-workflow-and-versioning` + the AGENTS byte budget are now governed by **ADR-002**; that bigger reduction needs the host's budget sign-off. | navigator + host (budget number) | Dedup landed; the lean/layer move is the remaining, host-gated piece. |
...
| R8 | **ui-test re-run** — fresh live-connector proof. **Cannot be produced by a background session** — needs the live tunnel + a human-driven chatbot UI. The only structural blocker in this list. | host (live + human) | Not an engineering gap; promoting it to a scored scenario ties to R1's later slices. |
