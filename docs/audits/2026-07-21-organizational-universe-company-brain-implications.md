...
of the checkpoints recorded below. Re-check every code, spec, product, privacy,
and operational prescription against fresh sources and current `origin/main`
before using it. The Claude verdict remains **ADAPT**, not approval or build
authority.
...
authorize an OpenSpec or implementation lane. The host-owned private-data/PLAN
decision and the independently identified consent storage/enforcement bug remain
unresolved gates.
