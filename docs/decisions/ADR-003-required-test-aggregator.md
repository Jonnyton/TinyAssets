...
| Cannot be neutered by its own PR | scope guard demands an exact-head review receipt for edits to `tests.yml`, `ci_required_tests.py`, `known-failing-tests.txt`, `drain_review_gate.py`. Deletion-only ledger edits are exempt, decided by **comparing parsed ledger content** between the trusted base checkout and the head sha — never diff metadata. Three fail-opens were caught in cross-family review before the flip: `.patch` is omitted on large/binary diffs; GitHub reports `additions: 0` for binary files (a NUL inside a comment makes the ledger "binary" while Python still parses a smuggled entry — verified on PR #2041); and pure renames also report `additions: 0` + `previous_filename`, so a pre-planted ledger renamed into place looked clean (verified on PR #2172). Content comparison is immune to all three, plus wipe-everything and rename-out, each with a boundary test |
