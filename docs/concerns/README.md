...
|---|---|---|
| **P2** | [OpenAI browser sign-in: no service-readiness confirmation, and dismissing the tab doesn't stop the flow](2026-09-04-openai-browser-signin-service-readiness-and-dismissal.md) — `LocalCallbackPlugin.start()` resolves before Android confirms foreground-service promotion, and no `browserFinished` listener stops the flow on early dismissal, so a stale "Finishing your sign-in…" notification can linger up to 10 minutes. Pre-existing, found during Codex review of the POST_NOTIFICATIONS fix | 2026-09-04 |
