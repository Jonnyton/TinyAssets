...
|---|---|---|
| **P2** | [Seven `test_first_contact` tests assert a message #2756 deliberately deleted](2026-09-02-first-contact-tests-assert-a-message-the-code-deliberately-removed.md) — proven with main's own file; blocks any PR touching `first_contact.py` because the required surface is diff-driven, so a stale test becomes a tripwire for whoever next edits the module | 2026-09-02 |
