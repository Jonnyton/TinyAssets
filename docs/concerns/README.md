...
| **P2** | [`write_graph` never documents `name` and `description`](2026-09-26-write-graph-name-and-description-are-undocumented.md) — the served build handle's `Args:` block covers 7 of its 9 parameters, on main, on every FastMCP version; an undocumented parameter is one the agent cannot use | 2026-09-26 |
