...
| **P1** | [Local MCP OAuth session lifecycle is only partly proven](2026-09-03-local-mcp-oauth-session-lifecycle-is-partly-proven.md) — explicit login reaches the founder universe in a fresh Codex process, but an already-running task stays anonymous and server-side logout/revocation is unproven | 2026-09-03 |
...
| **P1** | [One workspace slot for the whole host](2026-09-03-one-workspace-slot-for-the-whole-host.md) — `HOST_SLOT = "slot-0"` is a single global mutex, so ANY user's workspace job refuses every other user's with `workspace_busy`. Found in the founder's live conversation; violates the one platform-wide floor, which is not affecting other users | 2026-09-03 |
| **P2** | [Pinning `gh` to an exact apt version breaks every deploy, on every gh release](2026-09-02-the-gh-apt-pin-breaks-every-deploy-on-each-release.md) — the GitHub CLI apt repo carries exactly ONE version, so the pin is a fuse, not a pin; `build-image` fails, `deploy-prod` skips, nothing reaches production. Second occurrence (#2477 was the first) | 2026-09-02 |
...
| **P2** | [The `runnable` receipt encodes a gate the runtime removed](2026-09-01-the-runnable-receipt-encodes-a-gate-the-runtime-removed.md) — `_validate_source_code` no longer gates on approval; `api/branches.py` still reports `runnable: false` from approval provenance, so the universe plans around a false signal | 2026-09-01 |
