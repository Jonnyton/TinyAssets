...
| **P2** | [The source-channel policy store has no reader](2026-09-24-source-channel-policy-store-has-no-reader.md) — connector `set_policy` replies `policy_set` for a setting nothing enforces (`apply_auto_approval_policy` has no caller); C27 serves the consent row instead. Delete or rewire | 2026-09-24 |
