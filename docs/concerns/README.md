...
| **P2** | [The graph has no deterministic compute step](2026-08-30-the-graph-has-no-deterministic-compute-step.md) — four deploys of `$ta.*` operators to change one line of a fetched file; nothing can compute between a fetch and a write, and the `source_code` node that could is dead in the code | 2026-08-30 |
