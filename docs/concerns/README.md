...
| **P2** | [Two things the learned cursor needs before a reader](2026-09-26-learned-cursor-prerequisites-for-a-reader.md) — inert today: `latest_turn_no` fails SOFT to 0 so the contiguity guard can claim an owed turn, and nothing seeds `start_learned_cursor`, so a stage-2 drain would re-extract every existing conversation's history | 2026-09-26 |
...
| **P2** | [Installer tests are mostly dead on a Windows dev box](2026-09-25-installer-tests-dead-on-a-windows-dev-box.md) — the two halves of `test_host_uptime_installers.py` need different `bash`; 21 of them never start under Git Bash (no `flock`) yet report as failures, and DrvFs reports every mode as 777 | 2026-09-25 |
...
| **P2** | [Universe-file readers outside the turn path](2026-09-24-universe-file-readers-outside-the-turn-path.md) — harness S1 routes every turn-path read through `universe_files` and keeps the agent's write set small; ~20 other modules still read universe files raw, safe only while those paths stay agent read-only | 2026-09-24 |
...
| **P0** | [Provider subprocesses can read every user's universe](2026-09-24-provider-subprocesses-can-read-every-universe.md) — workflow-node model CLIs ran in `/app` with shell/file tools and could list `/data`; Claude path fixed in #3953, vendor-neutral jail for all providers in progress | 2026-09-24 |
| **P1** | [Model web tools can reach container-internal addresses](2026-09-24-webfetch-can-reach-container-internal-addresses.md) — WebFetch from a universe turn is not blocked from localhost/bridge/metadata (SSRF); not yet verified live | 2026-09-24 |
...
| **P2** | [GH_TOKEN still reaches the daemon's container config](2026-09-24-gh-token-backup-only-env.md) — the entrypoint strips it from the process, but it stays in the shared env file; the first backup-only move would have chowned `/etc/tinyassets` to root:root and locked the daemon out (withdrawn from #3966) | 2026-09-24 |
...
| **note** | [Cross-user deliverable connections](2026-09-09-cross-user-deliverable-connections.md) — structured delivery candidate awaits release proof; full artifact transfer remains open | 2026-09-09 |
...
| **P1** | [Scheduled uptime coverage remains incomplete](2026-09-08-scheduled-uptime-probes-remain-red.md) — deployed classifier honestly reports unavailable execution-quality and rendered observations; natural post-merge classification verified; cadence and independent coverage remain open | 2026-09-08 |
...
| **note** | [Hard-coded policy the user should be composing instead](2026-08-31-hard-coded-policy-that-should-be-user-composable.md) — inventory of graph-shape limits that constrain only a user's own workflow rather than protecting another user or the host | 2026-08-31 |
...
| **P3** | [Non-Source event subscriptions are stored but never fire](2026-09-02-non-source-event-subscriptions-never-fire.md) — `canon_change`, `branch_run_completed`, `canon_upload` and `pr_open` can be subscribed but have no emitter; the owner-identity half was resolved (Source events carry the hook owner; an empty principal is refused) | 2026-09-02 |
...
| **Watch** | [Tool-wait post-deployment observation](2026-09-21-provider-tool-wait-observation.md) —42.126s live single-tool call accepted; organic use and historical failure attribution remain unproven | 2026-09-21 |
| **P2** | [Optional tool-phase diagnostics on other exception paths](2026-09-21-tool-wait-diagnostic-followups.md) — typed idle/deadline evidence is covered; generic/early-exit catches omit optional phase/age | 2026-09-21 |
