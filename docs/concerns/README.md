...
| **P1** | [Provider admission lifecycle gaps](2026-08-28-provider-admission-lifecycle-gaps.md) — refusals consume launch bookkeeping, Codex cancellation releases before process death, async acquisition blocks the loop, and auth probes escape the bound | 2026-08-28 |
| **P2** | [Nested provider calls can starve](2026-08-28-nested-provider-calls-can-starve.md) — a served turn holds a provider slot while its own `run_graph` children queue behind it; bounded to a failed child by the 20s wait, but it starves under exactly the load the bound exists for | 2026-08-28 |
