...
| **P2** | [The checkout claim is a timestamp, not a lease on a session](2026-08-28-the-checkout-claim-is-not-tied-to-its-session.md) — three defects reduce to one gap: a lost response creates a second session, a delayed event for an old subscription releases a live claim, and abandonment locks the user out for ~37 min | 2026-08-28 |
...
| **P2** | [A cancelled subscription looks uncancelled](2026-08-28-a-cancelled-subscription-looks-uncancelled.md) — the confirmation is transient; on reload the chip still reads `Paid plan` and offers to cancel again, so a user cannot confirm their own cancellation took | 2026-08-28 |
| **P2** | [The edge Worker destroys every origin 5xx body](2026-08-28-worker-swallows-every-origin-5xx-body.md) — `worker.js` replaces the body of any origin 5xx with its own `bad_gateway` JSON, so a deliberate refusal reaches the user as "tunnel origin returned 5xx"; the origin can send no meaningful 5xx at all | 2026-08-28 |
