...
|---|---|---|
| **P2** | [There is no host-wide workspace bound at all](2026-09-03-one-workspace-slot-for-the-whole-host.md) — I filed the opposite and was refuted: the lock named `host` lives in a PER-UNIVERSE database, so it never crossed tenants. The real gap is that nothing bounds concurrent workspace jobs across universes, plus `wait_s` not governing SQLite's 30s busy timeout | 2026-09-03 |
