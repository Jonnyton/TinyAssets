...
| **P1** | [A universe cannot select its own serving provider](2026-09-03-a-universe-cannot-select-its-own-serving-provider.md) — binding lives on `POST /mcp/app/serving/bind` and no MCP tool selects which registered provider serves, so an agent can diagnose that it is unpowered and cannot fix it; cost the founder's universe a full day of prompt-node outage after the hard-coded-name wall was already fixed | 2026-09-03 |
