...
| **P2** | [Owner cannot pause or delete their own schedule from the app](2026-08-30-owner-cannot-pause-or-delete-own-schedule-from-app.md) — `schedule_branch` is `costly`, its undo actions are `admin`-scoped; the owner check `#2690` built sits behind a scope the owner's session lacks | 2026-08-30 |
...
| **P1** | [A file write through the GitHub contents API needs model-generated base64, so it corrupts the file](2026-08-29-file-writes-need-model-generated-base64.md) — live retest 2026-08-29: first `422 content is not valid Base64`, then a commit with every newline as a literal `
...
| **P2** | [The engine's run-rate cap (20/hour/universe) stalls a normal GitHub job](2026-08-29-run-rate-cap-stalls-a-normal-github-job.md) — one run per API call, so a one-line change with honest retries exhausted the hour and the clean attempt was refused at the write; reads count the same as writes | 2026-08-29 |
...
| **P2** | [The text channel still leaks raw ids](2026-08-30-text-channel-leaks-raw-ids.md) — goal-bind and patch-branch `text` carry raw ids; the four tests that catch it are quarantined | 2026-08-30 |
