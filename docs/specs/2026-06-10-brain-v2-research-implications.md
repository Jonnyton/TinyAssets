...
Concept: Brain v2 context engine w/ research mechanics. Next home: spec §5 (folded same day) → this repo PR → STATUS row for codex review gate when build begins. Write boundary (future build): new workflow/brain/ package + MCP tool surface; no existing-store migration in slice 1. First slice: entries schema + assemble(lens) read path over EXISTING wiki content (read-only projection — proves the lens before any migration). Blockers: codex review verdict; host ratification of spec (granted 2026-06-10). Verification: lens eval harness + view hit-rate baseline before/after.
