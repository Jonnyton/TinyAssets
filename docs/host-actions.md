...
| Closed test: 12 testers for 14 days, then apply for production access | **you** — Play reported 0 opted-in testers on 2026-09-08. The Alpha draft is configured for all 177 countries/regions and the one-member `Founder devices` list. Code 3 was removed; signed code `4 (1.0.3)` is saved as the sole draft bundle and its preview is open with no errors (one non-blocking deobfuscation warning). Preview Save/review submission and recruiting at least 11 more real testers remain before the 14-day clock can start. |
