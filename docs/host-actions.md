...
| Closed test: 12 testers for 14 days, then apply for production access | **you** — Play reported 0 opted-in testers on 2026-09-08. The Alpha release is configured for all 177 countries/regions and the one-member `Founder devices` list. Code 3 was removed; signed code `4 (1.0.3)` is the sole bundle. The verified reviewer credential and all 15 launch changes were submitted on 2026-09-08; Play's final quick checks completed and the Console says **Your changes are now in review**. Recruiting at least 11 more real testers remains before the 14-day clock can start. |
