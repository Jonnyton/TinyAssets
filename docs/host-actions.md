...
| Foreground-service declaration + behavior video | **video published; Save authorization remains** — the 27.11-second 1080×2340 privacy-redacted candidate has SHA-256 `7b49b48d21ca3a1f57acdce23ed8c5ac0f58b63aab57ea3d4cb5696ed61391f2`. It shows **Connect OpenAI** entering its pending state and `TinyAssets — Finishing your sign-in…` appearing immediately. Public URL: `https://github.com/Jonnyton/TinyAssets/releases/download/android-latest/tinyassets-fgs-play-evidence-final.mp4`. The Play form is filled with **Data sync → Network processing → Other** and this link, but not saved. |
...
| Closed test: 12 testers for 14 days, then apply for production access | **you** — Play reported 0 opted-in testers on 2026-09-08. The Alpha draft is configured for all 177 countries/regions and the one-member `Founder devices` list. Signed code `4 (1.0.3)` is uploaded to Play's artifact library and prepared in the edit form; removing code 3, saving/submitting the draft, and recruiting at least 11 more real testers remain before the 14-day clock can start. |
