...
**user access token** (8h). Store the access/refresh bundle under the
destination-scoped `user_review` binding; `VaultBackedTokenProvider` rotates it
through the vault refresh seam before expiry. Obtain it through the App's
user-authorization (OAuth) flow when the owner connects their account. Minimum
scope: `Pull requests: write` on the target repo.
...
Store it under the destination-scoped `ruleset_verify` binding. The live
verifier client leases or refreshes it through `VaultBackedTokenProvider` and
uses it ONLY for the gate read, never for merging. This is the disclosed extra
grant that autonomous merge costs; the App's merge identity stays minimal.

## Credential placement summary

| Credential | Secret? | Destination |
|---|---|---|
| App ID | no | broker connection metadata (JWT `iss`) |
| App private key (`.pem`) | **YES (crown jewel)** | vault `SecretStore` — never a repo/universe artifact, never logged |
| Installation ID | no | broker connection metadata (mints 1h installation token) |
| App integration/actor id | no | broker connection metadata for the `app_actor_id` bypass check |
