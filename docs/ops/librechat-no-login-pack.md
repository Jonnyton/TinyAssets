# LibreChat No-Hosted-Chatbot-Login Pack — Retired
...
Do not configure LibreChat without OAuth, do not reuse the former
`read_graph`/`read_page` allowlist, and do not describe this route as a
supported no-login pack. A current LibreChat integration requires a
host-supported OAuth 2.1 flow with the resource indicator
`https://tinyassets.io/mcp`. Until that flow has fresh rendered proof,
LibreChat is unsupported for TinyAssets rather than anonymous-read-only.
