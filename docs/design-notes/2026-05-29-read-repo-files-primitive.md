...
**Status:** DESIGN — writer:claude; reviewed by Cowork + Codex with amendments folded before build.
**Date:** 2026-05-29
...
contents API using a separate read-capability resolver/map** keyed by
destination, and writes `{path: contents}` JSON to the output key. No approval
wall, runs mid-graph, and returns file data directly into state. Trade-off:
keyed on `node_id` (the existing opaque-node convention) — effectively a
reserved node id.
...
**Option A.** It uses the existing domain-callable path, preserves the
source-code approval wall for user-supplied code, returns data mid-graph, and
composes directly into the existing loop:
...
2. **Path source + server-enforced limits.** `target_paths` carries the requested
   paths as a JSON array or comma list. The platform callable, not node config,
   must enforce normalized relative paths only, reject absolute paths and parent
   traversal, cap max files, cap bytes per file, cap total bytes per call, and
   return a structured `read_truncated` signal that `propose_changes` can see.
...
4. **Error kinds and per-file status.** Distinct kinds per failure
   (`read_ref_lookup_failed`, `read_contents_denied` for 401/403/404 = scope
   signal, `read_file_too_large`, `read_path_rejected`, `read_truncated`), never
   collapsed — same discipline as the BUG-111 write path. Return per-file status
   metadata so downstream nodes can distinguish present, missing, denied,
   rejected, and truncated content.
