...
- **Anti-rollback (implemented; the honest guarantee is "rollback forces reauthorization", NOT "bytes unrecoverable").** Claims, tombstones, and ciphertext share one SQLite recovery domain, so restoring an OLDER full snapshot would silently reopen consumed refresh claims and deletion tombstones. A monotonic epoch is bumped on every mutation and MIRRORED to a file kept OUTSIDE the restored snapshot (a sibling of the DB / outside the local store dir). After a full-DB or full-directory restore the DB epoch is behind the mirror, so every `get`/`begin_refresh` raises `REAUTHORIZATION_REQUIRED`. For rotating OAuth kinds the restored refresh token is already dead provider-side, so this is exactly correct. (A backup that also captures the external mirror on a separate volume is the operator's DR concern; the CORE detects the file/DB-snapshot restore the review class targets.)
...
4. S4/E4: GitHub adapter consumes `github_app_private_key`, `github_app_user_token`, or `github_pat`; mint installation tokens in memory; serialize 8h/6mo refresh; effectors receive leases, never values or whole vault files.
   - **S3 executor boundary (implemented primitive — wire to it, do NOT invent a parallel one):** an isolated worker resolves a tenant credential via an OPAQUE, job-scoped grant, never a raw forgeable `universe_id` or `ref`. The daemon calls `broker.mint_job_grant(binding, expected, run_id, ttl=...)` (derived from the authoritative run record; authenticates the credential first) → an opaque `JobGrant` (opaque id + a private non-observable bearer capability, no repr/asdict/pickle leak). The worker calls `broker.resolve_job_grant(grant, run_id, universe_id)` with its OWN authoritative `run_id`/`universe_id`; it fails CLOSED (typed `CredentialUnavailable`) for a missing/malformed grant (`NOT_FOUND`), a bad capability (`LEASE_LOST`), a wrong run or cross-universe (`CROSS_STORE_FORBIDDEN`), or an expired grant (`EXPIRED`).
