...
**Status:** IMPLEMENTED (CORE, `feat/credential-vault`, draft PR #1469). Claude-family review = APPROVE with adaptations (`docs/audits/2026-07-16-...-claude-review.md`); hardened through eleven Codex adversarial security-review rounds (`...-codex-review{,-2..11}.md`). Not yet wired into S2/S4/S5 callers (separate seams).
...
- **Anti-rollback (implemented; the honest guarantee is "rollback forces reauthorization", NOT "bytes unrecoverable").** Claims, tombstones, and ciphertext share one SQLite recovery domain, so restoring an OLDER full snapshot (production backs up/restores the WHOLE `/data` volume as a unit) would silently reopen consumed refresh claims and deletion tombstones. Each mutation holds the vault's `BEGIN IMMEDIATE` lock, compares a stable guard-first snapshot, durably **reserves the next epoch in the independent guard BEFORE the vault commit**, then commits that exact epoch with the credential change. A failed vault commit may leave the guard ahead and force reauthorization; a successful mutation can never outrun the guard. The guard lives in an **INDEPENDENT recovery domain OUTSIDE `/data`** — a tiny SQLite DB at `TINYASSETS_VAULT_ROLLBACK_GUARD` (recommended: a separate volume) or a home-dir default, deliberately NOT under `data_dir()` so a `/data` volume restore does not carry it. Its row key binds custody + physical recovery generation and, for local custody, store + daemon identity; independent daemons cannot alias. Missing/recreated/mismatched guard identity with a nonzero vault epoch raises `REAUTHORIZATION_REQUIRED`; corrupt/unavailable guard storage raises `BACKEND_UNAVAILABLE`. **EVERY authoritative mutation (`put`/`delete`/`complete_refresh`/`begin_refresh` and job-grant mint) checks rollback and reserves before commit.** After a full-volume / full-DB / full-directory restore the DB epoch differs from the guard, so every operation fails closed; `EpochGuard.bump_for_restore()` can force the signal explicitly. For rotating OAuth kinds the restored refresh token is already dead provider-side, so this is exactly correct.
...
4. S4/E4: GitHub adapter consumes `github_app_private_key`, `github_app_user_token`, or `github_pat`; mint installation tokens in memory; serialize 8h/6mo refresh; effectors receive leases, never values or whole vault files.
   - **S3 executor boundary (implemented primitive — wire to it, do NOT invent a parallel one):** an isolated worker resolves a tenant credential via an OPAQUE, job-scoped grant, never a raw forgeable `universe_id` or `ref`. The daemon calls `broker.mint_job_grant(binding, expected, run_id, ttl=...)`; `run_id` is only a lookup key, and the broker's injected authoritative run lookup must return the same run/founder/universe before mint succeeds. The credential is authenticated first and `ttl` is finite/positive/bounded. The returned `JobGrant` exposes no `run_id`/`universe_id`. **The grant is BROKER-AUTHORITATIVE:** the worker calls `broker.resolve_job_grant(grant, verify_context=<callback>)`; the callable signature retains `verify_context=None` for consumer compatibility, but omission fails closed. The broker normalizes every persisted grant field, resolves against the row's own authoritative context, then requires the live executor verifier to return `True`. A malformed capability/type/row, unknown or expired grant, forged capability, missing verifier, or rejected executor context is always a typed `CredentialUnavailable`, never a raw `AttributeError`/`ValueError`.
