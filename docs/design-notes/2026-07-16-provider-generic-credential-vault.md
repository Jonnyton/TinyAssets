...
- Store ciphertext rows in `/data/private/credential-vault/v1/vault.db` (SQLite WAL now; the broker contract permits a future Postgres backend). `/data` backups contain ciphertext, wrapped DEKs, and non-secret metadata only.
