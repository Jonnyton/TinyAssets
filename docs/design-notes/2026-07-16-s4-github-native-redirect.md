...
2. **Server-side founder handle is a real vault lookup.** `permissions.current_github_handle`
   resolves the connected owner login from the broker's fixed non-secret GitHub
   connection metadata, fail-closed to `""` when unconnected/ambiguous. No
   caller-supplied identity is trusted.
...
- **#3 Autonomous is reachable in prod.** `run_review_recovery_for_universe` now
  ...
  plus the App actor id + owner from broker connection metadata, and INVOKES
  `fire_due_not_before_timers`
...
metadata columns. The App private key and user/PAT values remain in vault CORE.
`github_client_from_vault` and `verifier_client_from_vault` now use
`VaultBackedTokenProvider`: it mints scoped installation tokens in memory,
atomically rotates user-token bundles through the refresh seam, and dynamically
leases PAT fallback. Production no longer snapshots vault values into
`StaticTokenProvider`.
