...
when the `component` field already names a file (e.g. `tinyassets/api/wiki.py`) or
...
- Code search **requires authentication**; our default posture is unauthenticated
  reads (subscription-only, no API keys — see `TINYASSETS_ALLOW_API_KEY_PROVIDERS`
...
- `search_query` — whitespace/comma/semicolon-separated terms. A term with glob
  ...
  slash form (`tinyassets/api/wiki`).
...
2. **Server-enforced caps.** Result count (`TINYASSETS_GITHUB_SEARCH_MAX_RESULTS`,
