...
# Universe Server → TinyAssets Server: Layer-3 Rename Scoping
...
- Layer 1 (already landed) — desktop shortcut + `start-tinyassets-server.bat` + tray rebrand strings.
...
- **Layer 3 (this note)** — the *machine* surfaces: the Python module path (`tinyassets/universe_server.py`), the env-var prefix (`UNIVERSE_SERVER_*`), and the plugin directory (`packaging/claude-plugin/plugins/tinyassets-universe-server/`).
...
**Cost.** Smaller than the Author→Daemon rename (one module, one env-var family, one plugin dir vs ~2366 file footprint). Moderate test-suite churn (~48 test files import `workflow.universe_server`). Compat shim pattern is already proven by `tinyassets/_rename_compat.py`.
...
### 2a. Env-var prefix: `UNIVERSE_SERVER_*` → `TINYASSETS_SERVER_*`

**Surface.** Eight distinct variables in active use:

| Variable | Purpose | Read sites |
|---|---|---|
| `UNIVERSE_SERVER_BASE` | Root output directory | `tinyassets/universe_server.py:112,525`, `tinyassets/auth/provider.py:192`, `tinyassets/node_eval.py:160`, `fantasy_daemon/__main__.py:2137`, `packaging/.../runtime/server.py:16-33`, `packaging/.../plugin.json:38` |
| `UNIVERSE_SERVER_DEFAULT_UNIVERSE` | Default universe ID | `tinyassets/universe_server.py:127`, `plugin.json:39` |
| `UNIVERSE_SERVER_USER` | Actor identity slug | `tinyassets/identity.py:44`, `tinyassets/universe_server.py:258,1696,2319,2555,2680,3956`, `tinyassets/git_bridge.py:242` |
| `UNIVERSE_SERVER_HOST_USER` | Host identity for tier-routing | `tinyassets/auth/provider.py:287`, `tinyassets/universe_server.py:1697,2320,2556,9759` |
| `UNIVERSE_SERVER_AUTH` | Auth mode selector | `tinyassets/auth/provider.py:467`, `tinyassets/auth/__init__.py:15` |
| `UNIVERSE_SERVER_URL` | Wellknown base URL override | `tinyassets/auth/wellknown.py:31` |
| `UNIVERSE_SERVER_PORT` | HTTP port | `tinyassets/auth/wellknown.py:32` |
| `UNIVERSE_SERVER_BASE` (also referenced by tests under tmp_path) | — | many tests |

**Compat shim pattern (recommended).** A small helper that reads new prefix first, falls back to old prefix, emits a `DeprecationWarning` (once per variable per process) when only the old is set. Centralized in `tinyassets/config.py` or a tiny new module `tinyassets/env_compat.py`:

```python
def get_workflow_env(name: str, default: str = "") -> str:
    """Read TINYASSETS_SERVER_<name>; fall back to UNIVERSE_SERVER_<name>."""
    new_key = f"TINYASSETS_SERVER_{name}"
...
### 2b. Module rename: `tinyassets/universe_server.py` → `tinyassets/workflow_server.py`

**Surface.** ~48 test files import `workflow.universe_server`. Plus the alias shim at `fantasy_daemon/universe_server.py`. Plus `tests/test_packaging_build.py` which inspects the staged bundle for `tinyassets/universe_server.py` paths.

**Avoiding a name collision.** "workflow.workflow_server" is a slight but acceptable nesting. Alternatives considered:
- `tinyassets/server.py` — clean, but collides semantically with the existing `packaging/.../runtime/server.py` (the bootstrap stub). Recommend not.
- `tinyassets/mcp_server.py` — there is **already** a `tinyassets/mcp_server.py` in the runtime mirror tree. Confirm in canonical tree before choosing this path; if it exists, this name is taken.
- `tinyassets/workflow_server.py` — slightly redundant but unambiguous and matches the user-facing brand "TinyAssets Server" exactly. **Recommended.**

The host should pick from these three; navigator's recommendation is `tinyassets/workflow_server.py` for matching the user-facing brand 1:1.

**Compat shim pattern.** Use the existing `tinyassets/_rename_compat.py` infrastructure. It already supports deep-submodule alias preservation (built for Author→Daemon). Add one line to register `workflow.universe_server → workflow.workflow_server`. Test suite can import either path during the transition cycle.

**Migration path:**
1. Copy/rename file to new path.
2. Add compat alias in `_rename_compat.py` initialization (gated by the same `TINYASSETS_AUTHOR_RENAME_COMPAT`-style flag, OR a parallel `TINYASSETS_SERVER_RENAME_COMPAT` flag if we want independent flip cadence).
3. Update *internal* call sites that import (currently 0 in `tinyassets/`, all live in `tests/` and `fantasy_daemon/`).
...
### 2c. Plugin directory: `packaging/claude-plugin/plugins/tinyassets-universe-server/` → `packaging/claude-plugin/plugins/workflow-server/`

**Surface.**
- The directory itself.
- `packaging/claude-plugin/build_plugin.py` `PLUGIN_ROOT` constant (per the audit note flag).
- `plugin.json` `"name"` field (`"tinyassets-universe-server"` → `"workflow-server"`).
- `plugin.json` `mcpServers` key name (`"tinyassets-universe-server"` → `"workflow-server"`).
- Any docs that reference the path (`docs/mcpb_packaging.md`, `IMPORT_COMPATIBILITY.md`).
- `pyproject.toml` entry point `tinyassets-universe-server` (per `IMPORT_COMPATIBILITY.md:126`).
...
**Entry point.** `pyproject.toml`'s `tinyassets-universe-server` console script gets renamed to `workflow-server`. Old entry point can stay as a deprecated alias for one cycle (similar pattern to env-vars) by registering both in `pyproject.toml`.
...
- **Packaging mirror byte-equality.** The audit at `docs/audits/2026-04-18-rename-tree-consistency-audit.md` documents that the canonical `tinyassets/` tree and the plugin-runtime mirror must be byte-identical. Module rename in canonical tree must immediately propagate via `scripts/sync-skills.ps1` (or its build-plugin equivalent) to the mirror tree. Verifier should run the rename-tree-consistency check after.
- **`packaging/dist/tinyassets-universe-server-src/`.** Stale — last built before rename. Will be regenerated under the new name on next packaging pass; no manual action.
...
|------|----------------------------|---------|--------|-------|
| **#26 Layer-3a env-var compat** | `tinyassets/config.py` (or new `tinyassets/env_compat.py`); migrate all `os.environ.get("UNIVERSE_SERVER_*", ...)` call sites in `tinyassets/**/*.py` to the helper | #24 | pending | Compat helper + mechanical call-site sweep. Tests still pass with old env names; deprecation warning surfaces. |
| **#27 Layer-3a env-var manifest cutover** | `packaging/claude-plugin/plugins/tinyassets-universe-server/.claude-plugin/plugin.json` env block; affected docs | #26 | pending | Switch the *configuration surface* to the new prefix. Old prefix still works for one cycle via #26's helper. |
| **#28 Layer-3b module rename + alias** | `tinyassets/universe_server.py` → `tinyassets/workflow_server.py`; `tinyassets/_rename_compat.py` (register alias); `fantasy_daemon/universe_server.py` shim retarget; `tests/test_packaging_build.py` path constants | #3 (Author→Daemon Phase 1) | pending | Add alias entry to `_rename_compat.py`. Both import paths resolve to the same module object. |
...
| **#30 Layer-3c plugin dir rename** | `packaging/claude-plugin/plugins/tinyassets-universe-server/` → `.../workflow-server/`; `packaging/claude-plugin/build_plugin.py` `PLUGIN_ROOT`; `plugin.json` `name` + `mcpServers` key; `pyproject.toml` entry point; `docs/mcpb_packaging.md`, `IMPORT_COMPATIBILITY.md` | #27, #28 | pending | Hard cutover. Ships with v0.2.0 release notes calling out reinstall. |
| **#31 Compat retirement** | Delete `TINYASSETS_SERVER_RENAME_COMPAT` shim entries; remove env-var fallback; remove module alias entry | #30 + one release cycle | pending | Clock starts when #30 lands. |
