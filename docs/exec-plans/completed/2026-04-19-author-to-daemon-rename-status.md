...
**(i) Layer-3 rename design note (`docs/design-notes/2026-04-19-universe-to-tinyassets-server-rename.md`, awaiting host §5 answers) sequences AFTER Phase 1 Part 2 lands.** Specifically, layer-3 task #28 (module rename `tinyassets/universe_server.py` → `tinyassets/workflow_server.py`) registers a new alias in `tinyassets/_rename_compat.py` — and #6 is touching that file right now. Dispatch order: #6 → host §5 answers → layer-3 tasks. No collision risk if sequenced correctly, but a *parallel* claim of layer-3 #28 against #6 would race on `_rename_compat.py`.
