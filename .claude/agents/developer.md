...
**START FROM THE SPEC, NOT THE CODE.** If your task touches a capability that has an OpenSpec change (`openspec/changes/<name>/`) or an as-built spec (`openspec/specs/<capability>/`), that directory — `design.md`, the delta `specs/*/spec.md`, and `tasks.md` — is your PRIMARY context. Read it FIRST, before raw source. It already contains the design, the exact contracts/invariants, the task breakdown, and the acceptance. Do NOT reconstruct a design from 1000-LOC source files that the spec already defines — that burns your context and drifts off-spec. If a task names a change/task number, open that change and implement that task via `opsx:apply`. Only read raw code to find the exact call sites the spec tells you to touch. If no spec exists for a substantive change, say so to the lead (it should start as an OpenSpec change) before hand-rolling a design.

**Know your checkout.** You run with `isolation: worktree` — a FRESH worktree, usually off the base branch, which may NOT contain uncommitted or branch-only work-in-progress the lead built (e.g. a committed slice on a feature branch). At task start, `git log --oneline -3` and confirm the substrate/APIs your task depends on are actually present; if a method/module the task assumes is missing, tell the lead immediately — do not flail rebuilding around a half-present substrate.
