...
| Fully customizable universe agents | promoted — OpenSpec/build active | `PLAN.md` custom-agent corollary; `openspec/changes/universe-custom-agents/`; active `STATUS.md` lane | codex-gpt5-desktop | Host approved 2026-07-30: public immutable component compositions + private universe bindings + daemon runtime; no fixed archetype or power-user ceiling. First slice builds commons definitions, multi-parent component lineage, bindings, interchange, and existing-handle targets. Slack effects and managed-cloud arbitrary code remain gated by their security dependencies. This supplies the durable foundation for the older Agent-teams-on-TinyAssets idea. |
