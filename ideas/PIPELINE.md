...
|------|-------|-----------|-------|-------|
| WorkOS Pipes user-owned GitHub destination authority | triaged — research handoff awaiting independent review | `docs/audits/2026-08-03-workos-pipes-cloud-authority-implications.md`; then existing `openspec/changes/activate-main-universe-spec-drain/` | Codex initial research → Claude review (same-provider fallback only under approved hard-limit rule) | WorkOS Pipes can start per-user GitHub OAuth and vend refreshed tokens to the backend. Adapt it into the existing credential-blind broker and exact destination-grant ledger; never treat AuthKit login or the platform GitHub capability as repository authority. Applies whenever cloud automation setup or outbound GitHub grants are touched. Provider-compute binding remains a separate blocker. |
| Main-account cloud OpenSpec drain | promoted — Ringer-informed implementation active | `docs/design-notes/2026-07-29-main-account-cloud-spec-drain.md`; `openspec/changes/activate-main-universe-spec-drain/`; `docs/audits/2026-07-30-ringer-production-orchestration-implications.md`; `docs/audits/2026-07-30-ringer-drain-hotpath-reevaluation.md` | Codex research → independent review → OpenSpec/runtime lane | Host approved 2026-07-29: one cloud-owned loop, tray stopped at cutover, phone-chatbot management/repair/evolution with the computer fully off, BYOC before market compute. The 2026-07-30 re-evaluation preserves the generic user-owned GitHub→spec target and promotes only a measured local-bridge hot-path repair before cloud work resumes. |
