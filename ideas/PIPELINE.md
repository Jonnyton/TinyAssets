...
| Democratized compute stack — harness→lithography creation ladder | triaged | `ideas/2026-07-15-democratized-compute-stack.md`; primitive layer already spec'd at `openspec/specs/hardware-creation` | unassigned | Captured 2026-07-15 as a standalone idea document; landed on `origin/main` 2026-07-22 via the #1490 recovery sweep, so it never passed through this table until now. **Review gate UNMET:** the document requires a Claude-family reviewer to re-check external sources and compare against current TinyAssets architecture before implementation. Host-approved *direction* + hard `$0` MVP only — not build authority. Re-scope before any build: the primitives it assumes now exist (`tinyassets/paid_market/{shuttle,fabrication}.py`), so the open work is the product-proof layer and its 6 open questions. |
| User-built model foundry — arbitrary architectures trained from random init | triaged | `ideas/2026-07-15-user-built-model-foundry.md`; funding half spec'd at `openspec/specs/pooled-training-ownership` + `paid-market-training` | unassigned | Child of the democratized-compute row. Same #1490 provenance. **Review gate UNMET:** requires an independent Claude-family re-check of pricing, sources, safety boundaries, and TinyAssets composition before build. Document self-declares "implementation authority: none until a specification is approved" and owes a `PLAN.md` foldback. 4 of 5 open decisions unresolved. Carries the load-bearing honesty rule that an unexecuted target run is **scale-ready / resource-blocked**, never scale-proven. |
| Conversational cookbook device — two-screen voice-first kitchen appliance | triaged | `ideas/2026-07-15-conversational-cookbook-device.md`; supersedes `docs/specs/2026-07-15-riscv-fpga-vertical-proof.md` as first-device candidate | unassigned | Child of the democratized-compute row. Same #1490 provenance. **Review gate UNMET:** requires a Claude-family re-check of the local-model sources and TinyAssets composition before implementation. Host-approved `$0` constraint; zero-purchase MVP-0 is a split-screen browser simulator making no physical-device claim. 3 of 4 open questions unresolved. The FPGA spec it displaced self-marks "paused" — retire or supersede that spec explicitly if this one is approved. |
