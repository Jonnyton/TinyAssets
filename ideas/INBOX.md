...
- [2026-07-24] (source: full-platform completion audit + Codex navigator, owner: unassigned, status: captured, priority: uptime-scale, size: large) **One honest §14 production-load harness with PASS/FAIL/NOT-RUN.** Replace the stale pre-draft with versioned `tests/load/` generators, invariant oracles, immutable run manifests, raw metrics/traces, fault timelines, reconciliation reports, and machine-readable verdicts. Mocks may test generators but never count as scale proof. Target scenarios cover 2,000 MCP sessions, real PostgreSQL RLS/pooling/CAS/outbox load, a Realtime project explicitly configured for the accepted thousands-connection envelope, 500-daemon bid/claim/delivery storms, hot-universe collaboration/CAS, replay-safe market settlements, and mixed gateway/database/Realtime failures. Build source-only generators early; mark PostgreSQL, Realtime, and distributed-execution scenarios NOT-RUN until their real substrates land.
