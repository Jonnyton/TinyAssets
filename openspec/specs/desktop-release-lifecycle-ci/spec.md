...
The desktop release workflow SHALL execute the exact unsigned Windows installer lifecycle beneath a non-PowerShell parent supervisor whose total deadline is independent of all waits and cleanup performed by the lifecycle child. A native watchdog SHALL also bound the supervisor's complete lifetime, including preflight, process-tree ownership, child wait, cleanup, capture replay, and interpreter teardown. On expiry it SHALL write all thread stacks directly to workflow stderr and exit non-zero before the GitHub job timeout begins cancellation. The child-wait deadline, whole-supervisor deadline, and GitHub job timeout MUST be ordered with explicit diagnostic margin, and the job timeout MUST remain defense in depth rather than the primary lifecycle bound.
