# deferred-turn-learning Specification

## Purpose
Where a conversation turn's LESSON gets recorded, and what the founder waits
for while it happens. Measured 2026-09-25: `converse` spent a third model
round-trip on learning extraction after the reply text already existed, on
every turn, on the founder's clock. The turn now records its own lesson
in-turn and the extra call runs only when it did not. Created by archiving
change deferred-learning-never-blocks-the-reply.
