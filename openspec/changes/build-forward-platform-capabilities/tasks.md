...
- [ ] 4.1 **Successor-outcome tracker — owner: `demand-side-signals` (assigned 2026-07-25).** Durable standing goals, timezone-aware inbox scheduling, archetype onboarding outcomes, and per-universe operational metrics. Blocked by D1's successor rule — **not** by the money transport: D3 names price, forward, training, data, pool, and hardware as transaction consumers, and standing-goal/onboarding work is non-monetary. Its real prerequisites are `boundary-layer` for inbox ingress, `daemon-runtime-and-dispatch` for the heartbeat, and `shared-goals-and-convergence` for the Goal primitive. This was the cheapest slice to promote next, and D1 is now satisfied: `openspec/changes/demand-side-signals/` is the narrower successor and the sole active owner of this half.
  ...
  - The successor additionally carries a MODIFIED delta against `daemon-runtime-and-dispatch`, because a declared missed-tick policy contradicts that capability's as-built limitation that cron ticks missed during downtime are silently dropped — the failure mode a standing goal is defined by. Per the 1.2 pattern, the contradiction is carried as a delta rather than a promise, and `demand-side` may not sync without it.
