...
- [ ] 3.4 Add failing tests for schedule, subscription, run, resume, daemon-cycle, and child-work issuance, including a conditional single-winner resume-attempt record, crash-idempotent receipt linkage, stale/revoked bindings, and no-broader child/retry/fallback constraints.
...
- [ ] 7.1 Add focused dark-mode tests for every daemon and graph requirement, keep V2 behavior observational while dark, and add server-owned per-universe and default-empty exact operation-plus-runtime maintenance-canary configuration with isolated credential binding/budget that caller data cannot select or widen.
