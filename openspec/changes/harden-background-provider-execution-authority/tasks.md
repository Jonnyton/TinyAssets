...
- [ ] 2.6 Make fresh issuance query the exact physical work-item key and fail on any fenced/non-conclusive prior receipt; implement bounded restart reconciliation from durable launch, outbound-proxy, provider-side idempotency/status, child-process journal, and durable result evidence, using transient attempt receipts only in-process and creating no durable attempt-receipt sink.
...
- [ ] 3.4 Add failing tests for schedule, subscription, run, resume, daemon-cycle, and child-work issuance, including a conditional single-winner resume-attempt record, crash-idempotent receipt linkage, stale/revoked bindings, atomic parent-to-child authority transfer, and exactly-once return of proven unused child authority only to the same active parent generation.
...
- [ ] 5.1 Convert branch-task workers, schedules, subscriptions, run/resume, selectors, cloud workers, and autonomous daemon cycles to claim exact background receipts independently of queue claims and leases; apply identical authority guards at startup and every dispatcher pick sweep, add monotonic BranchTask `lease_generation`, invalidate unprovable old claim generations before cancellation/sweep, compare-and-swap exact queue proof, replace the process-global lazy recovery boolean with synchronized per-universe state, and store fenced-run sentinel `provider_authority_fenced` in the existing flat error text.
