...
- [ ] 5.1 Convert branch-task workers, schedules, subscriptions, run/resume, selectors, cloud workers, and autonomous daemon cycles to claim exact background receipts independently of queue claims and leases; make branch-task reset invalidate the old claim generation then compare-and-swap the exact claim/lease/authority proof, and replace the process-global lazy recovery boolean with synchronized per-universe state that is retryable, isolates V2 failures from dark universes, and publicly marks fenced runs `interrupted` with reason `provider_authority_fenced`.
...
- [ ] 6.2 Under V2, retain exact shipped per-provider outcomes—codex missing file is dead but existing empty/corrupt/stale/cache-miss stays eligible; claude-code absent/empty/unreadable config is dead; probe kill switch and router unknown/inconclusive stay eligible—while preventing ordinary routing from launching the probe completion; implement the universe-less maintenance path through `ProviderExecutor` without ambient credential dereference or a second sink.
...
- [ ] 7.1 Add focused dark-mode tests for every daemon and graph requirement, keep V2 behavior observational while dark, require current maintenance/quarantine readiness before a listed universe becomes effective for a worker/provider, and add server-owned exact operation-plus-runtime maintenance-canary configuration with isolated credential binding/budget that caller data cannot select or widen.
