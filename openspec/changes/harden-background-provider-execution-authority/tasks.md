...
- [ ] 5.1 Convert branch-task workers, schedules, subscriptions, run/resume, selectors, cloud workers, and autonomous daemon cycles to claim exact background receipts independently of queue claims and leases; keep predecessor/leaseless reclaim startup-only while applying the V2 authority guard to startup, expired-lease hot sweeps, and graceful-drain orphan release; add and increment monotonic BranchTask `lease_generation` in every owning mutation, invalidate unprovable old claim generations before cancellation/sweep, compare-and-swap exact queue proof, replace the process-global lazy recovery boolean with synchronized per-universe state, and treat flat `provider_authority_fenced` text as a ledger projection only.
