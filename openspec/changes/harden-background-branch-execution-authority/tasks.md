...
- [ ] 1.1 Inventory every schedule, subscription, soul/`PROGRAM.md` loop, `BranchTask`, graph-enqueue, daemon, cloud-worker, and distributed-worker call site that can execute a branch after its authorizing request ends; add a CI closure assertion for the inventory.
- [ ] 1.2 Record the canonical identity, ACL, branch, daemon, run, queue, B2, provider-work, and provider-attempt read interfaces consumed by target authority without duplicating their truth.
...
- [ ] 2.6 Implement typed `authority_hold` transitions and non-secret projections for missing, stale, revoked, exhausted, unauthorized, source-mismatched, and indeterminate authority.
...
## 3. Authenticated Schedule and Subscription Lifecycle
...
- [ ] 4.2 Implement the soul binding prepare/edit-snapshot/commit-old-revoke protocol under the existing soul compare-and-swap and per-universe lock.
...
- [ ] 5.1 Create a non-serializable root/parent child-delegation context from current request or parent target authority and inject it into trusted graph execution context.
- [ ] 5.2 Implement atomic child-binding derivation that resolves the exact same-universe target and transfers—not copies—the parent's remaining depth/count/cost envelope.
...
- [ ] 5.4 Extend the epoch-1 `BranchTask` shape with only opaque binding reference/digest and required generations while preserving strict JSON validation and compatibility-free parsing.
...
- [ ] 5.6 Preserve stable root origin, run-wide budget, physical universe, depth, global-active, lifetime-lineage, archive-integrity, and corrupt-history fail-closed behavior under the new authority path.
...
- [ ] 6.3 Replace environment-derived run actors with canonical authorizer provenance and record daemon/runtime/worker separately as executors.
...
- [ ] 7.1 Build a read-only inventory/report command for legacy triggers, soul/`PROGRAM.md` loops, live/archive tasks, graph producers, and dispatch call sites with canonical-evidence classifications.
...
- [ ] 8.9 Sync the five capability deltas into canonical specs, archive the completed change, retire its `STATUS.md` row, and record the landing in git history only after every gate passes.
