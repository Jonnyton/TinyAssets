...
- [x] 1.3 Define typed `BackgroundBranchBinding`, `BackgroundBranchAttempt`, source-kind, target-mode, lifecycle, hold-reason, and provenance contracts with strict validation and lossless round trips. Completed 2026-07-29 in `tinyassets/background_branch_authority.py`: dark non-bearer dataclasses reject unknown/missing fields and invalid state combinations; 29 focused tests prove typed lossless round trips, closed child-policy/receipt/operation/executor schemas, immutable constructor inputs, target-mode, hold, terminal, audience/provenance consistency, root/child lineage, and secret-shaped reference rejection.
