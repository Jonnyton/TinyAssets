...
- Make protected-stdin secret installation build content entirely in the current Bash process, without a child or named plaintext temporary file.
...
- Production compose, deploy-secret custody, and operator guidance: `deploy/compose.yml`, `deploy/install-tinyassets-env.sh`, `.github/workflows/deploy-prod.yml`, `deploy/DEPLOY.md`, `deploy/request-idempotency-env.template`, `deploy/hetzner-bootstrap.sh`.
