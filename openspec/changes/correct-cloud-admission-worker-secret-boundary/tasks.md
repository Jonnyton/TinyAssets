...
- [x] 1.3 Add installer regressions proving Compose-valid assignment variants cannot bypass immutable, delete, absence, or no-plaintext-intermediate guarantees.
...
- [x] 2.2 Replace protected-stdin temporary-file/child custody with Compose-aware in-process Bash construction.
...
- 2026-08-02, Windows 11: `python -m pytest -q tests/test_deploy_prod_workflow.py tests/test_install_tinyassets_env.py tests/test_log_aggregation.py tests/test_validate_host_runtime_hmac_pair.py` -> 147 passed, 12 POSIX-only skipped.
...
- 2026-08-02, Ubuntu WSL: five Compose-valid malformed duplicate `set-once` shapes were refused before mutation with exit 5 and no value disclosure; delete/assert-absent removed or rejected the same grammar.
