...
- Each handle is a thin shape/payload router over the EXISTING `tinyassets/api/*` handlers — no change to primitive behavior, run semantics, or storage.
...
- **Forward-port #617.** Recover the directory_server implementation from git (`git show <merge-of-#617>:tinyassets/directory_server.py`) and adapt its router into `universe_server.py`, preserving its test coverage. Avoids re-deriving the fold-map.
...
- **Runtime mirror parity.** `packaging/claude-plugin` must stay byte-parity with `tinyassets/api` per existing rule; update both.
