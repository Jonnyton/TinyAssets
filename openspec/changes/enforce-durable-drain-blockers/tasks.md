...
- [ ] 3.1 Run the focused supervisor/watchdog suites, Ruff, strict OpenSpec validation, and a controlled run-directory simulation; obtain opposite-provider exact-head review or the approved same-provider fallback under the AGENTS.md hard-limit rule, then resolve every blocking finding.
  ...
  - **Fallback activated 2026-07-29:** Claude CLI and Claude.ai (Opus and Sonnet) report the account-wide monthly spend limit, so the host approved fresh-context independent Codex review. Four exact-head reviews found receipt, recovery, collision, migration, punctuation-identity, and proposal-scope blockers; the next candidate has regression fixes with 166 tests, Ruff, strict validation, and OpenSpec flow green, pending exact-head re-review.
