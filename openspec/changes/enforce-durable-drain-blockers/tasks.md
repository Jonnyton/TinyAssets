...
- [ ] 2.1 Add failing result-path tests proving claimable, missing, and refresh-failed targets reject `BLOCKED`, while an exact current-main blocked target is accepted; minimally implement fail-closed validation that retains admission and consumes the existing finite failure budget.
  - **Depends:** 1.1.
