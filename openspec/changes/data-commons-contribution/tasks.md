...
- [x] 0.8 Classify each task below as live / landed / inverted against current code before building.
  > Classified 2026-08-01 against `origin/main` `1dd8d7ba`: **landed prerequisite only:** the `write_page` commons selector consumed by 1.2 (PR #1857). **Live/unbuilt:** 1.1, 1.3–1.5, §§2–5, 6.1, 6.2a–6.6, and §7–8; current wiki frontmatter remains generic, no dataset manifest/admission implementation exists, and no dataset-specific lineage table exists. **Live but dependency-gated:** 1.2 consumes the landed selector; §2 still waits on `build-brain-canonical-store`; 5.4a waits on `outbound-boundary-layer`; 6.2 consumes the still-unbuilt generic N-parent owner. **Inverted:** none. `tinyassets/paid_market/license_terms.py` plus the canonical `paid-market-economy` license-lattice requirement remain the single landed resolution/composition owner; `tinyassets/contribution_events.py` remains the generic append-only event path and `tinyassets/api/market.py` remains the attribution writer.
