## 1. Queue OUTSIDE the run-executor worker

> The bounded-wait attempt (`1800cb83`) was reverted: it held one of four
> process-wide run workers per waiter, so one universe could starve every
> other user. A blocking effector thread is not a queue.
