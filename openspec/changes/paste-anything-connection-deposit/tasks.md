...
- [x] Add the owner-gated resolve handler: accepts credential *shape* + optional
      intent, returns a proposed policy, writes nothing.
- [x] Refuse payloads carrying full credential values, so the
      no-transmission guarantee is enforced at the boundary.
- [x] Narrow or reject any proposal broader than a hand-authored deposit can
...
- [x] Tests: unknown-service resolution, secret-bearing payload refused,
...
- [x] Split a paste into candidate values, sending label + public prefix +
      length only; keep the remainder in the browser.
- [x] Carry hostnames, URLs and field labels through as-is — already non-secret
      and usually decisive.
- [x] Test: a paste with four OAuth values plus extras yields shape with no
      high-entropy material.

## 3. The one-box surface

- [x] Replace the five-field card with one textarea + optional intent line;
      move the explicit fields behind a disclosure, pre-filled from the proposal.
- [x] Deposit straight through with no confirmation step, then show the receipt
      sentence with change and remove attached.
- [x] Test: a paste that resolves deposits with no further interaction; the
