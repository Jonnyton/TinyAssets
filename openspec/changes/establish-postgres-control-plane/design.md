...
- provider work SHALL consume #1784's owner-native
  `ProviderInvocation`/`ProviderExecutor` authority or accepted-market B2/B13
  authority before ordinary routing once that owner is implemented;
- execution SHALL consume #1573's trusted requirement, sealed binding, and
  two-phase admission/evidence validation once that owner is implemented; a
  queue row or database lease is not sandbox or execution-admission evidence;
...
- real-world handoff lifecycle plus outcome-registry integration remain owned
  by #1800, while canonical receipt storage, consent, effect authority, and
  dedup identity remain with the external-effect/outbound-boundary owners;
  #1800 task 5.4 verification/foldback remains unchecked, a handoff-shaped row
  or receipt grants no handoff or receipt authority, and prototype migration
  `014_real_world_handoffs.sql` remains fixture provenance rather than
  production migration history; and
