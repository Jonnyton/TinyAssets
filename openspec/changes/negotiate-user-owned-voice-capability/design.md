...
Jonathan's 2026-09-03 product decision is authoritative: one Voice control stays beside the composer. If the signed-in founder's current provider connection can supply realtime voice, one tap starts Voice. If the universe has no provider, the control reuses the existing provider-request flow. If the current provider powers text but not realtime, the UI reports that exact capability gap and may route into the existing user-owned connection/request surface; it never invents a second credential system or silently selects a different provider.
...
### 5. Session creation repeats every authority and capability check
...
- [The current provider is a subscription CLI] → Explicitly report unsupported until that user-owned adapter can advertise the generic bridge contract; never fall back to platform API usage.
...
- Which already-connected provider, if any, can currently expose the provider-neutral `tinyassets.voice.v1` bridge from Jonathan's own authorization? This is a live acceptance prerequisite, not a reason to add platform authority.
