...
- **AND** `fire_once` fires exactly once under the most recent missed period's own identity
- **AND** a bounded replay reuses each missed period's own schedule-period identity in chronological order

#### Scenario: DST gaps and overlaps fire exactly once with one identity
- **WHEN** a schedule's local fire time falls in a spring-forward gap that does not exist in its IANA zone
- **THEN** the period fires once at the next valid local instant, retaining the nominal period identity
- **AND** when the local fire time falls in a fall-back overlap that occurs twice, the period fires once on the first UTC occurrence with exactly one period identity, and the second occurrence neither fires nor mints an identity
