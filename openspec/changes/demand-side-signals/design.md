...
|---|---|---|
| A standing goal as a new substrate object | **Not irreducible.** A standing goal is `Goal` + `Trigger`, both already canonical work concepts. Nothing about "a goal with a schedule" is a new kind of thing. | Fields and actions on the existing Goal record via `write_graph` / `read_graph`; execution via `run_graph`. |
| Inbox scheduling as a `schedule` or `inbox` tool | **Not irreducible**, and split-owned. Ingress is `outbound-boundary-layer`'s; the consuming schedule is a parameter set on the goal record. | Schedule fields under `write_graph`; ingress stays with the boundary owner. |
...
This is the single highest-risk requirement in the change, because a standing goal's defining property is that it acts while its owner is absent. Two live findings make the risk concrete rather than theoretical: the STATUS Concern that the scheduler trusts a client-supplied `owner_actor` with no caller binding or branch-authority check, and the recurring project finding that a retired or unresolved engine which falls back to an ambient identity runs the user's work on the *host's* credentials. Both say the same thing: for scheduled work, "whoever asked" and "whoever owns" must be different variables, and an unresolvable owner must stop the run rather than borrow one.

So the requirement is written as three separate refusals — caller-supplied fields, externally-written scheduler rows, and ambient environment fallback — because in this codebase each has been a distinct live bypass, and a single "derive from the record" sentence has historically left one of them open. Fail-closed is the default and the escalation path is nothing: not host, not maintainer, not a cached prior identity.
...
- Does the leading metric count a paused standing goal? "Standing goals per active universe" is ambiguous on pause, and the answer changes what the metric measures.
