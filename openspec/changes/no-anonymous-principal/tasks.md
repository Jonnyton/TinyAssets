...
- [ ] 1. `auth/provider.py`: delete `ANONYMOUS`; `DevAuthProvider` resolves the
...
- [ ] 2. `auth/middleware.py`: ContextVar default `None`; `auth_middleware`
...
- [ ] 3. Exempt paths as one predicate with a test per row, reusing the
  ...
  `/mcp/pulse`.
- [ ] 4. Identity sources outside auth: `api/permissions.py` (no `"anonymous"`
...
- [ ] 5. `runs.actor`: `create_run` and every caller require `actor`; the SQL
...
- [ ] 6. Hooks run as their owner: `webhook_hooks` gains `owner_principal_id`,
...
- [ ] 7. `GET /mcp/pulse`: `git_sha`, `image_tag`, `deployed_at`,
  `uptime_seconds`; no principal, no universe data; served before the auth
  middleware.
- [ ] 8. The `canary` service principal: the bearer resolves to identity
...
- [ ] 9. Scripts: `_canary_common.py` sends the bearer; `mcp_public_canary.py`
...
- [ ] 10. Website: `live.ts`, `playground.ts`, `site-react/lib/live.ts` read
  `/mcp/pulse`; the playground shows the 401 challenge as step one.
- [ ] 11. Tests: `signed_in(user_id)` fixture; every test that asserted an
  anonymous read asserts the 401 / `PermissionError`; a test per exempt row;
  a Source event runs under its hook owner; the canary is refused on every
  handle outside its allowlist; none quarantined. Codex design round 3, then
  code review rounds (<=3); verdicts in the PR.
