...
- [ ] 2.6 Add failing host-pool composition tests proving insert-always sessions, current principal+generation on authenticated register/heartbeat/exact-idempotent-deregister, heartbeat's fresh-nonce monotonic server-time update and inability to mutate any other authority, sibling-session preservation, mixed-version rollback, zero-host durability, and unattested legacy/dev ineligibility.
