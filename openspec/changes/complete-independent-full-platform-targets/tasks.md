...
- [x] 3.4 Add clean-machine and upgrade tests under `tests/desktop_install/`, covering offline first run, expired auth, double launch, second-machine identity, crash-safe update, rollback, and content-preserving uninstall.
  - _built and locally verified 2026-07-24 (Windows, Python 3.14)_ — `python -m pytest tests/desktop_install -q` passes 66 tests covering every named case plus a clean-data packaged health probe, authority gating, native-backend allowlisting, signed-installer invocation/restore, signature rejection, SemVer downgrade/platform/channel mismatch, staged rollout, refresh-token rotation, post-stage tampering, packaging definitions, and publication gating. Commit `1894ebbc` also makes CI install, probe, same-version repair, and uninstall the exact unsigned Windows artifact while checking singleton autostart and content preservation. These automated tests do not claim 3.5's signed clean-VM acceptance matrix.
