...
- [x] 3.2 Implement first-run account binding, OS-secret-store use, pending-registration recovery, autostart, uninstall, and updater services in `tinyassets/desktop/onboarding.py`, `tinyassets/desktop/credentials.py`, and `tinyassets/desktop/updater.py`.
  - _built 2026-07-24_ — commits `e5fdb13d`, `24815840`, `688957b6`, and `772f49f6` add browser PKCE/state/nonce/issuer/audience/expiry/redirect validation, distinct host identity, allowlisted native-keyring refresh storage, expiry/retry enforcement, OS-native autostart, content-preserving uninstall, signed channel manifests, rollout cohorts, native Windows installer activation, crash recovery, and last-known-good installer restore. Packaged daemon/MCP roles fail closed until binding and unexpired native credentials are valid. macOS/Linux native update activation is an explicit not-implemented-yet gate, not reported success.
- [x] 3.3 Add `.github/workflows/desktop-release.yml` for Windows, macOS, and Linux artifact builds, provenance, signature/notarization verification, staged channels, and rollback evidence.
  - _built 2026-07-24; signed publication remains host-gated_ — commits `502ceffc` and `948bcdad` build and label unsigned CI artifacts, preserve the macOS bundle for signing, import the Apple certificate in an ephemeral keychain, emit post-sign provenance/SBOM/channel/platform-qualified rollback evidence, and record an explicit no-publication state when publication is not requested. Requested publication fails closed with `signing identity not provisioned` unless every platform identity plus the Ed25519 update-manifest identity is present; `publish-signed` depends on successful identity-bound signature/notarization verification.
- [x] 3.4 Add clean-machine and upgrade tests under `tests/desktop_install/`, covering offline first run, expired auth, double launch, second-machine identity, crash-safe update, rollback, and content-preserving uninstall.
  - _built and locally verified 2026-07-24 (Windows, Python 3.14)_ — `python -m pytest tests/desktop_install -q` passes 62 tests covering every named case plus a clean-data packaged health probe, authority gating, native-backend allowlisting, signed-installer invocation/restore, signature rejection, SemVer downgrade/platform/channel mismatch, staged rollout, refresh-token rotation, post-stage tampering, packaging definitions, and publication gating. These automated tests do not claim 3.5's exact-artifact clean-VM acceptance matrix.
