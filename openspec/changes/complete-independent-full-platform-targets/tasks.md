...
- [ ] 3.2 Implement first-run account binding, OS-secret-store use, pending-registration recovery, autostart, uninstall, and updater services in `tinyassets/desktop/onboarding.py`, `tinyassets/desktop/credentials.py`, and `tinyassets/desktop/updater.py`.
  - _partial 2026-07-24_ — commits `e5fdb13d`, `24815840`, `688957b6`, `772f49f6`, and `bc32ee6c` build validated browser-PKCE state, native-keyring storage, expiry/retry enforcement, authority gates, autostart/uninstall roles, signed rollout cohorts, and native Windows installer activation/restore with a pinned-key gate. Remaining: construct the production origin client/callback listener and expose sign-in/recovery in the tray. The origin already ships a PKCE authorization server in `tinyassets/auth/provider.py`, `wellknown.py`, and `workos_provider.py`; the narrower missing pieces are a native/public-client loopback redirect allowance plus the host-registration route that completes account binding. Until those exist, a clean user remains safely paused rather than falsely online. macOS/Linux native update activation remains an explicit not-implemented-yet gate.
