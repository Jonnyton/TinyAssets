...
- [ ] 3.2 Run focused pytest, ruff, plugin mirror build, strict OpenSpec validation, and independent cross-family review on the exact head.
- [ ] 3.3 Merge, deploy, run the authenticated public handle canary, and obtain sanitized rendered proof from a bound agent using an owner-controlled disposable Branch.
- [ ] 3.4 Sync the shipped delta specs, delete the resolved concern, archive this change, and verify production contains the exact merged SHA.
