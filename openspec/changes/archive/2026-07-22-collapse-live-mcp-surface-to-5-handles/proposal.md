...
- Expose the 5 canonical handles (`read.graph`, `write.graph`, `run.graph`, `read.page`, `write.page`) as the user-facing tools on the LIVE server `tinyassets/universe_server.py` (the process behind `https://tinyassets.io/mcp`).
- Implement each handle as a thin dispatch layer (a `shape=` / payload router) over the EXISTING action handlers in `tinyassets/api/*` — no behavior change to the underlying primitives, only the surface shape. Forward-port the PR-047/#617 `directory_server` implementation rather than rebuild from scratch where possible.
...
- Retire `tinyassets/directory_server.py` as a parallel surface once `universe_server.py` is the single source of the 5-handle contract.

## Capabilities

### New Capabilities
- `mcp-five-handle-surface`: The live user-facing MCP contract — exactly five tools (`read.graph`, `write.graph`, `run.graph`, `read.page`, `write.page`), their `shape=` parameter mapping to the 6 primitives, their routing to existing `tinyassets/api` handlers, and the canary invariant that the deployed `/mcp` advertises exactly these handles.
...
- **Code:** `tinyassets/universe_server.py` (tool registration cutover), `tinyassets/api/*` (handler routing under a `shape=` dispatcher), `tinyassets/directory_server.py` (retire after cutover), the `packaging/claude-plugin` runtime mirror (must stay byte-parity).
