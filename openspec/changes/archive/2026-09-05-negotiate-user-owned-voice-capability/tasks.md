...
- [x] 5.4 Treat the failed live acceptance as a product bug: add a disclosed browser/device speech mode around canonical `converse` when typed conversation works but no realtime bridge exists, remove the redundant Connect prompt, prove exact transcript routing and teardown in deterministic browser tests, review/land/deploy, then stop before Jonathan's microphone permission and live turn.
