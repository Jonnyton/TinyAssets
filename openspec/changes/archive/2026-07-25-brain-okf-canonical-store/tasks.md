...
- [x] 4.1 `sync-specs`: merge the `brain-canonical-store` delta into `openspec/specs/brain-canonical-store/spec.md` (after host merge key)
  ...
  >
  > **What was done instead:** the delta is relocated verbatim (all six Codex adaptations intact) into the successor change `openspec/changes/build-brain-canonical-store/`, which owns the build and syncs the capability when — and only when — the behavior ships. The delta keeps a live owner; the as-built spec surface stays truthful. The copy under this change archives as its historical record.
