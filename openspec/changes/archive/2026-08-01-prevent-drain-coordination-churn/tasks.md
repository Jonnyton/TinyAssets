...
- [x] 3.2 Re-run focused tests/lint/strict OpenSpec and exact-head review, land through a reviewed PR, restore the original `drain-20260801-113628-6deab6` run, prove its existing claim resumes automatically, then sync/archive and retire the STATUS row. Completed 2026-08-01: PR #2105 merged as `a5594989` after exact-head Claude approval; 202 focused tests passed; the original run resumed its preserved admission as attempt 35 with `claimable=1`, `owned=1`; watchdog health reported running/attach with the same identity.
