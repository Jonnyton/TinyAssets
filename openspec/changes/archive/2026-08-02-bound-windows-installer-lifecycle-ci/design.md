...
The workflow pins Python through `actions/setup-python` and invokes a stdlib-only supervisor. The supervisor launches `windows_lifecycle.ps1` as a child `pwsh` process, redirects stdout and stderr to private named temporary binary files, and calls `Popen.wait(timeout=...)` for the total deadline. PowerShell is now only the tested lifecycle child; process wait, timeout classification, byte-capped replay, and fallback cleanup are owned by the Python parent.
...
The child writes stdout and stderr to private `NamedTemporaryFile` writer handles. The supervisor records each writer's size, then reopens the path through an independent read handle and replays at most 256 KiB from that fixed byte horizon before returning success or failure. The separate read handle is required because duplicated Windows process handles share a current file position; seeking the writer handle would let a surviving descendant displace or overwrite the replay cursor. The supervisor emits the observed byte count when evidence is truncated and makes a best-effort bounded cleanup of both paths. Redirecting away from the runner pipe prevents an escaped descendant from retaining the workflow step's output handle; the byte-capped snapshot prevents a noisy or surviving writer from extending replay past the total-deadline margin.
...
- **A descendant survives both bounded cleanup attempts** -> private named capture handles prevent it from holding the workflow pipe open, and independent read handles preserve the diagnostic prefix; Python reports without waiting for descendants, while runner cleanup and the job timeout remain defense in depth. Windows may defer deletion of a capture still held by that descendant, so cleanup is best-effort and emits a warning rather than extending the verdict.
