...
is pre-listed in the server-owned isolated-canary set. Every operation that
makes such a universe observable as living SHALL persist
...
While the global flag is false and the canonical ID is not in the canary set,
...
data cannot add an ID to the canary set, and an existing user universe MUST
NOT be migrated merely to obtain pre-flip proof.
...
- **WHEN** any birth path succeeds while the global flag is false and its canonical ID is absent from the canary set
