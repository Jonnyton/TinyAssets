...
- [ ] 1.5 Obtain Claude Opus 5 re-review of the exact adapted artifacts;
  resolve every Critical and Important finding.
- [ ] 1.6 Run strict target/full-tree validation and land this target active
  and unsynced. Close/supersede draft #1691 only after the replacement and all
  citation handoffs are durable.

## 2. One-way sibling handoffs

- [ ] 2.1 `retire-mcp-provider-secret-deposit` consumes exported
  ...
  ownership there.
- [ ] 2.2 `universe-creation` removes its caller-built “immutable authority
  ...
  authority.
- [ ] 2.3 `provider-attempt-receipts` consumes same-call
...
- [ ] 2.4 Create or land `activate-requester-host-engines` under
  `daemon-identity-and-host-pool`, `desktop-host-runtime`, and
  `provider-routing`, consuming a stable authenticated account-to-host
  principal and `daemon_summon`; pool rows and unattested client identifiers
  grant no authority.
- [ ] 2.5 Preserve paid-market/distributed ownership: accepted agreement in
...
- [ ] 3.4 Prove at least one target-ready source can be deployed before
  cutover: requester-local opaque custody or the requester-host successor.
  Record the live founder home's current source/credential evidence without
  reading or exposing secret material.
...
- [ ] 4.1 Add failing auth-middleware tests proving only a validated
  ...
  `ProviderRequestCapability`, and reset/prior-request replay/lookalikes fail.
...
- [ ] 4.6 Add failing source tests proving self-hosted/host-daemon/market
  intent is held/deny-all without owning activation proof and that raw-secret
  refusal remains in the custody lane rather than this resolver.
...
- [ ] 5.2 Implement independent newborn deny-all initialization in the atomic
  birth transaction for every entry path.
- [ ] 5.3 Implement canonical requester-local resolver accepting only an
  existing opaque binding reference and strict service/writer mapping.
...
- [ ] 6.1 Add exhaustive call-site tests proving live requests retain the
  exact current capability, background work requires its owner receipt, and
  remaining paths are explicitly host-local or local-only.
...
- [ ] 6.4 Add canonical-preservation tests for `preferred_writer`,
  ...
  non-request absent-context fallback, and all existing
  `UniverseContext`/`ModelConfig`/`ProviderResponse` fields.
- [ ] 6.5 Add host-capability forgery tests across API/MCP/JSON/config/env/node
  ...
  prove no maintainer resource becomes reachable.
- [ ] 6.6 Add provider parity tests proving every CLI/local/HTTP/in-process
  invocation is reference-only and only executor-local `start()` dereferences
  after full binding revalidation.
...
- [ ] 7.1 Implement sink validation for exact current request capability or
  owner-defined background receipt, fresh assignment/binding tuple, and
  authority-derived provider set before dynamic filters.
- [ ] 7.2 Implement sole provider-layer propagation through every inventoried
  call site and bootstrap-only host-local capability for enumerated
  maintenance.
...
- [ ] 7.4 Replace direct one-phase provider execution with
  `start() -> ProviderLaunchHandle -> result()`, executor-local dereference,
...
- [ ] 8.3 Block cutover unless requester-local opaque custody or
  requester-host activation passes end-to-end and the live founder home has a
  reviewed ready mapping or explicit replacement. Stop before quiescing legacy
  writers if not.
