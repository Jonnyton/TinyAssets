...
authorization, the WorkOS Resource Indicator, and
`WORKOS_MCP_RESOURCE` must all be the same canonical URL after normalization.
The repair will capture current values without tokens, reproduce once with safe
diagnostics, and correct only the mismatched boundary the evidence identifies.
...
The active `activate-main-universe-spec-drain` change will describe Jonathan's
drain as the first conformance instance of a generic user-authored
GitHub-to-spec production pattern. Only planning, specification, review, and
dependency verification may advance until the live connector completes the
authenticated continuity sequence. No drain runtime code—including inactive
adapters, persistence, execution, or activation scaffolding—may start before
that proof.
