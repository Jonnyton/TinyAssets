...
- [ ] 1.2 Deploy diagnostics without changing token acceptance, reproduce one post-reconnect authenticated call, and record the safe failure category plus public metadata/deployed-resource configuration parity. Evidence through 2026-07-31/2026-08-01: ChatGPT returned through `link_success=true`, an explicitly reattached call hung, and a new Temporary Chat immediately rendered the connection expired. Runs 30680168689 and 30680303470 recorded `oauth_rejection_categories=[]`, but a deliberate malformed-bearer 401 in runs 30681000676/30681046575 also recorded `[]`; empty categories are not live-sensitive proof until the real logging envelope is identified. Public PRM/resource/scopes and deployed WorkOS variables match; AuthKit supports authorization code, refresh, PKCE S256, public-client exchange, and DCR but omits CIMD advertisement. The required authenticated call remains unproven.
