...
The run successor should preserve authorization beyond start: every direct or background run record stores a server-bound universe authority context independently of its display/provenance `actor`; later run surfaces conjoin universe ACL and parent-branch authority. Synthetic `scheduler:*` or `subscriber:*` actors remain provenance only, and a public universe does not make a private-branch run visible to non-authors.
...
The background successor should never mint migration authority from legacy `owner_actor`, environment identity, `soul.md`, or queue data. It may create an audited receipt only when an independent existing server record proves the same credential-bound principal and authorization for the unchanged target. Other legacy bindings become inactive with `reauthorization_required` until a fresh authenticated binding; direct public execution remains unaffected.

The existing epoch-1 in-node enqueue path remains public-only because its task row carries no authenticated authority receipt; the distributed-execution owner should not broaden it until an equivalent server-bound receipt exists.
