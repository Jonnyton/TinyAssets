...
#### Scenario: Caller attaches gate evidence to another author's private branch
- **WHEN** a caller claims a gate rung, claims from a run, or records a branch-scoped conformance pack for another author's private branch
- **THEN** the operation is denied before run output is disclosed or claim/conformance state is persisted

#### Scenario: Independent writer claims a public branch rung
- **WHEN** an authorized gates writer claims a rung or records a conformance pack for a public branch they do not author
- **THEN** the independent evidence remains allowed and its claimant/pack author is the credential-validated request subject

#### Scenario: Public-branch claim names an unreadable run
- **WHEN** an authorized gates writer uses `claim_from_branch_run` for a public branch but lacks read authority for the named completed run
- **THEN** the operation is denied before run output is read and no claim is persisted
...
#### Scenario: Caller schedules another author's private branch
- **WHEN** a caller schedules or subscribes another author's private branch while supplying that author's `owner_actor`
