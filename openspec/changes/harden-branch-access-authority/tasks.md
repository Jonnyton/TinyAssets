...
- [ ] 8.6 Add RED gate-event citation tests proving new globally readable events reject every private-parent or missing-parent version before persistence, while public citations remain available; seed legacy private citations and prove get/list/gate-event-leaderboard filters before counts/ranks and exposes no parent ID.
- [ ] 8.7 Add RED remix/provenance tests proving unreadable-parent and non-owned-child denials persist no edge/credit, accepted attribution uses the request subject, and root/ancestor/edge traversal stops before every unreadable branch without IDs, actors, credits, or counts.
- [ ] 8.8 Add RED scheduler/subscription tests proving branch-author authority, server-bound owner, environment/caller-owner non-authority, subject-only lists, foreign-ID/missing-ID equivalence, and no schedule/subscription/provider work after denial.
- [ ] 8.9 Consume the shared request-subject, readable-branch, version-parent, and author helpers at each action boundary before lookup, projection, attachment, Goal mutation, citation, attribution, or scheduling; do not create a new capability or treat version-ID possession/publication as authority.
- [ ] 8.10 Run `python -m pytest tests/test_branch_adjacent_authority.py tests/test_run_branch_authority.py tests/test_branch_evaluation_authority.py -q`, surrounding goal/gate/leaderboard/selector/gate-event/attribution/scheduler suites, and mutation probes for bind, global version binding, gate attachment/read, each projection viewer, gate-event citation filtering, remix/provenance, scheduler ownership, and dry-inspect gates.
