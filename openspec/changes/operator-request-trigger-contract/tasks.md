...
- [ ] 5.9 Add integration tests proving a won epoch-2 scheduling claim without valid B2 signed authority cannot create an external lease, execute, or submit a result, and that queue/admission/heartbeat rows only narrow or reject B2 authority. Closed-consumer inertness is mutation-proven at the real materialization/restart seam, and admission/claim/heartbeat rows cannot be signed as B2 records; the external B2 handoff remains unwired behind 5.8 and distributed-execution 6.4.

## 6. Worker Protocol Evidence and Operational Surfaces

- [x] 6.1 Add failing heartbeat/descriptor tests in `tests/test_cloud_worker.py` and `tests/test_supervisor_liveness.py` for protocol, capability, build/config SHA, boot/worker/runtime/universe IDs, per-worker isolation, and 90-second expiry.
- [ ] 6.2 Extend `tinyassets/cloud_worker.py` and `tinyassets/daemon_registry.py` so release/runtime-derived descriptors are written to durable runtime metadata and each `.worker_supervisor.<worker>.json`; never derive support from request arguments, identity grants, or provider auth. Descriptor persistence plus 5.7 claim-gated materialization and claim-bound selection are implemented and verified, but publication remains intentionally disabled until 5.8 makes the supervised daemon a truthful epoch-2 claimant and lifecycle owner.
- [x] 6.3 Extend `tinyassets/api/status.py` and `tinyassets/api/universe.py` operational reads to merge epoch counts and distinguish `awaiting_compatible_capacity`, `invalid_operator_admission`, `quarantined`, and `policy_parked`.
- [ ] 6.4 Update `tinyassets/cloud_worker.py::_has_pickable_branch_task` and wakeup/restart decisions to use eligible merged candidates without treating pending-no-capacity as invalid or spending platform compute. Safe staged selection/status groundwork exists, but activation requires 5.8 to install a production expired/dead-peer lease-recovery owner before `EPOCH2_QUEUE_CONSUMER_READY` may become true.
