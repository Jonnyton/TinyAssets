...
- [ ] 6.2 Extend `tinyassets/cloud_worker.py` and `tinyassets/daemon_registry.py` so release/runtime-derived descriptors are written to durable runtime metadata and each `.worker_supervisor.<worker>.json`; never derive support from request arguments, identity grants, or provider auth. Descriptor persistence and 5.7 claim-bound materialization are implemented and verified, but publication remains intentionally disabled until 5.8 makes the supervised daemon a truthful epoch-2 claimant and lifecycle owner.
