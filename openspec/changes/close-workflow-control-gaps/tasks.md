...
- [x] 1.1 Complete independent shape/basic-safety review and adapt the shared lifecycle design.
- [ ] 1.2 Pin baseline and add failing regressions through actual dispatch/read/edit/control paths for the five reported gaps.

## 2. Implement

- [ ] 2.1 Make edit guidance and validated owned-node operations agree, with creation/update safety parity.
- [x] 2.2 Pass trusted graph ancestry to workspace resolution and prove discard plus sibling exclusion through dispatch.
- [ ] 2.3 Expose exact, bounded owned-run output reading through existing graph handles.
- [ ] 2.4 Record failed code-node identity and terminal state without mislabeling parallel siblings.
- [ ] 2.5 Expose scoped queued/running cancellation with accurate pending and terminal readback.
