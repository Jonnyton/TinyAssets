...
- [x] 3.7 Add a Slack-first dark app-event boundary that verifies the signature over the exact bounded raw body before parsing, derives installation/event identity only from the authenticated envelope, and persists content-free replay evidence without mapping a TinyAssets principal or invoking custody, runtime, effects, routes, or a new MCP handle. Completed 2026-08-03 in `tinyassets/app_event_ingress.py` plus the content-free store; no production consumer or packaged mirror was added while the cloud lane owns the broad runtime mirror write-set.
- [x] 3.8 Prove fail-closed behavior for stale/malformed/missing signatures, wrong app identity, malformed/oversized envelopes, deprecated verification-token-only requests, conflicting replay, store tampering, and concurrent duplicate delivery; rejected authentication/shape writes no row and an exact replay returns one record. Evidence 2026-08-03: 21 focused tests include Slack's published signature vector and a 64-way duplicate race; 50 existing outbound-boundary/receipt regressions pass; strict OpenSpec and mirror parity pass.
