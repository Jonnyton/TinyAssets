...
Until single-active cloud cutover, the OpenSpec drain supervisor SHALL treat an exact assigned refinery target that is proven owned by a pre-existing open pull request as unavailable for the bounded run rather than as a failed delivery attempt. It MUST suppress only that target, MUST preserve every suppressed target for the entire bounded run, MUST immediately continue to another eligible candidate, and MUST NOT record a completed slice, merge receipt, or failure strike. The exact refinery assignment MUST be persisted before dispatch so crash recovery can apply the same target and fresh pull-request checks before consuming an unrecorded result. Missing, malformed, non-open, same-run, or unqueryable pull-request evidence MUST remain on the ordinary fail-closed path.

#### Scenario: Older open PR owns the assigned refinery target
- **WHEN** an assigned refinery worker reports `FAILED` with an exact pull request that is still open and predates the bounded run
...
- **AND** failed verification leaves the result unconsumed for ordinary fail-closed retry handling

#### Scenario: Reported PR does not prove pre-existing live ownership
- **WHEN** the reported pull request is unavailable, closed, merged, malformed, or created during the bounded run
