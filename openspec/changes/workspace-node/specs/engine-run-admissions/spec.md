...
The engine SHALL admit every `workspace` operation (`checkout`, `push`, `discard`, provisioning) as kind `workspace` in the per-universe rolling ledger, bounded by jobs per hour (default 10) and bytes per hour (default 20 GiB), both tier-raisable, and SHALL name the exhausted bound in a refusal.
