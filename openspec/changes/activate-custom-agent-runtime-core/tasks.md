...
- [ ] 2.1 Test-first, add the shared typed execution subject to canonical activation, derive the sole agent automation key from `(universe_id, agent_binding_id)`, and prove concurrent aliases cannot create a second row while every Branch CAS/epoch/executor/lease guard remains unchanged.
