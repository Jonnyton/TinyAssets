...
- [x] 1.12 Add first-connect tests proving an authenticated founder with no home universe gets a blank seed universe created and bound and that opening `converse` resolves that home rather than mutating `get_status`. (`tests/test_first_contact.py`; birth/routing only, not provider-backed speech.)
- [x] 1.13 Add first-connect tests proving an authenticated founder with an existing home universe resolves and loads that learned universe for opening `converse`. (`tests/test_first_contact.py`; resolution only, not provider-backed speech.)
- [ ] 1.13a Add end-to-end execution-authority tests for both outcomes: **(a)** an eligible complete authority bundle — requester-owned compute or an accepted market compute offer, plus requester-owned model access or an accepted market model grant when the workload is gated — yields birth plus a first-person reply; **(b)** when no eligible complete bundle exists, the path never uses maintainer credentials, quota, or hardware and returns an actionable held/setup response instead of generic `provider_exhausted`. The missing-authority case MUST go red against `519fb2ea` before any fix; the existing stubbed `call_provider` reply assertion is not this proof.
...
- [~] 2.7 Implement founder home-universe resolution for MCP first contact: authenticated founder -> existing home universe or new blank seed universe -> loaded soul/persona -> first-person universe voice.
      > **Partially landed; re-opened 2026-07-22.** The resolution/birth half shipped in `519fb2ea` and is live-verified: opening `converse` (no `graph_id`) resolves-or-creates and binds the home. Execution authority is incomplete. Task 1.13a must prove both paths: an eligible complete bundle — requester-owned compute or accepted market compute, plus requester-owned model access or an accepted market model grant when gated — yields the universe's own reply, while missing authority never consumes maintainer credentials/quota/hardware and yields an actionable held/setup response rather than generic `provider_exhausted`.
