...
- [x] 1.12 Add first-connect tests proving an authenticated founder with no home universe gets a blank seed universe created and bound, the chatbot loads that seed soul/persona, and the first response speaks in first person as the universe rather than returning platform status as the main experience.
      > **Tests exist and pass; the end state does NOT hold live (verified 2026-07-22, prod `519fb2ea`).** Birth works. Speech does not — `converse` returns "All providers exhausted for role=writer" because a newborn universe has no vault credential and `providers/base.py:181-189` (from `92dd60c5`) correctly refuses to fall back to host credentials. See task 1.14. Left checked because the test artifact is real; do not read it as live acceptance.
- [x] 1.13 Add first-connect tests proving an authenticated founder with an existing home universe loads that learned universe soul/persona and speaks as that universe.
      > Same caveat as 1.12: unit-proven, not live-proven.
- [ ] 1.14 Add an end-to-end test that a founder with no prior universe completes first contact **and receives a first-person reply**. Every existing first-contact test asserts the universe is BORN; none assert it can SPEAK, which is how 97 green tests shipped a mute universe on 2026-07-22. This test MUST go red against `519fb2ea` before any fix, or it is not testing the defect.
...
- [ ] 2.7 Implement founder home-universe resolution for MCP first contact: authenticated founder -> existing home universe or new blank seed universe -> loaded soul/persona -> first-person universe voice.
      > **Re-opened 2026-07-22.** The resolution/birth half shipped in `519fb2ea` and is live-verified: the founder's opening `converse` (no `graph_id`) resolves-or-creates and binds the home. The FINAL link — "first-person universe voice" — is not delivered: the universe is mute for want of a credential. This task is not done until a founder's first contact actually yields the universe's own voice in a rendered chatbot.
