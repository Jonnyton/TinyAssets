...
If the media connection drops while `MCP.converse` is in flight, that request is allowed to finish and its result is shown in text; the app does not resubmit it. Each in-flight result is bound to the voice epoch and data channel that originated it, so a late success cannot speak into or pre-authorize a replacement transport and a late failure cannot tear that transport down. Connection attempts likewise own their local peer, channel, audio element, and stream: a stale or failed attempt disposes only those locals and cannot invoke global teardown on a newer attempt. If delivery outcome is unknown, the user sees an explicit retry action instead of automatic replay. After reconnect, the app reloads canonical text history but does not replay prior audio or automatically read the last reply.
