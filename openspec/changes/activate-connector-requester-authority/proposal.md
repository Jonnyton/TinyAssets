...
None. This change composes five existing capability owners and adds no new top-level platform primitive.
...
- `paid-market-economy`: Defines the internal tenant-scoped `paid_market.accept_agreement_v1` producer and per-job logical budget reservation/accounting intent, while preserving the rule that submission, ranking, matching, claiming, database accounting, or provider evidence grants no domain capacity, real-fund, wallet/chain, host, or execution authority.
