...
revocable, non-executable activation grant created through the sole B13
production composition root and bound to the authenticated owner, tenant,
target universe, accepted agreement and route, capacity fence, spend ceiling,
expiry, revocation generation, and idempotency digest. It MUST NOT mint or
store a B2 execution grant before a concrete job and capsule exist. For each
later job, only the B13 root may create the exact B2 grant bound to owner,
daemon, job, capsule digest, lease, generation/fence, capability ceiling,
expiry, and idempotency identity required by the distributed-execution
contract.
...
#### Scenario: complete B13 activation authority binds the remote assignment

- **WHEN** the B13 production root returns a current non-executable activation grant bound to the exact accepted agreement, route, and target universe
...
- **WHEN** an activation grant, quote, agreement, match, payment record, Request, BranchTask, scheduling claim, user automation, provider-attempt receipt, or admission receipt exists without an exact current per-job B2 grant
...
re-derive the current accepted agreement and B13-bound activation grant, then
obtain and verify a fresh B2 grant for the concrete message/job and capsule at
the distributed-execution sink before dispatch. Only that exact B2 grant SHALL
enter the signed remote-execution seam before the ordinary provider router,
and the ordinary provider ceiling/chains MUST NOT be consulted for that work.

An empty `allowed_providers` list in `remote_ready` state means remote-only
dispatch under a current activation grant plus an exact per-job B2 grant; it
MUST NOT mean ambient permission to use maintainer, requester-host, local,
BYOC, free, environment-selected, or role-default providers.

#### Scenario: valid accepted-market universe executes remotely

- **WHEN** the next authenticated `converse` targets an accepted-market universe whose agreement and activation grant are current and the B13 root returns the exact per-job B2 grant
...
- **THEN** only a current activation grant plus an exact per-job B2 grant can authorize work and no ordinary provider chain or maintainer resource is attempted
...
an absent, expired, revoked, cancelled, superseded, fenced, inconsistent, or
unverifiable activation grant or per-job B2 grant. The assignment owner SHALL
atomically downgrade stale `remote_ready` to `held + []`, preserve signed
...
- **WHEN** activation-grant revocation, per-job B2 revocation, or a higher generation/fence commits before the first accepted-market `converse` authority decision
...
- **WHEN** current activation or per-job B2 authority cannot be verified because its production signer, trust root, or verification owner is unavailable
...
- **WHEN** a current accepted-market activation grant and exact per-job B2 grant exist but capability preflight, capsule binding, sandbox admission, or runner enforcement evidence is incomplete
...
- **WHEN** a job has valid admission and sandbox evidence but lacks its current activation grant or exact per-job B2 grant
