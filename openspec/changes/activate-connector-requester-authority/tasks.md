...
- [ ] 2.7 Land internal tenant-scoped `paid_market.accept_agreement_v1`, consuming the canonical request, current quote, and explicit acceptance without reusing submission/bid/match/claim/delivery as acceptance; bind all request fields server-side and require canonical `requester_user_id`/tenant/universe equality with current authenticated scope.
...
- [ ] 4.2 Invoke internal `paid_market.accept_agreement_v1` with the rehydrated current request and quote after exact requester/tenant/universe equality; do not treat submission, ranking, bids, matches, claims, reservations, payment intent, or a database row as acceptance, money, provider, host, or execution authority.
