...
- [ ] 4.3 Obtain the current bounded non-executable B13 market mandate and atomically commit its opaque reference plus the accepted agreement and `engine_source="accepted_market"`, `engine_assignment_state="remote_ready"`, `allowed_providers=[]`; every fault or losing race commits none of that activation state.
- [ ] 4.4 Make concurrent first activation single-winner and cover cancellation, quote expiry, capacity loss, fee/version drift, revocation-generation, fence, timeout, and post-commit response-loss retries without duplicate agreement, mandate, reservation, charge, grant, or assignment.
