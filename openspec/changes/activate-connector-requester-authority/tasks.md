...
- [ ] 2.4 Land the B13 production composition root interface for a bounded non-executable market mandate and exact per-job firm-quote/capacity/funding/B2 authority creation; do not pre-mint B2 before a concrete job/capsule exists.
...
- [ ] 2.6 Land the separately owned wallet/chain-settlement authority, an activation transaction capable of committing accepted agreement, mandate reference, and universe assignment atomically, and a per-job transaction capable of reserving requester funding and capacity without oversubscription.
...
- [ ] 3.2 Implement the closed `accepted-market-activation/v1` acceptance object with exact request, selection-receipt, firm-quote, descriptor, integer-micros budget/spend-cap, currency, fee, demand, acceptance/settlement-policy, deadline, and expiry commitments; forbid actor, tenant, provider, host, credential, wallet, grant, lease, or authority-carrier fields.
...
- [ ] 3.4 Authorize before any replay lookup; domain-separate idempotency as `write_graph/engine/activate_accepted_market`; bind actor/tenant/universe/key plus the canonical body digest; prove no request/other-action collision; return the original typed result for same-body replay and conflict on changed-body reuse.
...
- [ ] 5.1 Before ordinary provider routing, re-derive the accepted agreement and current market mandate for every accepted-market `converse`; derive the concrete job demand and quantity; obtain a fresh executable firm quote; revalidate fees, currency, service policy, capacity, and remaining budget; atomically consume capacity and reserve requester-owned or explicitly delegated funding; then build the concrete job/capsule and ask B13 for its exact current B2 grant.
- [ ] 5.2 Bind and verify the agreement, mandate, quote, demand, quantity, capacity-consumption, funding-reservation, fee, spend-ledger, owner, tenant, universe, daemon/host, job, capsule digest, lease, generation/fence, capability ceiling, expiry, revocation, idempotency, and Engine OS admission at the distributed-execution sink before external work.
...
- [ ] 5.4 Serialize concurrent budget/capacity/funding consumption; reuse the same reservation and B2 for same-job retries; conflict on changed bodies; release pre-dispatch reservations exactly once; settle verified post-dispatch use and release/refund unused amounts idempotently; on absent, expired, revoked, fenced, cancelled, overspent, consumed, inconsistent, or unverifiable mandate/per-job authority, atomically downgrade stale `remote_ready` to `held + []` and return a typed accepted-market refusal, repair, or renewal result without widening authority.
...
- [ ] 6.3 Add atomic transaction and fault-injection tests for each owner boundary plus simultaneous activation/job/retry/cancel/expiry/capacity/funding/budget/fence/revocation races, proving one activation winner, no oversubscription, exactly-once release/refund/settlement, and zero phantom `remote_ready` states.
- [ ] 6.4 Add market tests proving explicit accepted agreement, full canonical-request rehydration, integer-micros budget/spend binding, current selection/quote/fee/settlement/policy binding, no silent lane substitution, and no promotion of submission/ranking/match/claim/reservation/payment/database evidence.
- [ ] 6.5 Add distributed-execution tests proving market mandates are non-executable, every B2 binds the exact per-job economic/capacity/funding consumption and concrete job/capsule, non-authoritative receipts cannot be promoted, per-job admission remains mandatory, and ordinary provider routing is never called.
