...
- [ ] 4.4 Build `scripts/migrate_engine_assignments.py` plus focused tests and the deploy/runbook gate: strict raw config/vault/ledger inventory emits a secret-free reviewed decision manifest; apply reclassifies under the exclusive lock, preserves unrelated bytes/keys, writes atomically/idempotently, and requires zero unclassified/unmigrated assignments. Offline production execution must quiesce/mask all legacy writers/recovery timers, prove no `/data` mount users, migrate with the new immutable image, run daemon-only loopback canary before exposure, and forbid automatic rollback to a pre-fence writer. Production access or fence unavailability blocks merge/rollout.
