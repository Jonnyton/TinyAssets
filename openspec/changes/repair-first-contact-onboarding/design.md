...
Runtime files are owned by the active universe-creation, universe-visibility/shared-Goals, and `control_station` prompt-truth lanes; newborn BYOC behavior landed in #1759 and is a read-only base, while executable run guidance still waits on the full authority/isolation gates. The production Postgres substrate and `scope=commons` route are also read-only dependencies. Retire-legacy caller inventory v4 landed in #1772 and left implementation pending; this packet adds its missing publish, approval, and remix/lineage gates as retire-legacy tasks 2.3a/2.3b/4.0 without claiming runtime. No `broad-test` lane exists. This change therefore produces the contract and review packet only.
...
- This lane does not itself repair the existing exact-branch related-wiki and hidden-Goal projection leaks. New catalog/create/publish runtime, deployment, prompts, and rendered inspection remain hard-blocked until the visibility/Goals owners land or accept safe canonical/internal exact-branch projections and their shared helpers.
...
### 4. The shared branch catalog has one Postgres authority
...
The platform catalog is a transactional control-plane domain. Postgres is the sole canonical read and mutation authority for hosted Branch definitions and versions, verified owner bindings, catalog publications, catalog projection rows, create/publish idempotency records, cursor-nonce leases, and catalog outbox events. A hosted create, patch, publish, rollback, or catalog read never treats SQLite, Git/YAML, an in-process cache, or environment identity as positive catalog authority. Each accepted domain command commits its affected canonical state and outbox events in one Postgres transaction.

Existing SQLite Branch state is bridge input only. A reviewed one-way cutover may quiesce legacy shared writes, snapshot and hash the source, import into Postgres, reconcile content and counts, and then switch authority once. After cutover, any SQLite execution copy is a disposable, rebuildable outbox projection: stale, missing, forged, or conflicting local rows cannot change catalog eligibility, ownership, publication, or idempotency truth. Postgres unavailability fails hosted operations closed rather than falling back or dual-writing. An isolated OSS/local-only namespace remains locally readable until it explicitly imports through the canonical boundary, but its routing/IDs stay distinct and can neither collide with shared IDs nor claim shared-catalog eligibility.

Results preserve the handle's existing unset/default limit of 30 and cap explicit limits at 100. A Postgres catalog projection gives bounded filtering and non-starving keyset ordering `(created_at DESC, branch_def_id ASC)`, so unrelated mutations do not invalidate a walk. The ten-minute AES-256-GCM cursor has a closed envelope, a durable Postgres per-key unique 32-bit issuer-prefix lease plus 64-bit counter nonce, full 128-bit tag, immutable last-examined tuple, and actor/filter digest. `TINYASSETS_BRANCH_CRYPTO_KEYRING` supplies versioned root keys; HKDF separates cursor encryption from idempotency HMAC, rotation retains prior keys for the configured record lifetime (minimum 24 hours), and absent/malformed configuration fails catalog/create/publish closed. A `published` result verifies active canonical version/publication rows and the current monotonic publication revision in one bounded Postgres authority; compare-and-advance outbox consumption prevents delayed events from regressing the pointer. Named bounded maintenance reconciles projections without mutating state from the read-only handle. The response exposes no hidden total count. Summaries use exact bounded fields, split ordinary and conditional edge counts, and report structural validation rather than ambiguous execution readiness. They are deliberately smaller than exact branch reads and never carry node bodies, prompts, source, wiki metadata, Goal data, gate/custody data, or execution authority.
...
Create and publish require printable-ASCII 16–128 character `idempotency_key` values. The raw key is HMAC-fingerprinted under HKDF-derived keyring secrets and actor scope, and each command is bound to an RFC 8785 canonical digest. Retry lookup and unique alias reservation cover every retained secret version, preventing rotation or rolling servers from creating a duplicate. Create, patch, and publish each use one connection-accepting Postgres domain transaction. Create commits the canonical Branch definition/version, projection, verified owner, idempotency aliases/result, and attribution outbox event together. Patch commits its Branch revision, projection, immutable non-catalog pre/post Postgres BranchVersions, and replica outbox events together. Evaluation, selector, and service snapshot intents synchronously create/reuse a non-catalog canonical Postgres BranchVersion and return its stable ID; SQLite is only its asynchronous execution replica. Publish resolves replay before mutable content, verifies current ownership and snapshot in Postgres, and commits `BranchCatalogPublication`, monotonic revision, projection pointer, idempotency result, and outbox events together. Consumers compare-and-advance and deduplicate by event identity so delayed delivery cannot regress or duplicate state.
...
The registered `control_station`, `meet_universe`, `extension_guide`, and `branch_design_guide` prompts must describe workflows using only the seven handles and their supported targets. The first-contact path starts with `converse`; starter-branch examples compose catalog, create, inspect, explicit publish, and rediscovery through `read_graph`/`write_graph`. V1 guidance is prompt-template-only. Until the requester-owned BYOC/accepted-market authority bundle, ambient-credential isolation, held/setup-required envelope, and race-safe authority receipts land, guidance stops at rediscovery and does not present `run_graph` as executable. After those owners land, an optional run must use only the resolved requester authority and never maintainer/founder/platform quota. Advanced source-code authoring stays out of the first-contact path until a canonical approval route exists; any broader guide must disclose that the current compiled path is not OS-isolated rather than promising sandbox execution.
...
Before runtime landing, a deterministic fixture exercises 500 logical clients and 1,000 mixed operations (750 catalog reads, 200 creates, 50 publishes) against the approved production-shaped Postgres topology, role/RLS boundary, pool, and migration history. It includes tied immutable timestamps, 100 concurrent identical retries, unsupported-private requests, deliberate-versus-snapshot publication, publish/rollback races, mutation-during-pagination completion, nonce uniqueness/tampering, cross-actor cursor replay, and healthy/delayed/unavailable/crash-restarted SQLite projection consumers. It must show no 5xx, restricted metadata leak, duplicate/partial canonical create/publication, silent pagination duplicate/skip or starvation, fallback/dual-write, or extra advertised handle, with p99 under three seconds on declared hardware and topology. A forced full 400-candidate window records the bounded Postgres query count/plan shape, while outbox lag and replica convergence make downstream execution behavior falsifiable.

Final acceptance then uses a real browser-rendered chatbot through `https://tinyassets.io/mcp`: discover a branch without an ID, create a new complete public branch, inspect it, explicitly publish it, and rediscover it while preserving the exact seven-tool listing. It omits execution unless the named requester-authority gates have deployed; any admitted run proves requester-owned BYOC/market authority. None of the new catalog/create/publish modes can deploy while an exact Branch path can reveal restricted page or hidden Goal metadata. Post-fix organic use is checked separately or left as a STATUS watch.
...
- **[Risk] Catalog/create/publish IDs amplify exact-get wiki or hidden-Goal leaks.** → Hard-block all three new modes, deployment, prompts, and rendered inspection until every exact Branch path and shared helper provides a visibility-safe projection.
- **[Risk] Legacy SQLite bridge state is mistaken for hosted authority.** → Cut over once to Postgres, forbid dual-write/fallback, and adversarially test stale, forged, missing, and unavailable bridge rows.
...
2. Obtain accept/adapt from the current universe-creation, universe-visibility, shared-Goals, and `control_station` owners; require landed SHAs or explicit file releases rather than stale actor labels; preserve newborn-BYOC behavior from #1759; and provision/catalog `TINYASSETS_BRANCH_CRYPTO_KEYRING`. Preserve legacy publish, approval, and remix/lineage seams until their canonical replacements and rendered proofs land.
3. Land the existing `establish-postgres-control-plane` change from draft PR #1670 with its opposite-provider/host gates, production migration/role/RLS/recovery/stock-Postgres-exit substrate, packaged driver, first-write boundary, and this domain's allocated exact migration filename. This lane consumes that substrate rather than creating a parallel runner or credential authority.
4. Claim exact runtime/test files only after every owner releases them; write failing Postgres authority, bridge-adversary, visibility, idempotency, pagination, guidance, rollback, and load tests first.
5. Perform the one-way legacy import/cutover, then implement the canonical router, Postgres branch/catalog transaction owner, downstream SQLite projection consumer, keyring integration, and packaged-runtime parity.
6. Deploy through the normal pipeline only after the exact-branch projection is visibility-safe; rehearse the retained prior image, run the exact-seven canary, the rendered chatbot journey, and post-fix organic-use check.
7. After the owned `scope=commons` route and a separately reviewed serialized/locked wiki patch boundary exist, apply live corrections one page at a time with dry-run and hash-precondition evidence.
8. Reconcile the two changes' concurrent `Canonical Advertised Handle Set` modifications, then sync this delta together with the legacy-retirement delta without dropping either requirement set. Archive only after implementation and acceptance land. Before the first canonical Postgres write, an isolated prior-image rollback may restore the old authority. After one-way cutover and first write, the prior image stays traffic-isolated and recovery is forward-fix; it must never restore SQLite/shared mutation authority.
