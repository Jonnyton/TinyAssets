...
3. Under a separate reviewed OpenSpec baseline, establish the production Postgres migration/role/RLS/recovery/stock-Postgres-exit substrate and allocate this domain's exact migration filename. This lane consumes that substrate rather than creating a parallel runner or credential authority.
...
8. Reconcile the two changes' concurrent `Canonical Advertised Handle Set` modifications, then sync this delta together with the legacy-retirement delta without dropping either requirement set. Archive only after implementation and acceptance land. Rollback restores the prior image; any rollback reopens the onboarding, guidance, publication, and retirement gates.
