...
The system SHALL maintain a bounded catalog projection in the canonical branch database containing the allowed summary fields, internal verified-principal-derived `author_subject`, immutable `created_at`, mutable `updated_at`, and a candidate `publication_id` plus monotonic per-branch `publication_revision`. Branch create and patch SHALL update their projection row in the same database transaction as the branch mutation. Every branch-version write SHALL flow through a connection-accepting `publish_branch_version` chokepoint with an explicit closed intent of exactly `snapshot` or `catalog_publish`. `snapshot` MAY be used by patch, evaluation, selector, and service callers without a request principal, including default-selector bootstrap; it SHALL create or reuse only an operational `BranchVersion` and SHALL never create a `BranchCatalogPublication`, projection outbox event, or catalog eligibility. `catalog_publish` SHALL be accepted only after canonical publish mode verifies the current branch owner; service/environment principals and `UNIVERSE_SERVER_USER` MUST NOT supply that authority. That path SHALL create an active `BranchCatalogPublication` and emit a durable revisioned projection outbox event. Service catalog publication requires a later separately reviewed scoped-capability contract.
...
The version store SHALL preserve operational branch-version metadata and store deliberate catalog metadata separately in `branch_catalog_publications`. Existing operational `branch_versions.publisher`, `published_at`, notes, and parent metadata are untrusted for catalog authority because the legacy action accepted a caller-supplied publisher without branch-owner authorization; they MUST NOT establish branch ownership, deliberate publication, catalog eligibility, or attribution. A publication record SHALL contain unique `publication_id`, `branch_def_id`, `branch_version_id`, snapshot digest, owner-derived `catalog_publisher_subject`, `catalog_published_at`, monotonic per-branch `publication_revision`, and `status=active|superseded|rolled_back`. If `publish_branch_version` deduplicates to an existing operational snapshot, publication SHALL reference it without overwriting its operational publisher/time. A new deliberate publication SHALL supersede the prior active publication in the same transaction.
...
The system SHALL provide a deterministic declared-environment load fixture with 500 logical clients and 1,000 mixed operations: 750 catalog reads, 200 create attempts, and 50 publish attempts, including tied immutable timestamps, 100 concurrent same-key retries, unsupported-private probes, deliberate-versus-snapshot publication, cursor completion during unrelated mutations, repeated nonce uniqueness, altered cursor components, authoritative `status='active'` verification, and cross-actor cursor replay. The fixture SHALL use the production SQLite journaling and busy-timeout configuration on declared hardware. At `limit=100` it SHALL force a full 400-candidate window and record no more than 400 branch-version plus 400 publication-row verifications per request, together with version-store query count, batching, and cross-store topology. The proof MUST show zero 5xx responses, restricted metadata leaks, duplicate or partial branch/publication results, silent pagination duplicates/skips or restart starvation, repeated cursor nonces under a key, and advertised-handle drift, with p99 below three seconds in the declared environment.
