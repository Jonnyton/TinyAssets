...
#### Scenario: Goal identifiers are outside the V1 catalog contract
- **WHEN** an anonymous or authenticated caller supplies a non-default `goal_id` filter, or lists a public branch whose Goal is private, deleted, missing, or otherwise hidden
- **THEN** the filter returns `branch_read_mode_invalid` before catalog access
- **AND** projection, summary, cursor context, count, ordering, and cursor behavior disclose no Goal identifier or existence evidence
