...
A committed patch SHALL enqueue immutable pre-patch and post-patch snapshot events containing a UUIDv4 `event_id`, `branch_def_id`, canonical Branch revision, snapshot role exactly `pre|post`, canonical snapshot digest, and the bounded snapshot payload or canonical payload reference. The Postgres outbox SHALL enforce uniqueness for each logical `(event_id, snapshot_role)` delivery.
