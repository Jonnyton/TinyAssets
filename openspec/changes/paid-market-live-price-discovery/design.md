...
Existing demand, offer, claim, artifact, gate, and settlement primitives remain authoritative. A public `CapabilityDescriptor` classifies one atomic correlated supply tuple; its derived `descriptor_id` is exact validated supply-content identity, not price-index equivalence, demand identity, an offer, or authority. This explicitly adopts the landed quote/routing vocabulary: exact supply is `descriptor_id`, while index aggregation is `market_class_id`; the two identities are never aliases. An offer that can provide multiple model/runtime, device/topology, task/environment, fabrication/material, region, privacy, or reliability combinations carries multiple independently hashed descriptors. Every point and cross-field combination admitted by one descriptor's ranges and sets must be independently supportable at the same time; otherwise the offer must split those combinations into separate descriptors. V1 never accepts a caller-chosen `profile_id` and never contains `profiles[]`.
...
descriptor_id = "sha256:" + hashlib.sha256(canonical_bytes).hexdigest()
```
...
Every quote records quote identity, exact `descriptor_id`, derived `market_class_id`, demand commitment, `market_scope_revision`, canonical `public_scope_dimensions`, origin adapter, issuer, unit semantics, one settlement currency, priced-component coverage/missing fields, canonical fee schedule/version, landed monetary total, verified provider-eligibility facts, observation/issue time, expiry, and terms digest. An indicative quote is browseable and nonbinding. A native firm quote additionally binds authenticated issuer and tenant, nonce, signature domain, exact quantity, offer version, and an immutable domain capacity grant/fence.
...
Each price surface is keyed by `(market_class_id, market_scope_revision, public_scope_dimensions)`, not by exact supply `descriptor_id`. `market_scope_revision` is a globally immutable content-addressed projection contract that derives an allowlisted canonical ASCII object of coarse public dimensions from resolved quote/domain terms before execution, such as an execution-region/SLO bucket or an approved fabrication origin/destination/shipping-service bucket. The trusted scope projector sorts semantic sets and constructs `public_scope_dimensions` with the same bounded `json.dumps(... sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False)` method; the canonical bytes of that object are what records bind. Quotes, native asks, external references, and settlement observations bind the revision and derived object before publication or execution; settlement ingestion re-derives and revalidates them against accepted delivery/settlement evidence. The aggregator only groups identical bound keys and may not post-hoc bucket or rewrite scope.
...
- **[Risk] Public demand leaks sensitive work or this change silently decides the private-storage dispute.** → Match on the minimum descriptor/policy envelope and tenant-keyed digests. Private-data custody is OPEN RESEARCH across four modes—host machine, private universe brain, vault, and platform-held—and this change settles no default, hierarchy, or authorization model among them. It authorizes only opaque commitments and bounded market/evidence facts; it does not establish host-resident custody or platform storage of private payloads.
...
- Private-data custody remains OPEN RESEARCH across host machine, private universe brain, vault, and platform-held modes. This change authorizes only opaque commitments and bounded market/evidence facts and does not settle private-payload storage in any mode.
