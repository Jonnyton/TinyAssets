...
- **THEN** compatibility is evaluated without creating a new descriptor identity for every request
...
- **AND** matching does not manufacture a synthetic descriptor id or compatible tuple
...
#### Scenario: private demand does not enter the public descriptor id
...
- **THEN** the public descriptor id remains the same for the same validated descriptor metadata
...
- **THEN** validation returns `domain_validator_unavailable` or `domain_validation_failed` with no usable descriptor id
- **AND** no public or executable quote is emitted through a fallback validator

#### Scenario: caller direction and descriptor identity are refused
- **WHEN** input contains `profile_id`, `descriptor_id`, `profiles`, `direction`, or range-inclusivity fields
- **THEN** validation returns `unknown_field` with a non-echoing safe path
- **AND** the library derives no descriptor id from the caller-controlled shape
...
- **WHEN** two different descriptor ids both satisfy the same normalized public demand requirements but expose different extra set members or range headroom
...
- **WHEN** any quote identity, descriptor id, market class id, scope revision/dimensions, descriptor, demand, unit, price, fee, total, currency, eligibility, terms, quantity, tenant, issue/expiry, offer-version, or capacity field changes after signing
...
The live price service SHALL key every public aggregate by `(market_class_id, market_scope_revision, public_scope_dimensions)`, never exact supply `descriptor_id`. `market_scope_revision` SHALL be a globally immutable content-addressed projection contract that derives one bounded canonical ASCII object of allowlisted coarse public dimensions from resolved quote/domain terms before execution, such as an execution-region/SLO bucket or approved fabrication origin/destination/shipping-service bucket. The trusted scope projector SHALL sort semantic sets and construct the dimensions using the same bounded `json.dumps(..., sort_keys=True, separators=(",", ":"), ensure_ascii=True, allow_nan=False).encode("ascii")` method; those canonical object bytes are what records bind. Every quote, native ask, external reference, and settlement observation SHALL bind the revision and canonical dimensions before publication or execution; settlement ingestion SHALL re-derive and revalidate them against accepted evidence. The aggregator SHALL only group identical bound keys and SHALL NOT create or rewrite a post-hoc bucket.
...
- **WHEN** different exact descriptor ids satisfy the same market class and bind the same scope revision/dimensions
- **THEN** their eligible observations may contribute to the same public aggregate
- **AND** the aggregate still retains each source's exact descriptor id as evidence
