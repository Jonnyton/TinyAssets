...
REQUIRED_BACKGROUND_ROOTS: Mapping[str, tuple[SourceReference, ...]] = {
    "schedule_and_event": (
        SourceReference("tinyassets/scheduler.py", "def _maybe_fire_schedule("),
        SourceReference("tinyassets/scheduler.py", "def _dispatch_event("),
    ),
    "goal_subscription": (
    ...
    "soul_and_compiled_cycle": (
        SourceReference(
            "fantasy_daemon/__main__.py", "def _resolve_loop_daemon_context("
        ),
        SourceReference(
            "fantasy_daemon/branches/universe_cycle.yaml", "graph_nodes:"
        ),
    ),
    "request_admission": (
        SourceReference(
            "tinyassets/storage/request_admissions.py", "class RequestAdmissionStore:"
        ),
    ...
    "branch_task_and_graph_enqueue": (
        ...
        SourceReference(
            "tinyassets/graph_compiler.py", "def _node_enqueue_branch_run("
        ),
    ),
    ...
    "resume_and_recovery": (
        ...
        SourceReference(
            "tinyassets/branch_tasks_v2.py", "    def recover_expired("
        ),
    ),
    "request_actor_boundary": (
        SourceReference(
            "tinyassets/api/permissions.py", "def current_request_actor_id("
        ),
        SourceReference("tinyassets/api/engine_helpers.py", "def _current_actor("),
    ),
    "daemon_cloud_and_distributed_worker": (
        SourceReference(
            "fantasy_daemon/__main__.py", "def _try_execute_claimed_branch_task("
        ),
        SourceReference("tinyassets/cloud_worker.py", "def run_supervisor("),
        SourceReference(
            "tinyassets/execution_authority/records.py", "class ExecutionGrantV1:"
        ),
    ),
    "selector_leaderboard_and_market_delegate": (
        SourceReference(
            "tinyassets/api/selector_dispatch.py", "def dispatch_selector("
        ),
        SourceReference(
            "tinyassets/api/quality_leaderboard.py",
        ...
        SourceReference(
            "tinyassets/api/market.py", "def _action_goal_run_canonical("
        ),
    ),
...
CANONICAL_READ_INTERFACES: Mapping[str, tuple[SourceReference, ...]] = {
    ...
    "acl": (
        SourceReference(
            "tinyassets/api/permissions.py", "def universe_access_allows("
        ),
    ),
    "branch": (
    ...
    "daemon": (
        SourceReference("tinyassets/daemon_registry.py", "def daemon_control_status("),
    ),
    "run": (
    ...
    "goal_subscription": (
        ...
        SourceReference(
            "tinyassets/scheduler.py", "def list_scheduler_subscriptions("
        ),
    ),
    ...
    "queue": (
        ...
        SourceReference(
            "tinyassets/branch_tasks_v2.py", "class Epoch2OperationalRead:"
        ),
    ),
    "b2": (
        SourceReference(
    ...
    "provider_attempt": (
        SourceReference(
            "openspec/changes/provider-attempt-receipts/specs/"
            "provider-routing/spec.md",
...
_SCAN_ROOTS = ("tinyassets", "fantasy_daemon")
...
class _CallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if callee in self._names:
            self.calls.add((".".join(self._scope) or "<module>", callee))
        self.generic_visit(node)
...
def scan_python_calls(
    ...
    *,
    ...
    return {
        CallSite(path=relative, function=function, callee=callee)
        for function, callee in visitor.calls
    }


def collect_sensitive_call_sites(repo_root: Path) -> set[CallSite]:
...
def validate_inventory(repo_root: Path) -> list[str]:
    ...
    expected = set(EXPECTED_SENSITIVE_CALL_SITES)
    for callsite in sorted(observed - expected):
        errors.append(f"unreviewed sensitive callsite: {callsite}")
    for callsite in sorted(expected - observed):
        errors.append(f"stale sensitive callsite: {callsite}")
    ...
    for owner, ref in _all_references():
        ...
        if ref.marker not in text:
            errors.append(
                f"{owner} missing marker in {ref.path}: {ref.marker!r}"
            )
...
def _format_calls(calls: Sequence[CallSite]) -> str:
    return "\n".join(
        f"{call.path}::{call.function} -> {call.callee}" for call in sorted(calls)
    )
