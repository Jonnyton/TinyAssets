...
stranded until the record is migrated out. This script is the runnable, IDEMPOTENT
remediation the host runs as a deploy step.
...
Modes (exactly one):
  ...
  --migrate     Quarantine + remove the llm_subscription records (idempotent).
                Archives them to ``.credential-vault-quarantine.json`` per universe.
...
Examples:
  python scripts/migrate_retired_subscription_records.py --inventory
  python scripts/migrate_retired_subscription_records.py --migrate
  python scripts/migrate_retired_subscription_records.py --rollback --universe u-01k...
"""
