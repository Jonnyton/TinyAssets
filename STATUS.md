...
- **[P1 filed:2026-07-02 verified:2026-07-02] MERGE BLOCKER (Codex do-not-merge, runtime-confirmed):** PR #1437's per-universe ACL gate (`permissions.universe_access_allows`, provider-independent, no dev bypass) DENIES all anonymous writes — incl. via public `write_graph`/`write_page`. Live prod deploys `UNIVERSE_SERVER_AUTH=optional` (deploy-prod.yml:571) → anon-open writes work TODAY; merging as-is = live WRITE OUTAGE (optional mode has no founder-auth path). Safe cutover REQUIRES coordinated flip to `UNIVERSE_SERVER_AUTH=workos` + WorkOS secrets in the Droplet/GH Actions (host-only) SAME as the merge. host-decision: accept anon-write cutover + provide WorkOS live values.
