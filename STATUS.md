...
| Register the ChatGPT connector as `TinyAssets` (never environment suffixes in the user-facing name) against `https://tinyassets.io/mcp-directory`; verify exactly 5 handles, then retire stale `/mcp` registration | OpenAI workspace admin | - | host-action |
