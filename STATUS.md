...
| **R2-1a set_engine/credential ceiling** — exact provider binding; incomplete sources empty; explicit-universe env strips ambient auth and vault errors fail closed | tinyassets/api/universe.py, tinyassets/config.py, tinyassets/credential_vault.py, tinyassets/providers/base.py, tinyassets/providers/router.py, tests/test_{s2_engine_assignment,s6_s8_engine_sources,credential_fail_closed,provider_allowlist,per_universe_engine_resolution}.py, packaging/claude-plugin/plugins/tinyassets-universe-server/runtime/tinyassets/{api/universe.py,config.py,credential_vault.py,providers/base.py,providers/router.py}, openspec/changes/constrain-founder-provider-routing/, openspec/changes/universe-creation/{design.md,tasks.md}, STATUS.md, .agents/worktrees.md | generic subprocess isolation prerequisite only; request authority/run_graph stays #1582; R2-1b receipts waits | claimed:codex-gpt5-desktop-2 ACTIVE 2026-07-22 |
