...
| Mobile app download wiring — home/start buttons live-check GitHub `android-latest` release (see `docs/reference/mobile-app-release-convention.md`); CI in `release-android.yml` is dormant until `clients/android` lands on main | WebSite/site/src/lib/{mcp/appRelease.ts,components/AppDownload.svelte}, WebSite/site/src/routes/{+page.svelte,start/+page.svelte}, .github/workflows/release-android.yml | mobile app scaffold branch (`wf-ios-app-scaffold`) merging `clients/android` to main | dev-ready |
