...
| **Adapt connector manifests for canonical `/mcp` and near-term `/mcp-directory` retirement** — rewrite three-product contract to remote `/mcp` + local MCPB; planning only | openspec/changes/reconcile-external-connector-manifests/ | #1684 cleanup; Claude opposite-provider research review before runtime | in-flight PR #1688 |
