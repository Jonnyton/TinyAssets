...
| **Fix external-directory catalog drift** — checked-in registry URL is live-404; regenerate from connector catalog and enforce `--check` in packaging CI | openspec/changes/fix-external-directory-catalog-drift/, packaging/registry/server.json, packaging/registry/generate_server_json.py, tests/test_connector_catalog.py, .github/workflows/build-bundle.yml | live current catalog 200; PR #1335 audit | claimed:codex-gpt5-desktop ACTIVE 2026-07-22 |
| External directory acceptance — canaries green 2026-05-02; needs clean ChatGPT/Claude proof + first-user evidence | docs/ops/mcp-* | catalog drift fix | host-action |
