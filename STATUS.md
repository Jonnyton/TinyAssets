...
| OpenAI app submission hardening — `/mcp-directory` now has 5 reviewed handles; retire stale 11-tool copy, reduce tests from 9+4 to required 5+3, serve domain challenge, refresh proof | chatgpt-app-submission.json, docs/ops/openai-app-submission-*.md, domain challenge route | `TinyAssets DEV` re-registration + clean ChatGPT proof | dev-ready |
...
| Re-register `TinyAssets DEV` ChatGPT connector as workspace admin against `https://tinyassets.io/mcp-directory`; verify exactly 5 handles, then retire stale `/mcp` registration | OpenAI workspace admin | - | host-action |
