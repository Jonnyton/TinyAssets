...
| **Retire `/mcp-directory*`** — remove runtime/catalog/discovery/edge routing so only canonical `/mcp` survives; exact ordinary-404 proof | openspec/changes/reconcile-external-connector-manifests/tasks.md; tinyassets/{universe_server.py,directory_server.py,connector_catalog.py,auth/middleware.py}; deploy/cloudflare-worker/{worker.js,worker.test.js}; packaging/{registry/,claude-plugin/plugins/tinyassets-universe-server/runtime/tinyassets/}; focused route/parity/admission tests | PR #1688; maintained guidance write-set will be collision-checked before broadening | claimed:codex-gpt56-mcp-retirement ACTIVE 2026-07-24 |
