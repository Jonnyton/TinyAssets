...
| **Split connector-manifest product contracts** — #1597 correctly shares seven names but conflates remote WorkOS `/mcp` with local MCPB stdio/auth-config; keep directory remote-five separate | openspec/changes/reconcile-external-connector-manifests/ | #1597; blocks legacy retirement | pending |
