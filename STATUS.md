...
|------|-------|---------|--------|
| **P0 onboarding: separate agent coordination from wiki discovery + restore branch-authoring schema/path** | `tinyassets/api/wiki.py`, `tinyassets/universe_server.py`, `tinyassets/directory_server.py`, `tinyassets/wiki/`, plugin mirrors, `tests/test_wiki_tools.py`, `tests/test_universe_server_five_handles.py`, `tests/test_directory_server.py`, `tests/test_api_wiki.py`, `openspec/changes/separate-wiki-discovery-and-restore-branch-authoring/`, `docs/audits/2026-07-21-wiki-discovery-contamination.md`, `REFLECTION.md` | publish blocked: shell network denied; GitHub branch mutation cancelled twice. After deploy: rendered `ui-test` + organic-use watch | host-action: approve GitHub publish |
