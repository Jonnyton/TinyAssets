...
|------|-------|---------|--------|
| **Fail closed Agent Village LAN access** — protect session/state APIs, CSRF-sensitive writes, and provider-budget dispatch without query-string secrets | command_center/__main__.py; command_center/collector.py; command_center/server.py; command_center/web/app.js; command_center/README.md; scripts/peer_agent.py; tests/command_center/test_server.py; tests/command_center/test_hire.py; tests/command_center/test_cli.py; tests/test_peer_agent.py; openspec/changes/harden-command-center-lan-auth/; openspec/specs/development-coordination-runtime/spec.md; STATUS.md; .agents/worktrees.md; REFLECTION.md | #1489 landed; draft PR #1657; blocker: quota reservation leaks on inbox/thread-start failure | claimed:codex-gpt5-desktop-lan ACTIVE 2026-07-23 |
