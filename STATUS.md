...
- **[P1 filed:2026-07-02 verified:2026-07-02] MERGE of PR #1437 BLOCKED on host WorkOS secret (coordinated cutover, host-approved).** #1437's per-universe ACL gate denies ALL anon writes regardless of provider mode (Codex do-not-merge, runtime-confirmed), so merging without WorkOS live = write outage. Deploy cutover PREPPED + CI-green: `deploy-prod.yml` flips `optional→workos` (+WORKOS_MCP_RESOURCE fail-closed audience, WORKOS_REQUIRE_AUTH=1 force-login per host) ONLY when the `WORKOS_AUTHKIT_DOMAIN` GH Actions secret is present. **host-action to unblock merge:** (1) add GH secret `WORKOS_AUTHKIT_DOMAIN`=<live AuthKit domain>; (2) register `https://tinyassets.io/mcp` as a WorkOS Resource Indicator + confirm DCR/claude.ai redirect. Then Claude merges → deploy → canary + chatbot ui-test.
