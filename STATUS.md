...
|------|-------|---------|--------|
| **Patch-loop S1 r26** — malformed-dedup reconciliation + atomic wiki winner + legacy provenance; bundled deploy only (S1→S3→S4→S2→S5) | tinyassets/{branch_tasks,bug_investigation}.py, tinyassets/api/wiki.py, tinyassets/wiki/trigger_receipts.py, plugin mirror, tests/test_{bug_investigation_wiring,wiki_trigger_receipts}.py | S3 enforcement + Phase-2 runner; S4 owner review + github_merge | host-review |
...
5. **Spec-driven development is the standard (host 2026-07-19)** — every substantive change starts as an OpenSpec change; as-built specs live in `openspec/specs/` (AGENTS.md § Spec-driven development).
