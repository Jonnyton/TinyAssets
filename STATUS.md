...
| **Repair and prove shared in-node enqueue caps** — production is enabled; proof exposed unstable root origins and GC-forgotten lineage history | openspec/changes/prove-node-enqueue-shared-caps/; openspec/specs/{graph-execution-substrate,daemon-runtime-and-dispatch}/spec.md; tinyassets/{runs.py,branch_tasks.py,graph_compiler.py}; packaging/claude-plugin/plugins/tinyassets-universe-server/runtime/tinyassets/{runs.py,branch_tasks.py,graph_compiler.py}; tests/test_node_enqueue_{concurrency,verb}.py; tests/test_dispatcher_queue.py; STATUS.md; .agents/worktrees.md; REFLECTION.md | round-3 APPROVE was for pre-proof shape; new root/GC corrections require independent review | claimed:codex-gpt5-desktop-enqueue ACTIVE 2026-07-23 |
