...
| **Repair and prove shared in-node enqueue caps** — draft PR #1672; security ADAPT adds run-wide budget, physical-universe authority, public-only v1 targets, strict blank history | openspec/changes/prove-node-enqueue-shared-caps/; openspec/specs/{graph-execution-substrate,daemon-runtime-and-dispatch}/spec.md; tinyassets/{runs.py,branch_tasks.py,graph_compiler.py}; fantasy_daemon/__main__.py; packaging/claude-plugin/plugins/tinyassets-universe-server/runtime/tinyassets/{runs.py,branch_tasks.py,graph_compiler.py}; tests/test_node_enqueue_{concurrency,verb}.py; tests/test_{dispatcher_queue,bug_investigation_dispatcher}.py; STATUS.md; .agents/worktrees.md; REFLECTION.md | concurrency APPROVE; security ADAPT at 9488ec28; epoch-2 transactional equivalents remain an explicit foldback gate | claimed:codex-gpt5-desktop-enqueue ACTIVE 2026-07-23 |
