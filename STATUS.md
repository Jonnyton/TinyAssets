...
| **Backfill graph-run coordination contracts** — shipped run evidence receipts + local teammate mailbox missed by full-coverage audit | openspec/changes/backfill-graph-run-coordination-contracts/; openspec/specs/graph-execution-substrate/spec.md | read active `distributed-execution`; preserve local/no-truth-rank/auth limitations | claimed:codex-gpt56-spec-5 ACTIVE 2026-07-23 |
