...
| **Adapt connector-manifest reconciliation; retire `/mcp-directory` after canonical `/mcp` is review-safe** — two products only: remote `/mcp` + local MCPB; migrate every host/catalog, then remove old route | openspec/changes/reconcile-external-connector-manifests/; openspec/specs/live-mcp-connector-surface/spec.md; tinyassets/{directory_server.py,universe_server.py,connector_catalog.py}; deploy/cloudflare-worker/{worker.js,worker.test.js}; packaging/registry/{server.json,generate_server_json.py}; chatgpt-app-submission.json; docs/ops/mcp-*; tests/ | host directive 2026-07-24; `/mcp` status redaction, neutral instructions, OAuth securitySchemes/challenges, accurate hints/descriptions, privacy policy; rendered ChatGPT/Claude + client migration proof | pending |
...
| External directory acceptance — migrate registry/hosts to canonical `/mcp`; then needs clean ChatGPT/Claude proof + first-user evidence | docs/ops/mcp-* | adapted reconcile-external-connector-manifests | host-action |
| OpenAI app submission hardening — target canonical `/mcp`; update seven-handle tests/copy, OAuth metadata, hints, privacy disclosure, domain challenge, and proof | chatgpt-app-submission.json, docs/ops/openai-app-submission-*.md, deploy/cloudflare-worker/{worker.js,worker.test.js}, WebSite/site-react/app/legal/page.tsx | adapted reconcile-external-connector-manifests; `TinyAssets` re-registration + clean ChatGPT proof | dev-ready |
...
| Register the ChatGPT connector as `TinyAssets` against canonical `https://tinyassets.io/mcp`; verify exactly 7 handles | OpenAI workspace admin | adapted reconcile-external-connector-manifests + canonical `/mcp` review-safety proof | host-action |
