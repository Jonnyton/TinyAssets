...
|------|-------|---------|--------|
| **#1699 distributed-execution current-main design + B2 extraction manifest** — complete OpenSpec design; preserve epoch-2 scheduling while keeping admission/queue claims distinct from signed execution authority; no live route/provider/credential activation | STATUS.md; .agents/worktrees.md; openspec/changes/distributed-execution/{proposal.md,design.md,tasks.md,specs/distributed-execution/spec.md} | #1697; PLAN/epoch-2 reconciliation before claim integration; opposite-provider review before live apply | claimed:codex-gpt5-desktop ACTIVE 2026-07-24 |
...
| **Epoch-2 transactional quarantine maintenance** — classify invalid rows separately; atomic sanitized receipt+disable; selection stays pure; failure is red/inert | STATUS.md; tinyassets/{branch_tasks_v2.py,storage/request_admissions.py}; packaging/claude-plugin/plugins/tinyassets-universe-server/runtime/tinyassets/{branch_tasks_v2.py,storage/request_admissions.py}; tests/{test_branch_tasks_v2.py,test_request_admission_store.py,test_dispatcher_queue.py}; openspec/changes/operator-request-trigger-contract/tasks.md; REFLECTION.md | #1697; OpenSpec task 6.5 | claimed:codex-gpt5-desktop-operator ACTIVE 2026-07-24 |
