...
|------|-------|---------|--------|
| **Exercise converged host uptime jobs** — #1677 added bounded retry; run 30071110351 rolled back before it because S3 no-op `mkdir` made the first request. Remove mkdir so list retry is the sole gate, then prove backup | .github/workflows/{deploy-prod,install-host-services}.yml; tests/{test_deploy_prod_workflow,test_host_uptime_installers}.py; openspec/changes/preserve-production-backup-config/; openspec/specs/uptime-and-alarms/spec.md; docs/ops/backup-restore-runbook.md; docs/audits/2026-07-23-host-uptime-installer-concurrency-proof.md; STATUS.md; production workflow/systemd evidence | exact-SHA production backup/restore evidence | claimed:codex-gpt5-desktop ACTIVE 2026-07-23 |
