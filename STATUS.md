...
- [filed:2026-07-02 verified:2026-07-02] Multi-tenant isolation PROVEN deterministically (`tests/test_multi_tenant_isolation.py`, 5 tests, Codex ADAPT folded — `docs/audits/2026-07-02-multi-tenant-isolation-proof-codex-review.md`): N concurrent founders → distinct universes, correct home/ACL binding, cross-founder write/private-read denial, no ContextVar bleed, cold-start-race safe.
- **[P1 filed:2026-07-02 verified:2026-07-02] MERGE of PR #1437 BLOCKED on branch full-suite test debt.** WorkOS coordinated-cutover config now DONE (Claude via browser 2026-07-02): AuthKit domain `inventive-van-62-staging.authkit.app` confirmed live (issuer+JWKS+DCR-enabled), GH secret `WORKOS_AUTHKIT_DOMAIN` set, MCP resource indicator `https://tinyassets.io/mcp` registered; `deploy-prod.yml` flips optional→workos gated on that secret (+fail-closed audience, WORKOS_REQUIRE_AUTH=1). REMAINING blocker: full single-process suite = 167 failed/8452 passed (CI never runs full suite, so this was hidden behind green focused runs). Some are BRANCH-INTRODUCED (rename `_default_universe`→`_request_universe` orphaned `test_get_recent_events` monkeypatch, etc.), some pre-existing/env (browser/worktree/skills, known Windows/clean-clone rows). Triage (branch-vs-main diff) in flight; fix branch-introduced before merge.
