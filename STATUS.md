...
- [filed:2026-07-01 verified:2026-07-01] founder-identity all-slices: get_status first-contact now scope-gated (read-only founder can't birth; not a scope bypass) + get_status/list_run_receipts(all modes)/get_memory_scope_status now ACL-gated for private universes. Residual (pre-live): missing-token protected WRITES return tool JSON error, not an HTTP 401 WWW-Authenticate challenge — required before WorkOS live rollout.
