...
|------|-------|---------|--------|
| WIKI-PATCH #321 continuity-audit primitive — add read-only author-facing audit over prose fragments, canon, and wiki constraints. | `workflow/api/universe.py`, `workflow/universe_server.py`, `workflow/api/prompts.py`, `tests/test_universe_server_continuity_audit.py`, `tests/test_universe_server_ledger.py`, `tests/test_universe_server_metadata.py`, `packaging/claude-plugin/plugins/workflow-universe-server/runtime/`, `_PURPOSE.md`, `.agents/worktrees.md` | opposite-family checker after code | claimed:codex-wiki-patch |
