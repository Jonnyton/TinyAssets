...
|------|-------|---------|--------|
| Issue #892 WIKI-PATCH — privileged-node primitive: node-level `host_controlled` flag with host-private live execution and public fork/amend surface. | `workflow/branches.py`, `workflow/api/branches.py`, `workflow/graph_compiler.py`, `tests/test_branches.py`, `tests/test_branch_authoring_actions.py`, `tests/test_graph_compiler_privileged_node.py` | opposite-family checker before commit | claimed:codex-wiki-patch ACTIVE 2026-05-17 |
