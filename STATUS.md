...
- **[P1 filed:2026-06-30]** WorkOS PRODUCTION auth enforcement gap (Codex slice-4 review `docs/audits/2026-06-30-founder-identity-slice4-codex-review.md`): `WorkOSAuthProvider.is_auth_required()` returns False → anonymous WorkOS-mode caller can create/write (founder_id ""); WorkOS discovery not wired (PRM advertises self as AS, well-known 404). Needs resolve-always mode + WorkOS-aware well-known before WorkOS goes live. Auth-slice scope, not a slice-4 regression.
