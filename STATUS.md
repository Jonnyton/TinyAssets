...
| Paid-market Track E Wave 2: cheapest adequate executable Internet route is the top-line reference; native/connected supply competes below it by supply/demand; fee on every settlement. Reconcile stale external-pass-through no-fee text; renumber migrations + add schema_migrations before 006–008 go live | openspec/, tinyassets/paid_market/ | - | pending |
...
| **Founder-account setup surface** — founder recognition is built+deployed (PR #2348) but unprovable live: `AppPrincipalMappingService.provision` has NO user-facing caller, so no founder mapping can exist. Host's shape: "editing your custom agent, list the founder accounts per platform" | tinyassets/app_principal_mapping.py, universe_server.py | PR #2348 | pending |
| **`resolve_interlocutor_tier` grants T2/FOUNDER to any `write` ACL holder** (`api/interlocutor.py:130`) — on the MCP path a collaborator reads `founder.md` + commits learning. Codex-found 2026-08-05, outside PR #2348's diff | tinyassets/api/interlocutor.py | - | pending |
