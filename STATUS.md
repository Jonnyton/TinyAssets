...
|------|-------|---------|--------|
| **Publish terminal deploy/rollback truth** — every post-mutation terminal path records attempted/active digest and honest rollback outcome; failure issue text is conditional | .github/workflows/deploy-prod.yml; tests/test_deploy_prod_workflow.py; openspec/changes/publish-terminal-deploy-rollback-receipt/; STATUS.md; .agents/worktrees.md | disk-remediation lane releases canonical uptime spec before sync; stale-primary broad `tests/` overlap absent on origin/main | claimed:codex-gpt5-desktop-rollback ACTIVE 2026-07-23 |
