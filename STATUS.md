...
| **R2-1a set_engine constrains allowed_providers** — BYO key must bind its matching provider; mismatch/failure cannot fall through to host/provider quota | tinyassets/api/universe.py, tinyassets/providers/router.py, tests/test_s2_engine_assignment.py, tests/test_provider_allowlist.py, openspec/changes/constrain-founder-provider-routing/, STATUS.md, .agents/worktrees.md | universe-creation authority resolver remains separate | claimed:codex-gpt5-desktop-2 ACTIVE 2026-07-22 |
