...
class EffectChain:
    ...
    def revoke_workspace(self, node_key: str) -> Any:
        """Drop the capability AND close its descriptors: a later ``ws`` node
        in this run refuses. Idempotent; returns what was dropped (None if
        nothing).

        Closing here is what makes the lease reclaimable -- a revoked mount
        whose handles stay open pins the directory the outbox is trying to
        wipe.
        """
        with self.lock:
    ...
    def close_workspaces(self) -> None:
        """Close every mount this run still holds. Called at settle: a run that
        ended without an explicit discard must not leak its descriptors."""
        with self.lock:
