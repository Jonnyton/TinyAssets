...
cannot supply its body) — see tinyassets/effectors/github_read.py.
"""
...
def _syntax_error(path: str, content: str) -> str:
    ...
    (the patch-loop targets Jonnyton/TinyAssets on the same 3.11+ runtime). A repo
...
def validate_patch(state: dict) -> dict:
    ...
    from tinyassets.effectors.github_pr import _apply_edit_blocks, _parse_packet
...
def register_validate_patch() -> None:
    ...
    Idempotent. Called from tinyassets/effectors/__init__.py at import and lazily
