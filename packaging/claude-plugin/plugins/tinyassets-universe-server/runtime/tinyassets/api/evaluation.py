"""Evaluation subsystem — extracted from tinyassets/universe_server.py
...
Same pattern as Task #11 (tinyassets/api/runs.py) used for its 7 cross-module
symbols.
...
def _action_judge_run(kwargs: dict[str, Any]) -> str:
    ...
    from tinyassets.api.runs import _branch_name_for_run
    from tinyassets.runs import add_judgment
...
def _action_list_judgments(kwargs: dict[str, Any]) -> str:
    from tinyassets.runs import list_judgments as _list_judgments
...
def _action_get_node_output(kwargs: dict[str, Any]) -> str:
    ...
    from tinyassets.api.runs import _branch_name_for_run
    from tinyassets.runs import get_run as _get_run
    from tinyassets.runs import node_output_from_run
...
def _action_rollback_node(kwargs: dict[str, Any]) -> str:
    ...
    from tinyassets.api.branches import _reconcile_node_approval
