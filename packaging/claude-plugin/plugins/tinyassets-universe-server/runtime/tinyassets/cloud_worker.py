...
Motivation
----------
Today's droplet container runs ``python -m tinyassets.universe_server`` which
...
Why supervise a subprocess instead of re-implementing the claim loop?
  ...
  - The subprocess inherits ``/etc/tinyassets/env`` (via compose env_file)
    after project-wide provider auth policy is applied. API-key provider
    env vars are stripped unless ``TINYASSETS_ALLOW_API_KEY_PROVIDERS=1``.
...
Concurrency safety
------------------
``tinyassets.branch_tasks.claim_task`` is file-locked via a sidecar .lock
file (see ``tinyassets/branch_tasks.py:_file_lock``). Two fantasy_daemon
...
``TINYASSETS_ALLOW_API_KEY_PROVIDERS=1`` for this daemon. Optionally, host can
set ``FANTASY_DAEMON_LLM_TYPES`` or ``TINYASSETS_PIN_WRITER`` on the cloud side
...
supervisor and applies naturally via ``/etc/tinyassets/env``.
...
def _resolve_universe_path() -> Path:
    ...
    Precedence (first non-empty wins):
      1. ``TINYASSETS_UNIVERSE`` env var (explicit override).
      2. ``$TINYASSETS_DATA_DIR/$UNIVERSE_SERVER_DEFAULT_UNIVERSE``.
      3. First directory under ``TINYASSETS_DATA_DIR`` that has a
         ``PROGRAM.md`` or ``soul.md`` (auto-selects the active universe in the common
         "one universe" droplet deployment).
      4. ``$TINYASSETS_DATA_DIR/default-universe`` as a last-resort fallback.
    """
    explicit = os.environ.get("TINYASSETS_UNIVERSE", "").strip()
    if explicit:
...
def _build_subprocess_env(universe: Path | None = None) -> dict[str, str]:
    ...
    subprocess launch.
    """
    from tinyassets.providers.base import subprocess_env_without_api_keys

    env = subprocess_env_without_api_keys() or dict(os.environ)
    env["UNIVERSE_SERVER_HOST_USER"] = _cloud_host_user()
    if universe is not None:
        env["TINYASSETS_UNIVERSE"] = str(universe)
    ...
    env.setdefault("TINYASSETS_UNIFIED_EXECUTION", "1")
    return env
...
def _soul_loop_declared_for_universe(universe: Path) -> bool:
    ...
    if not _truthy_env("TINYASSETS_SOUL_LOOP_DISPATCH"):
        return False
    try:
...
def _register_branch_task_producers_from_env() -> None:
    ...
    """
    if _truthy_env("TINYASSETS_GOAL_POOL"):
        from tinyassets.producers.goal_pool import register_if_enabled

        register_if_enabled()
    if _truthy_env("TINYASSETS_PAID_MARKET"):
        from tinyassets.producers.node_bid import register_if_enabled
...
def _pump_branch_task_producers(universe: Path) -> int:
    ...
    """
    if not (_truthy_env("TINYASSETS_GOAL_POOL") or _truthy_env("TINYASSETS_PAID_MARKET")):
        return 0
    try:
...
# consumers (tinyassets.api.universe worker_liveness, the activity canary,
# tinyassets.cloud_worker_healthcheck) read it to distinguish "worker wedged"
...
def _release_own_orphaned_leases(universe: Path) -> int:
    ...
    if not worker_id or worker_id == DEFAULT_HOST_USER:
        return 0
    try:
        from tinyassets.branch_tasks import reclaim_predecessor_tasks
