...
def _resolve_db_path() -> Path:
    ...
    Honors ``TINYASSETS_TRIGGER_RECEIPTS_DB`` env override (test-friendly),
    otherwise places the file under ``TINYASSETS_DATA_DIR`` (resolved by
    ``tinyassets.storage.data_dir()`` when available, else env fallback).
    """
    override = os.environ.get("TINYASSETS_TRIGGER_RECEIPTS_DB", "").strip()
    if override:
    ...
    try:
        ...
        base = Path(os.environ.get("TINYASSETS_DATA_DIR", str(Path.home() / ".tinyassets")))
