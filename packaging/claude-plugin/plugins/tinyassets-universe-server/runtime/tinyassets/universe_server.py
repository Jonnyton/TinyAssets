...
async def _slack_app_events(request):  # type: ignore[no-untyped-def]
    ...
    # retries anything it does not hear back from within 3 seconds and an agent
    # turn is far slower, so coupling them would make every conversation a retry
    # storm — each retry costing another HMAC.
    #
    # Only a freshly admitted event dispatches. A replay must not answer twice,
    # which is what the dedup ledger in front of this is for.
...
def _dispatch_slack_reply(raw_body: bytes, headers) -> None:
    ...
    re-deriving it here keeps that true instead of adding a second way to obtain
    one. The HMAC is cheap next to the provider call it precedes.
    """
