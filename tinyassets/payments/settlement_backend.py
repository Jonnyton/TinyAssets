...
accumulated balance OUT via a backend, selected by ``TINYASSETS_SETTLEMENT_BACKEND``:
...
def settlement_backend_name() -> str:
    """Read ``TINYASSETS_SETTLEMENT_BACKEND``. Default 'internal'."""
    return (os.environ.get("TINYASSETS_SETTLEMENT_BACKEND") or "internal").strip().lower()
