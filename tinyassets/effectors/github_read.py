...
Build decisions from review (design note §"Build decisions from review"):
  1. Read scope is SEPARATE from write scope — resolve a read token from
     ``TINYASSETS_GITHUB_READ_CAPABILITIES`` (NOT the write map). When no token is
...
def _read_request(
    *, destination: str, path: str, token: str
    ...
    headers = {
        ...
        "User-Agent": "tinyassets-github-read/1.0",
...
def read_repo_files(state: dict) -> dict:
    ...
    max_files = _int_env("TINYASSETS_GITHUB_READ_MAX_FILES", _DEFAULT_MAX_FILES)
    max_bytes_file = _int_env(
        "TINYASSETS_GITHUB_READ_MAX_BYTES_PER_FILE", _DEFAULT_MAX_BYTES_PER_FILE
    )
    max_total = _int_env(
        "TINYASSETS_GITHUB_READ_MAX_TOTAL_BYTES", _DEFAULT_MAX_TOTAL_BYTES
    )
...
def register_read_repo_files() -> None:
    ...
    log). Called from tinyassets/effectors/__init__.py at import and lazily from
