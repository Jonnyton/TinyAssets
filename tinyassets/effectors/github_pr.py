...
.. code-block:: json

    {
      ...
      "destination": "Jonnyton/TinyAssets",       # Phase 2 — required for real writes
      "payload": {
        "title": "...",
        "body":  "...",
        "base_branch": "main",
...
1. **Capability token (vault first, then secrets-vended env).** The
   ...
   ``TINYASSETS_GITHUB_PUSH_CAPABILITIES`` (a JSON map of
   ``{"<owner>/<repo>": "<token>"}``), accepting the older
   ``TINYASSETS_GITHUB_PR_CAPABILITIES`` map as a legacy fallback during
   migration. The JSON map keys by the literal ``owner/repo`` destination
   (round-2 fix for Codex P1.2 — the round-1
   ``TINYASSETS_GITHUB_PR_CAPABILITY_REPO_<OWNER>_<REPO>`` suffix encoding
...
3. **Idempotency receipt — atomic reservation.** Round-2 fix for
   ...
   ``tinyassets.storage.external_write_receipts`` — the reservation is
...
# auth/secrets provider (``tinyassets.auth.provider.vend_github_destination_secret``)
# as a destination-keyed ``push`` capability. The canonical env map is
# ``TINYASSETS_GITHUB_PUSH_CAPABILITIES``; the older PR-specific map
# ``TINYASSETS_GITHUB_PR_CAPABILITIES`` is still accepted as a legacy
...
_PUSH_CAPABILITIES_ENV = "TINYASSETS_GITHUB_PUSH_CAPABILITIES"
...
def _vend_push_token(destination: str) -> str:
    ...
    canonical ``TINYASSETS_GITHUB_PUSH_CAPABILITIES`` map and also accepts
    the PR-scoped ``TINYASSETS_GITHUB_PR_CAPABILITIES`` capability map.
...
# Plugin-mirror parity. The repo keeps a generated copy of the live ``tinyassets/``
# package at ``packaging/claude-plugin/.../runtime/tinyassets/`` (build_plugin.py is
...
# ``tinyassets/`` path to its runtime copy — otherwise every such PR fails the
...
_MIRRORED_SOURCE_ROOT = "tinyassets/"
...
def _mirror_path_for(path: str) -> str | None:
    """Return the plugin-runtime mirror path for a canonical ``tinyassets/`` path.

    None when the path is outside ``tinyassets/`` (not mirrored) or matches a
...
def _materialize_branch(
    *,
    ...
    # Plugin-mirror parity: mirror every changed canonical ``tinyassets/`` path to
...
def run_github_pr_effector(
    *,
    ...
    # â”€â”€ Operator kill switch (round-3 P1 fix for Codex round-2) â”€â”€â”€â”€â”€â”€â”€â”€
    # ``TINYASSETS_EXTERNAL_WRITE_DRY_RUN`` is a hard override. When
    ...
    # â”€â”€ Gate 1: capability (vault first, then env-vended push token) â”€â”€â”€
    # Vault is the higher-priority source. When no vault is bound, the
    # token is vended through the shared auth provider as a ``push``
    # capability (canonical ``TINYASSETS_GITHUB_PUSH_CAPABILITIES`` map,
    # legacy ``TINYASSETS_GITHUB_PR_CAPABILITIES`` fallback). Lookup is by
