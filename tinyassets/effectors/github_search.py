...
Contract (opaque callable — ``fn(state) -> dict`` of state updates):
  ...
  reads from state:
    ...
    - ``search_query`` (str): whitespace/comma/semicolon-separated terms. A term
      ...
      Dotted module names (``tinyassets.api.wiki``) also match their slash form
      (``tinyassets/api/wiki``).
...
Build decisions (mirror read_repo_files):
  1. Read scope — resolves a token from ``TINYASSETS_GITHUB_READ_CAPABILITIES``
...
_READ_CAPABILITIES_ENV = "TINYASSETS_GITHUB_READ_CAPABILITIES"
...
def _http_get_json(url: str, token: str) -> tuple[object | None, dict | None]:
    ...
    headers = {
        ...
        "User-Agent": "tinyassets-github-search/1.0",
...
def _match_rank(path: str, terms: list[str]) -> int:
    ...
    path ``tinyassets/api/__init__.py`` and a common basename term gets the exact
...
def search_repo_files(state: dict) -> dict:
    ...
    max_results = _int_env("TINYASSETS_GITHUB_SEARCH_MAX_RESULTS", _DEFAULT_MAX_RESULTS)
...
def register_search_repo_files() -> None:
    ...
    Idempotent. Called from tinyassets/effectors/__init__.py at import so a branch
