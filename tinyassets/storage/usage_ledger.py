...
# Tier state, the checkout claim, and the Stripe event ordering that used to live
# below this line are GONE. They moved to `tinyassets/storage/subscription_state.py`
# when billing landed on its own, and that copy has since been corrected several
# times over -- session expiry, an attempt-scoped idempotency key, a guarded release.
# Keeping a second copy here would be two authorities for one fact, and the stale one
# would be the one this module's callers read.
#
# This module now does exactly one thing: count effects. Ask
# `subscription_state.get_tier` who is paying.
