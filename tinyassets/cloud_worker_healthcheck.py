...
Wire-up (deploy/compose.yml ``worker`` service):
    ...
    healthcheck:
      test: ["CMD", "python", "-m", "tinyassets.cloud_worker_healthcheck"]
      interval: 60s
      timeout: 15s
      retries: 3
      start_period: 120s
...
def check(
    ...
    *,
    ...
    try:
        from tinyassets.cloud_worker import _has_pickable_branch_task
...
def main(argv: list[str] | None = None) -> int:
    ...
    healthy, reason = check(
        universe,
        worker_id=os.environ.get("TINYASSETS_WORKER_ID", "").strip() or None,
    )
    if healthy:
        print(reason)
        return 0
