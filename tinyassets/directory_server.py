...
``tinyassets.universe_server`` surface remains available at ``/mcp`` for custom
...
directory_mcp = FastMCP(
    "tinyassets-directory",
    instructions=(
        "TinyAssets helps users discover durable workflow goals, inspect "
        "universes, browse the public knowledge wiki, and submit bounded "
        "requests into the TinyAssets daemon loop. Use these tools only when "
        "the user asks for TinyAssets, durable AI workflow design, shared "
...
def _structured_return(raw):
    ...
    Mirrors the helpers in tinyassets.universe_server applied via PR #493 + #495.
    """
...
def _register_structured_tool(
    ...
    *,
    ...
    # See tinyassets.mcp_schema_utils.
