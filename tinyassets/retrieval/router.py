...
from tinyassets.knowledge.hipporag import HippoRAG
...
from tinyassets.knowledge.raptor import RaptorTree, query_raptor_tree
...
from tinyassets.retrieval.phase_context import should_use_backend
from tinyassets.retrieval.vector_store import VectorStore
...
# ``TINYASSETS_TIERED_SCOPE`` — flag OFF preserves 2b.2-era behavior of
...
def tiered_scope_enabled() -> bool:
    """Read ``TINYASSETS_TIERED_SCOPE``. Default OFF. Stage 2b.3 flag.
    ...
    """
    value = os.environ.get("TINYASSETS_TIERED_SCOPE", "off")
...
def assert_scope_match(row: Any, caller_scope: MemoryScope) -> bool:
    ...
    ``user_id``) are only enforced when ``TINYASSETS_TIERED_SCOPE`` is
...
def _drop_cross_universe_rows(
    ...
    when ``TINYASSETS_TIERED_SCOPE`` is on. When off, only
