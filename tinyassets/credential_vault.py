...
#: A refreshed bundle is a rotation of the SAME credential, so these fields must
#: not move. `account_id` is the stable subject; a changed one means a different
#: login, which is a re-deposit and must go through the authenticated path.
_CODEX_STABLE_IDENTITY_FIELDS = ("account_id",)

#: A refreshed auth.json is the same shape as the deposited one. Anything much
#: larger is not a token bundle, and is refused rather than stored.
_MAX_REFRESHED_AUTH_BYTES = 64 * 1024


def _codex_identity(material: bytes) -> tuple[Any, ...]:
    """The stable identity fields of a codex auth bundle, for comparison."""
    document = json.loads(material.decode("utf-8"))
    if not isinstance(document, dict):
        raise ValueError("codex auth bundle is not an object")
    tokens = document.get("tokens")
    source = tokens if isinstance(tokens, dict) else document
    return tuple(source.get(field) for field in _CODEX_STABLE_IDENTITY_FIELDS)


def promote_refreshed_llm_credential(
    snapshot: LLMCredentialSnapshot | None,
    custody: LLMCredentialCustodyReference | None,
    *,
    universe_dir: str | Path,
) -> bool:
    """Persist a provider-refreshed auth bundle back into the vault.

    Returns True when a rotation was stored. Never raises: a failed promotion
    must not turn a completed provider call into an error, and the worst case is
    exactly the behaviour that existed before this function -- the rotation is
    lost.

    The safety argument, in order:

    1. **Nothing to do unless the bytes changed.** The unrefreshed call is the
       common case and touches no storage at all.
    2. **Shape.** The new material must parse as a JSON object and be bounded.
       Whatever the child wrote, only a credible bundle is stored.
    3. **Identity.** ``account_id`` must be unchanged. A different subject is a
       different login, and that belongs to the authenticated deposit path, not
       to a provider subprocess.
    4. **Compare-and-swap under the deposit lock.** Taken under the same
       exclusive admission a deposit takes, and refused unless the stored record
       still hashes to the custody digest this snapshot was cut from -- so a
       re-deposit that lands mid-call wins and a stale rotation cannot overwrite
       it.

    What this deliberately does NOT do is give the child any path to the vault.
    It reads a file the child wrote inside its own disposable snapshot, and the
    trusted parent decides whether that file becomes the stored credential.
    """
    if snapshot is None or custody is None:
        return False
    if snapshot.service != "codex" or custody.service != "codex":
        return False
    try:
        from tinyassets.provider_assignment import provider_assignment_admission

        directory = _locate_tracked_snapshot(snapshot)
        if directory is None:
            return False
        auth_file = directory / "auth.json"
        if auth_file.is_symlink() or not auth_file.is_file():
            return False
        refreshed = auth_file.read_bytes()
        if not refreshed or len(refreshed) > _MAX_REFRESHED_AUTH_BYTES:
            return False

        universe = Path(universe_dir).resolve(strict=False)
        with provider_assignment_admission().exclusive(universe):
            records = _llm_records(universe, "codex")
            if len(records) != 1:
                return False
            record = records[0]

            current = _subscription_material(universe, "codex", record)
            if refreshed == current:
                return False        # not a rotation; nothing happened

            if _codex_identity(refreshed) != _codex_identity(current):
                logger.warning(
                    "refused a refreshed codex bundle whose account identity changed"
                )
                return False

            # THE COMPARE-AND-SWAP. `snapshot_llm_subscription_credential`
            # proves a snapshot matches its custody with exactly this digest;
            # recomputing it here proves the vault has not moved since.
            current_digest = _canonical_digest({
                "material_digest": "sha256:" + hashlib.sha256(current).hexdigest(),
                "record": record,
            })
            if current_digest != custody._record_digest:
                logger.warning(
                    "refused a refreshed codex bundle: the vault moved during the call"
                )
                return False

            updated = dict(record)
            updated["auth_json_b64"] = base64.b64encode(refreshed).decode("ascii")
            write_credential_vault(universe, [updated])
            logger.info("stored a refreshed codex bundle for the universe")
            return True
    except Exception:  # noqa: BLE001 - a completed call must not fail on this
        logger.warning("could not promote a refreshed codex bundle", exc_info=True)
        return False
