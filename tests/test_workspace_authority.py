...
#: NOTE: no GIT_HOST constant. A revision on 2026-08-31 mapped api.github.com
#: -> github.com in a platform table; the table is gone, because it was one
#: company's products living in the platform. A connection declares ONE host and
#: derives that host, whatever it is -- this fixture declares an API host, so
#: that is what its keys carry. A user whose git lives elsewhere deposits a
#: connection naming that host. See tests/test_git_host_is_not_the_api_host.py.
GITHUB_ENDPOINT = {
    "host": ENDPOINT_HOST,
