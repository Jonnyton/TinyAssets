...
Covers ``tinyassets.auth.provider.vend_github_destination_secret`` and the
...
legacy ``TINYASSETS_GITHUB_PR_CAPABILITIES`` fallback).
"""
...
_GITHUB_SECRET_ENVS = (
    ...
    "TINYASSETS_GITHUB_READ_CAPABILITIES",
)
...
def test_load_secret_map_parses_json_object(monkeypatch):
    ...
    parsed = _load_destination_secret_map("TINYASSETS_GITHUB_PUSH_CAPABILITIES")
    assert parsed == {_DESTINATION: "tok-push", "octo/other": "tok-other"}


def test_load_secret_map_unset_returns_empty():
    assert _load_destination_secret_map("TINYASSETS_GITHUB_PUSH_CAPABILITIES") == {}


def test_load_secret_map_malformed_json_returns_empty(monkeypatch):
    monkeypatch.setenv("TINYASSETS_GITHUB_PUSH_CAPABILITIES", "{not valid json}")
    assert _load_destination_secret_map("TINYASSETS_GITHUB_PUSH_CAPABILITIES") == {}


def test_load_secret_map_non_object_returns_empty(monkeypatch):
    for value in ("[]", '"a string"', "42", "null"):
        monkeypatch.setenv("TINYASSETS_GITHUB_PUSH_CAPABILITIES", value)
        assert _load_destination_secret_map("TINYASSETS_GITHUB_PUSH_CAPABILITIES") == {}
...
def test_vend_push_falls_back_to_legacy_env(monkeypatch):
    ...
    assert vended["source_env_var"] == "TINYASSETS_GITHUB_PR_CAPABILITIES"
...
def test_vend_read_uses_read_env_only(monkeypatch):
    ...
    assert vended["source_env_var"] == "TINYASSETS_GITHUB_READ_CAPABILITIES"
