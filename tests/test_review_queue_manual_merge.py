...
def test_run_review_recovery_builds_client_from_vault(tmp_path):
...
def test_run_review_recovery_no_credential_fails_closed(tmp_path):
...
def test_full_manual_flow_end_to_end_via_daemon(monkeypatch, tmp_path):
    ...
    # Vault carries BOTH the installation token (merge/reads) and the owner user
    # token (review submission).
    write_credential_vault(tmp_path, [
        {"credential_type": "vcs", "service": "github", "destination": _DEST,
         "token": "ghs_inst", "purpose": "write", "account_login": "owner"},
        {"credential_type": "vcs", "service": "github", "destination": _DEST,
         "token": "gho_user", "purpose": "user_review"},
    ])
