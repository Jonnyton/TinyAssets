...
class TestHonestNotice:
    @staticmethod
    def _notice(exc):
        from tinyassets.app_ingress import _failure_notice

        return _failure_notice(exc).lower()

    def test_idle_timeout_is_not_called_capacity(self):
        text = self._notice(ProviderIdleTimeoutError("idle"))
        assert "capacity" not in text
        assert "rate" not in text
        assert "progress" in text

    def test_interactive_deadline_is_not_called_capacity(self):
        text = self._notice(InteractiveDeadlineError("cap"))
        assert "capacity" not in text
        assert "window" in text

    def test_rate_limited_notice_mentions_retry_after(self):
        text = self._notice(ProviderRateLimitedError("rl", retry_after=30))
        assert "rate-limited" in text
        assert "30s" in text

    def test_overloaded_has_its_own_wording_not_rate_limited(self):
        # Blocker I: provider_overloaded must NOT be rendered as "rate-limited".
        text = self._notice(ProviderOverloadedError("ov", retry_after=8))
        assert "overloaded" in text
        assert "rate-limited" not in text
        assert "capacity" not in text
        assert "8s" in text

    def test_authority_held_notice_mentions_reconnect(self):
        from tinyassets.exceptions import ProviderAuthorityHeldError

        text = self._notice(ProviderAuthorityHeldError("held"))
        assert "capacity" not in text
        assert "reconnect" in text or "revoked" in text

    def test_aggregate_error_carrying_idle_class_is_honest(self):
        # The single-provider served chain raises AllProvidersExhaustedError
        # but carries the dominant failure_class — the notice must follow it.
        exc = AllProvidersExhaustedError(
            "Served provider exhausted", failure_class="provider_idle_timeout"
        )
        text = self._notice(exc)
        assert "capacity" not in text
        assert "progress" in text

    def test_unclassified_exhaustion_is_an_honest_generic_error_not_capacity(self):
        # Blocker I: a bare exhaustion with NO failure_class means we do not KNOW
        # it was a capacity/rate-limit event, so we must NOT claim it was one.
        # (The old code substring-guessed "capacity" from the word "exhausted" —
        # the exact mislabel this whole change removes.) It renders an honest
        # generic error instead.
        exc = AllProvidersExhaustedError("All providers exhausted for role=writer")
        text = self._notice(exc).lower()
        assert "capacity" not in text
        assert "rate limit" not in text
        assert "error finishing that turn" in text


# ---------------------------------------------------------------------------
# 5.7 Backward-safe: a non-streaming / other provider still returns a
