...
def test_record_candidate_marks_result_blobs_referenced(tmp_path: Path) -> None:
    ...
    class BlobStoreSpy:
        ...
        def validate_reference(self, blob_ref, **binding):
            proof = self._inner.validate_reference(blob_ref, **binding)
            self.validated.append((blob_ref, proof, binding))
            return proof

        def mark_referenced(self, proof, **binding):
            self.marked.append((proof, binding))
            return self._inner.mark_referenced(proof, **binding)
    ...
    assert len(spy.marked) == len(spy.validated)
    for (_, validated_proof, _), (marked_proof, _) in zip(
        spy.validated, spy.marked, strict=True
    ):
        assert marked_proof is validated_proof
