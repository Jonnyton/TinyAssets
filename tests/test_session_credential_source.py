...
def test_the_bearer_handle_is_never_used_as_a_filename():
    ...
    assert 'f"{handle}.json"' not in SRC, (
        "the handle IS a bearer credential; using it as a filename publishes every "
        "live session to anything that can list the directory"
    )
    assert SRC.count("_handle_path_key(handle)") >= 3


def test_the_stored_filename_does_not_contain_the_handle():
    handle = onboarding._mint_refresh_session("rt_secret")
    names = [p.name for p in onboarding._refresh_store_dir().iterdir()]
...
def test_the_digest_is_keyed_when_a_key_is_configured(monkeypatch):
    ...
    monkeypatch.setenv("TINYASSETS_BILLING_ENTITLEMENT_KEY", "k1")
    a = onboarding._handle_path_key("H" * 43)
    monkeypatch.setenv("TINYASSETS_BILLING_ENTITLEMENT_KEY", "k2")
...
def test_an_unconfigured_deployment_still_does_not_publish_the_handle(monkeypatch):
    monkeypatch.delenv("TINYASSETS_BILLING_ENTITLEMENT_KEY", raising=False)
