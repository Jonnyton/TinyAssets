...
def test_resume_refinery_owner_fails_closed_when_pr_is_not_preexisting(
    tmp_path: Path,
) -> None:
    results_dir = tmp_path / "results"
    results_dir.mkdir()
    ...
    assert recovered is False
    assert state["last_consumed_attempt"] == 0
    assert state["refinery_assignment"] == assignment
...
def test_verify_preexisting_open_pr_owner_requires_exact_repo_and_older_pr(
    ...
    def runner(command: list[str], **_: object) -> subprocess.CompletedProcess[str]:
        ...
        return subprocess.CompletedProcess(
            ...
            0,
            stdout=json.dumps(
                {"state": "OPEN", "createdAt": "2026-08-03T16:00:00Z"}
            ),
            stderr="",
        )
