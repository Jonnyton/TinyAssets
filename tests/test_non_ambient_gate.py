...
def _no_pinned_writer(monkeypatch, platform_vault_env):
...
def test_declared_broken_binding_fails_closed_even_with_flag_off(tmp_path, monkeypatch):
    """The r22 credential-integrity chokepoint is stronger than the rollout flag:
    an unreadable/declared-broken credential state never reaches ambient auth."""
    monkeypatch.delenv(NON_AMBIENT_WORK_ENV, raising=False)
...
def test_flag_on_fails_loud_on_misconfigured_binding(tmp_path, monkeypatch):
    ...
    assert spawn_calls == []
    assert state.retired_fail_closed_count == 2
    assert state.idle_until_bound_count == 0
