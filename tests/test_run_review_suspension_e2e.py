...
def test_live_recovery_completes_autonomous_continuation(monkeypatch, tmp_path):
    """r18: the production recovery caller passes verifier + App identity."""
    run_id = _suspend_a_real_run(tmp_path, monkeypatch, preference="auto")
    rq.decide_and_resume(
        tmp_path,
        destination=_DEST,
    ...
    recovered = runs.run_review_recovery_for_universe(tmp_path)

    continuation = recovered["replay_continuations"][0]
    assert continuation["applied"] is True
    assert continuation["review_workflow_state"] == "approved_auto_merge_enabled"
    assert runs.get_run(tmp_path, run_id)["status"] == runs.RUN_STATUS_COMPLETED


def test_live_recovery_starts_reshape_revision(monkeypatch, tmp_path):
    """r18: the production recovery caller supplies its revision starter."""
    run_id = _suspend_a_real_run(tmp_path, monkeypatch)
    route = {
    ...
    rq.decide_and_resume(
        ...
        expected_head_sha=_HEAD,
        directive=directive,
        reshape={**route, "recorded_call": _REQUEST_CHANGES_CALL},
    )
    api = FakeApi()
    ...
    monkeypatch.setattr(
        "tinyassets.credential_broker.github_connection_metadata",
        lambda *_args, **_kwargs: {"account_login": "owner"},
    )
    monkeypatch.setattr(
        runs,
        "_start_review_revision",
        lambda universe_dir, route_back: (
            starts.append((universe_dir, route_back)) or "revised-run"
        ),
    )
    ...
    continuation = recovered["replay_continuations"][0]
    assert continuation["applied"] is True
    assert continuation["review_workflow_state"] == "reshaped_revising"
    assert starts == [(tmp_path, route)]
