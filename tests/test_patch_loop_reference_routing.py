...
def _enable_sandbox_enforcement(monkeypatch):
    ...
    # end-to-end. Individual routing invocations install S3's shared TYPED
    # worker simulator; a boolean readiness flag alone is not an execution
    # boundary. The legacy availability seam remains here for S1-specific probes.
    monkeypatch.setattr(
...
def _invoke_with_provider(
    branch, provider: Callable[..., str],
) -> tuple[list[str], dict[str, Any]]:
    """Invoke through S3's typed worker simulator and return visit order/state."""
    visited: list[str] = []
    ...
    with pytest.MonkeyPatch.context() as worker_patch:
        ...
        install_worker_sim(worker_patch)
        ...
        def _worker_provider(
            ...
            *,
            role: str = "writer",
            config: Any | None = None,
            universe_context: Any | None = None,
        ) -> str:
            return provider(prompt, system, role=role, config=config)

        worker_patch.setattr(
            "tinyassets.providers.call.call_provider", _worker_provider,
        )
...
def _invoke_compiled(
    ...
    return _invoke_with_provider(
        branch, _make_provider(verify_verdicts, gate_verdict),
    )
...
def test_send_back_owner_gate_routes_back_to_draft_and_never_merge():
    ...
    visited, result = _invoke_with_provider(_reference_branch(), _call)
