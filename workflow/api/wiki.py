...
_FILE_BUG_COMPAT_IGNORED_DEFAULTS: dict[str, Any] = {
    "dry_run": True,
...
def _rejected_file_bug_kwargs(kwargs: dict[str, Any]) -> list[str]:
    """Return truthy unsupported file_bug kwargs that should be rejected."""
    rejected: list[str] = []
    for key, value in kwargs.items():
        if value in ("", None, False):
            continue
        if (
            key in _FILE_BUG_COMPAT_IGNORED_DEFAULTS
            and value == _FILE_BUG_COMPAT_IGNORED_DEFAULTS[key]
        ):
            continue
        rejected.append(key)
...
def _wiki_file_bug(
    ...
    **_kwargs: Any,
    ...
    """
    rejected_kwargs = _rejected_file_bug_kwargs(_kwargs)
    if rejected_kwargs:
        return json.dumps({
            "error": (
