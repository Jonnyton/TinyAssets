...
14. **Merged is not deployed.** A merge to `main` can build nothing: merges performed by the Actions app via `GITHUB_TOKEN` do not trigger workflow runs, which is how five PRs landed on 2026-07-21 and none reached production. Before calling a fix shipped, check the sha production is actually serving — `get_status` -> `release_state.git_sha` (written to the host volume by `deploy-prod.yml` at deploy time) — and confirm it contains your commit. `release-reconcile.yml` reconciles this every 15 minutes, so drift self-heals; a *claim* of shipped still needs the sha, not the merge.
