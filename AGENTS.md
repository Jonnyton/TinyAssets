...
**High-risk PRs stay draft until exact-head approval.** Auth, storage, migration, concurrency, public-surface, and data-loss-risk PRs MUST be opened as drafts so auto-enrollment cannot merge them ahead of independent review. Mark the PR ready only after the required reviewer posts an approval artifact naming the unchanged exact head SHA and every blocking finding is resolved. Before any later push, rebase, update-branch action, or other head-changing update, convert the PR back to draft; keep it draft through the update and fresh exact-head approval, then mark it ready again.
